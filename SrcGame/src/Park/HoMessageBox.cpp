...
int HoMessage::Init()
{
	...
	balloonTalk[1] = CreateTextureMaterial("game\\GUI\\Interface\\Chat\\BalloonTalk\\leftTop.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	balloonTalk[2] = CreateTextureMaterial("game\\GUI\\Interface\\Chat\\BalloonTalk\\middleTop.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	balloonTalk[3] = CreateTextureMaterial("game\\GUI\\Interface\\Chat\\BalloonTalk\\rightTop.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	balloonTalk[5] = CreateTextureMaterial("game\\GUI\\Interface\\Chat\\BalloonTalk\\leftMiddle.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	balloonTalk[6] = CreateTextureMaterial("game\\GUI\\Interface\\Chat\\BalloonTalk\\rightMiddle.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	balloonTalk[7] = CreateTextureMaterial("game\\GUI\\Interface\\Chat\\BalloonTalk\\leftBottom.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	balloonTalk[8] = CreateTextureMaterial("game\\GUI\\Interface\\Chat\\BalloonTalk\\middleBottom.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	balloonTalk[9] = CreateTextureMaterial("game\\GUI\\Interface\\Chat\\BalloonTalk\\rightBottom.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
...
void HoMessage::DrawMessageBox(int x, int y, char* message, int boxAttrib, int Color)
{
	...
	if (x + 16 * 2 + messageWidth * 12 >= SETTINGSHANDLER->getWidth())
		x = SETTINGSHANDLER->getWidth() - messageWidth * 12 - 16 * 2;

	if (y + 16 * 2 + messageHeight * 12 >= SETTINGSHANDLER->getHeight())
		y = SETTINGSHANDLER->getHeight() - messageHeight * 12 - 16 * 2;
	...
	dsTextLineOut(0, x + textStartPosX, TextStartPosY + 16, message, lstrlen(message));
...
void HoMessage::DrawClanName(int x, int y, char* clanName)
{
	...
	dsTextLineOut(0, x + 15, y + 3, clanName, lstrlen(clanName));

	SetFontTextColor(RGB(230, 255, 160));
	dsTextLineOut(0, x + 15, y + 2, clanName, lstrlen(clanName));
...
void HoMessage::DrawClanMark(LPDIRECT3DTEXTURE9 clanMark, char* clanName, int x, int y, BOOL frameFlag)
{
	...
	//Tamanho Img Clan
	DrawSprite(x + 4, y + 4, clanMark, 0, 0, 32, 32, true);

	if (clanName != NULL)
		DrawClanName(x, y, clanName);
}
void HoMessage::DrawClanMark1(LPDIRECT3DTEXTURE9 clanMark, char* clanName, int x, int y, BOOL frameFlag)
{
	...
	//Tamanho Img Clan
	DrawSprite(x + 2, y + 5, clanMark, 0, 0, 16, 16, true);
...
void HoMessage::DrawTwoLineMessage(int x, int y, char* message1, char* message2, DWORD color1, DWORD color2, LPDIRECT3DTEXTURE9 clanMark, BOOL selected, int bellaMarkIndex)
{
	...
	{
		if (clanMark == NULL)
			dsTextLineOut(0, x + 9 + messageWidth * 6 / 2 - messageTempLen1 * 6 / 2 + 0, y + 5 + 2, MessageOne, messageTempLen1);
		else
			dsTextLineOut(0, x + 9 + 6 * 3 + 30, y + 5 + 2, MessageOne, messageTempLen1);
	}
	...
	if (messageTempLen2 > 0)
		dsTextLineOut(0, x + 9 + messageWidth * 6 / 2 - messageTempLen2 * 6 / 2 + 0, y + 5 + 2 + 16, MessageTwo, messageTempLen2);
...
void HoMessage::DrawThreeLineMessage(int x, int y, char* message1, char* message2, char* message3, DWORD color1, DWORD color2, DWORD color3, LPDIRECT3DTEXTURE9 clanMark, BOOL selected, int bellaMarkIndex)
{
	...
	if (messageTempLen3 > 0)
		dsTextLineOut(0, x + 9 + messageWidth * 6 / 2 - messageTempLen3 * 6 / 1 + 10, y + 5 + 2 + 30, MessageThree, messageTempLen3);
...
void HoMessage::DrawFourLineMessage(int x, int y, char* message1, char* message2, char* message3, char* message4, DWORD color1, DWORD color2, DWORD color3, DWORD color4, LPDIRECT3DTEXTURE9 clanMark, BOOL selected, int bellaMarkIndex)
{
	char MessageOne[64] = { 0 };
	char MessageTwo[64] = { 0 };
	char MessageThree[64] = { 0 };
	char MessageFour[64] = { 0 };
	...
	int messageTempLen2 = strlen(MessageTwo);
	int messageTempLen3 = strlen(MessageThree);
	int messageTempLen4 = strlen(MessageFour);
	...
	if (messageTempLen1 > messageTempLen2)
		...
		messageLen = messageTempLen2;
	else if (messageTempLen3 > messageTempLen4)
		messageLen = messageTempLen3;
	else messageLen = messageTempLen4;
	...
	{
		if (clanMark == NULL)
			dsTextLineOut(0, x + 9 + messageWidth * 6 / 2 - messageTempLen1 * 6 / 2 + 20, y + 5 + 2, MessageOne, messageTempLen1);
		else
			dsTextLineOut(0, x + 9 + messageWidth * 6 / 2 - messageTempLen1 * 6 / 2 + 20, y + 5 + 2, MessageOne, messageTempLen1);
	}
	...
	if (messageTempLen2 > 0)
		dsTextLineOut(0, x + 9 + messageWidth * 6 / 2 - messageTempLen2 * 6 / 2 + 20, y + 5 + 2 + 16, MessageTwo, messageTempLen2);
	...
	if (messageTempLen3 > 0)
		dsTextLineOut(0, x + 9 + messageWidth * 6 / 2 - messageTempLen3 * 6 / 2 + 20, y + 5 + 2 + 30, MessageThree, messageTempLen3);


	if (color4 == 0)
		SetFontTextColor(RGB(255, 255, 255));
	else
		SetFontTextColor(color4);


	if (messageTempLen4 > 0)
	{
		dsTextLineOut(0, x + 9 + messageWidth * 6 / 2 - messageTempLen3 * 6 / 2 + 30, y + 5 + 2 + 44, MessageFour, messageTempLen4);
	}
...
int HoMessage::textarrange(const std::string& text, bool clanm)
{
	int len = 0;
	int	curLine = 0;
	int	curChar = 0;
	...
	if (len < 64 && cEmoji.getTextSize(text) <= 32)
	{
		text.copy(&textdata[0][curChar], len);
	}
	else
	{
		for (int i = 0; i < len; i++)
		{
			textdata[curLine][curChar++] = text[i];
			if ((text[i] == '(' && !bfound) || (text[i] == ')' && bfound)) {
				found = curChar - 1;
				bfound = !bfound;
			}
			...
			if (curChar > 32)
			{
				if (curChar - found <= 12 && bfound)
				{
					lstrcpy(&textdata[curLine + 1][0], &textdata[curLine][found]);
					textdata[curLine][found] = 0;
					curChar = curChar - found;
					curLine++;
				}
				else if (text[curChar - 1] == ' ')
				{
					curLine++;
					curChar = 0;
				}
...
void HoMessage::DrawClanMessage(int x, int y, char* message, int lineSize, DWORD color)
{
	...
	if (lineSize != 0)
	{
...
#endif
			dsTextLineOut(0, x + 11, y + 13 + m * 14, buffer, lstrlen(buffer));
		...
		dsTextLineOut(0, x + 11, y + 13, message, lstrlen(message));
...
void HoMessage::DrawCharacterMessage(int x, int y, char* message, int lineSize, LPDIRECT3DTEXTURE9 clanMark, char* clanName, DWORD color1, BOOL selected, int bellaMarkIndex)
{
	...
	{
		txtsz = cEmoji.getTextSize(textdata[i]);
		GetFontTextExtentPoint(0, textdata[i], txtsz, &sizecheck);
	...
	if (MessageFlag || CharNameFlag)
	{
		...
		if (x < 0 || y < 0 || (x + 18 + messageWidth * 6) >= SETTINGSHANDLER->getWidth() || (y + 5 * 2 + ((messageHeight - 1) * 16 + 14) >= SETTINGSHANDLER->getHeight()))
		{
			return;
		}
	...
	if (ClanMarkFlag)
	{
		...
		if (CharNameFlag == false && MessageFlag == false)
			DrawClanMark1(clanMark, clanName, clanX + 3, clanY, true);
		else
			DrawClanMark1(clanMark, clanName, clanX + 3, clanY);
	...
	if (MessageFlag)
	{
		...
		{
			dsTextLineOut(0, x + 9, y + 2 + m * textsize.y + 2, textdata[m], lstrlen(textdata[m]));
		}
#endif
	}
...
#ifdef _LANGUAGE_BRAZIL
		dsTextLineOut(0, x + 9, y + 2 + 2, cname, lstrlen(cname));
#endif
	}
...
void HoMessage::DrawShopSellMessage(int x, int y, char* message, int lineSize, LPDIRECT3DTEXTURE9 clanMark, DWORD color1, BOOL selected)
{
	...
	for (int i = 0; i < linesize; i++)
	{
		GetFontTextExtentPoint(0, textdata[i], lstrlen(textdata[i]), &sizecheck);
		if (textsize.x < sizecheck.cx) textsize.x = sizecheck.cx;
		if (textsize.y < sizecheck.cy) textsize.y = sizecheck.cy;
	}
	...
	if (MessageFlag || CharNameFlag)
	{
		...
		if (x < 0 || y < 0 ||
			(x + 18 + messageWidth * 6) >= SETTINGSHANDLER->getWidth() ||
			(y + 5 * 2 + ((messageHeight - 1) * 16 + 14) >= SETTINGSHANDLER->getHeight()))
			return;
	...
	if (ClanMarkFlag)
	{
		...
		DrawClanMark1(clanMark, NULL, clanX + 3, clanY);
	}
	...
	if (MessageFlag)
	{
		...
		for (int m = 0; m < linesize; m++)
		{
			dsTextLineOut(0, x + 9, y + 5 + m * textsize.y + 2, textdata[m], lstrlen(textdata[m]));
		}
#endif
	}
...
#ifdef _LANGUAGE_BRAZIL
		dsTextLineOut(0, x + 9, y + 5 + 2, cname, lstrlen(cname));
#endif
	}
...
void HoMessage::Draw(int x, int y, char* message, int lineSize, int addLine, int boxAttrib)
{
	...
	{
		if (x < 0 || y < 0 ||
			(x + 16 * 2 + messageWidth * 6) >= SETTINGSHANDLER->getWidth() || y < 0 ||
			x < 0 || (y + 16 * 2 + messageHeight * 6) >= SETTINGSHANDLER->getHeight() ||
			(x + 16 * 2 + messageWidth * 6) >= SETTINGSHANDLER->getWidth() || (y + 16 * 2 + messageHeight * 6) >= SETTINGSHANDLER->getHeight())
			return;
	}
	...
	if (lineSize != 0)
	{
		...
		for (m = 0; m < messageHeight; m++)
		{
			...
			dsTextLineOut(0, x + 17, TextStartPosY + 16 + m * 15, buffer, lstrlen(buffer));
		...
		dsTextLineOut(0, x + 17, y + 16, message, lstrlen(message));
...
int DrawFourLineMessage(int x, int y, char* messageOne, char* messageTwo, char* messageThree, char* messageFour, DWORD color1, DWORD color2, DWORD color3, DWORD color4, LPDIRECT3DTEXTURE9 clanMark, BOOL selected, int bellaMarkIndex)
{
	ChatMessage.DrawFourLineMessage(x, y, messageOne, messageTwo, messageThree, messageFour, color1, color2, color3, color4, clanMark, selected, bellaMarkIndex);
	return TRUE;
}
