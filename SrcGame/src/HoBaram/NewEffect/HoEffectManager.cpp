...
bool HoEffectManager::StopMaintainEffect(int code, smCHAR *pAttachChar, int iHead)
{
	int maintainEffectCount = m_MaintainEffectTypeList.size();
...
HoEffectWing::~HoEffectWing()
{
	delete(m_pEffectGroup);
}
void HoEffectWing::Init(smCHAR *pChar, HoEffectGroup *effectGroup, int iHead)
{
	m_pAttachChar = pChar;
	m_pEffectGroup = effectGroup;
	m_bLiveFlag = true;
	...
	pos.x = (float)m_pAttachChar->pX / 256.f;
	pos.y = (float)(m_pAttachChar->pY - 1000) / 256.f;
	pos.z = (float)m_pAttachChar->pZ / 256.f;
	...
	if (pChar->AnimPattern)
		m_pObj = pChar->AnimPattern->GetObjectFromName("Bip in01");
...
void HoEffectWing::UpdateView(int x, int y, int z, int ax, int ay, int az)
{
	if (m_pAttachChar == NULL || m_pAttachChar->FlagShow == 0)
		return;
...
void HoEffectWing::Main(float elapsedTime)
{
	if (m_pAttachChar == NULL || m_pObj == NULL)
	{
		m_bLiveFlag = false;
		return;
	}
	...
	AnimObjectTree(m_pObj, m_pAttachChar->frame, 0, (-m_pAttachChar->Angle.y + ANGLE_180)&ANGCLIP, 0);
	...
	smRotateYMatrix(mMatix, 1250);

	int x = 0;
	int y = 0;
	int z = 0;

	switch (m_pAttachChar->smCharInfo.JOB_CODE)
	{
	case JOBCODE_FIGHTER:
	case JOBCODE_MECHANICIAN:
	case JOBCODE_KNIGHT:
		x = -3; y = -3; z = 5;
		if (m_pAttachChar->OnStageField >= 0 && StageField[m_pAttachChar->OnStageField]->State != FIELD_STATE_VILLAGE)
		{
	...
	smTranslateMatrix(mMatix, x, y, z);
	smMatrixMult(*mWorld, mMatix, *mWorld);

	point3 pos;

	pos.x = (float)(((m_pAttachChar->pX + mWorld->_41) / 256.f));
	pos.y = (float)(((m_pAttachChar->pY + mWorld->_43) / 256.f) - 36.0f);
	pos.z = (float)(((m_pAttachChar->pZ + mWorld->_42) / 256.f));

	point3 angle;

	angle.x = (float)m_pAttachChar->Angle.x;
	angle.y = (float)m_pAttachChar->Angle.y + mWorld->_43 / 256;
	angle.z = (float)m_pAttachChar->Angle.z;

	if (m_pEffectGroup)
		m_pEffectGroup->SetCalcTranslate(&pos, &angle);

	if (m_pAttachChar->Flag == FALSE || !m_pAttachChar->MotionInfo || (m_pEffectGroup && !m_pEffectGroup->IsLive()) || m_pAttachChar->dwWingEffectCode == 0)
		m_bLiveFlag = false;
