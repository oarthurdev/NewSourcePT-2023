...
void ParkWingEffect(smCHAR *pChar, int code)
{
	//if (!HoEffectManager::GetInstance()->StopMaintainEffect(SKILL_WING_EFFECT, pChar, code))
	{
		HoEffectGroup *effectGroup = NULL;
		switch (code)
		{
		...
		case sinCW1 | sin06:
			...
			break;
		case sinCW1 | sin07:
			effectGroup = MainWindow.LoadScript("Effect\\NewEffect\\Angel_wings.lua");
			break;
		case sinCW1 | sin08:
		...
		if (effectGroup)
		{
			...
			effectType->Init(pChar, effectGroup, code);
