...
public:
	...
	int		GetHeadCode() { return m_iHead; }
};
...
public:
	...
	bool	StopMaintainEffect(int code, smCHAR *pAttachChar, int iHead);
	void	Clear();
	BOOL    GetSkillCode(smCHAR *pChar, int code);
};
class HoEffectWing :public HoEffectTypeInterface
{
private:
	bool			m_bMyCharacterFlag;
	HoEffectGroup	*m_pEffectGroup;
	smOBJ3D			*m_pObj;
public:
	HoEffectWing() : m_pEffectGroup(NULL),
		m_pObj(NULL),
		m_bMyCharacterFlag(false) {};
	~HoEffectWing();

	void Init(smCHAR *pChar, HoEffectGroup *effectGroup, int iHead);
	void UpdateView(int x, int y, int z, int ax, int ay, int az);
	void Main(float elapsed);
};
