...
{
	int cnt = 0; //Rubinho crash teleport crash
...
void CKeepSkillEffect::Main()
{
	...
	if (EndFlag == FALSE)
	{
		if (CODE == HIGH_LEVEL_MERIT_SKILL || CODE == REIPVP)
		{
			if (Time > Max_Time - 140 || (!MyCharacterFlag && pChar->Flag == 0))
			{
				Time = Max_Time - 140;
				if (ParticleID != -1)
					g_NewParticleMgr.SetFastStop(ParticleID);
		...
		else if (Time > Max_Time - 140 || (!MyCharacterFlag && pChar->Flag == 0) ||
			(!pChar->MotionInfo || pChar->MotionInfo->State == CHRMOTION_STATE_DEAD) ||
			(pChar->OnStageField >= 0 && StageField[pChar->OnStageField]->State == FIELD_STATE_VILLAGE))
		{
			Time = Max_Time - 140;
...
//modopvp
AssaSkillModoPvP::AssaSkillModoPvP()
{
	ParticleID = -1;
	MyCharacterFlag = FALSE;
}
...
void AssaSkillModoPvP::Start(smCHAR* character, int liveCount)
{
	pChar = character;
	if (pChar == NULL)
		return;
	...
	ParticleID = g_NewParticleMgr.Start("WarMode", Posi);
...
void AssaShelltomUpWeapon2::Start(smCHAR *pCharacter, int timeCount, int shellTomCode)
{
	...
	Face.TexRect.right = 1;
	Face.TexRect.bottom = 1;
	//Rubinho corre豫o teleporte, Apagar os ShelltomCode acima...
	if (shellTomCode == 50)
	{
		Width = 20 * fONE;
	}
...
void AssaShelltomUpWeapon2::Main()
{
	...
	ZeroMemory(WeaponPos, sizeof(POINT3D)*MAX_WEAPON_POS);
	ZeroMemory(WeaponPos2, sizeof(POINT3D)*MAX_WEAPON_POS);
	/*memset(WeaponPos, 0, sizeof(WeaponPos));*/
	int index = 0;
	int StartZPos = 0;
	int WeaponSize = 0;
	...
	if(pChar->HvRightHand.dwItemCode != 0)
	{
		...
		else if ((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) == sinWD1)
		{
		StartZPos = 0;
		WeaponSize = pChar->HvRightHand.SizeMax;
		WeaponPosCount = WeaponSize / 1000;
		...
		else if ((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) == sinWN1)
		{
		StartZPos = 0;
		WeaponSize = pChar->HvRightHand.SizeMax;
		WeaponPosCount = WeaponSize / 1000;
		...
		if ((pChar->HvLeftHand.dwItemCode & sinITEM_MASK2) == sinWD1)
		{
			mWorld2 = &pChar->HvLeftHand_Matrix;

			StartZPos = 0;
			WeaponSize = pChar->HvLeftHand.SizeMax;
			WeaponPosCount2 = WeaponSize / 1000;

			for (index = 0; index < MAX_WEAPON_POS; index++)
				WeaponPos2[index].z = StartZPos + index * 1000;
		}
	}
	else if (pChar->HvLeftHand.dwItemCode != 0)
	{
...
void AssaShelltomUpWeapon2::Draw(POINT3D *pPosi, POINT3D *pAngle)
{
	...
	if (GlowFlag && GlowPosIndex >= 0 && GlowPosIndex < 9)
	{
		smRender.Init(); //데이터 초기화 
		smRender.SetMaterialGroup(smMaterialGroup);				 //읽어드린 메트리얼을 연결 
