...
void ParkAssaParticleDamage(smCHAR* pChar, int Kind)
{
	char particle[32];
	...
	{
	case 1:
		lstrcpy(particle, "DefenseInfo");
		break;
	case 2:
		lstrcpy(particle, "BlockInfo");
		break;
	case 3:
		lstrcpy(particle, "EvadeInfo");
		break;
	}

	int i = GetAssaEffect();
	cAssaTraceCharMeshParticle *traceMeshParticle = new cAssaTraceCharMeshParticle;
	traceMeshParticle->Start(pChar, "Bip01 Head", particle);
	traceMeshParticle->SetScale(float(pChar->smCharInfo.wPlayClass[1]) / 256.f);
	cAssaEffect[i] = traceMeshParticle;
	cAssaEffect[i]->CODE = 1415;
}
...
//Efeito Modo PvP
void AssaSkill_ModoPvP(smCHAR* pDestChar, int liveCount)
{

	if (AddAssaCodeEffectTime(pDestChar, BUFF_WARMODE, liveCount * 70 + 140) == false)
	{
		POINT3D charPos;
		charPos.x = pDestChar->pX;
		charPos.y = pDestChar->pY;
		...
		int i = GetAssaEffect();
		AssaSkillModoPvP* ModoPvP = new AssaSkillModoPvP;
		ModoPvP->Start(pDestChar, liveCount);
		cAssaEffect[i] = ModoPvP;
	}
}
