...
void HoParty::resize()
{
	...
	int MidX = (int)((float)SETTINGSHANDLER->getWidth() * 0.92f);
	int MidY = (int)((float)SETTINGSHANDLER->getHeight() * 0.1f);

	int h = (int)((float)SETTINGSHANDLER->getHeight() / 6);
	int w = (int)((float)SETTINGSHANDLER->getHeight() * 1.34f / 8);
...
void HoParty::DrawPartyMessage(int x, int y)
{
	...
	if(MessageBoxOpenFlag == TRUE)
	{
		...
		dsTextLineOut(0, x + 36, y + 16, PartyMessageBoxChrName, lstrlen(PartyMessageBoxChrName));
		dsTextLineOut(0, x + 10, y + 38, HoText_PartyMessage, lstrlen(HoText_PartyMessage));
...
int HoParty_KTJ::Draw()
{
	...
	for(i = qNameS; i < qNameE; i++)
	{
		...
		if(questSelN == 5)
		{
			...
			dsTextLineOut(0, qNamePosx - 30, qNamePosy, qNameb[i], lstrlen(qNameb[i]));
#else
			dsTextLineOut(0, qNamePosx, qNamePosy, qNameb[i], lstrlen(qNameb[i]));
#endif
		}
		else
		{
#ifdef _LANGUAGE_ARGENTINA
			dsTextLineOut(0, qNamePosx - 30, qNamePosy, qNameb_E[i], lstrlen(qNameb_E[i]));
#else
			dsTextLineOut(0, qNamePosx, qNamePosy, qNameb_E[i], lstrlen(qNameb_E[i]));
#endif
		}
...
menu_dongru:
	...
	for(int index = 0; index < PartyMemberCount; index++)
	{
		...
		dsTextLineOut(0, TextStartPos.x + 11, TextStartPos.y + 30 * index + 1, PartyMember[index].Name, lstrlen(PartyMember[index].Name));
		memset(levelBuffer, 0, sizeof(levelBuffer));

		wsprintf(levelBuffer, "%s%d", "L", PartyMember[index].Level);
		dsTextLineOut(0, TextStartPos.x + 110, TextStartPos.y + 5 + 30 * index + 1, levelBuffer, lstrlen(levelBuffer));
	}
	...
	for(int index = 0; index < PartyMemberCount; index++)
	{
		...
		if(pCursorPos.x > TextStartPos.x && pCursorPos.y > TextStartPos.y + 30 * index - 10 &&
			pCursorPos.x < TextStartPos.x + 110 && pCursorPos.y < TextStartPos.y + 30 * index + 18)
			SetFontTextColor( RGB(255, 150, 0));
		dsTextLineOut(0, TextStartPos.x + 10, TextStartPos.y + 30 * index, PartyMember[index].Name, lstrlen(PartyMember[index].Name));
		...
		SetFontTextColor( RGB(255, 190, 107));
		dsTextLineOut(0, TextStartPos.x + 110, TextStartPos.y + 5 + 30 * index, levelBuffer, lstrlen(levelBuffer));
	}
...
menu_frList:
	...
	if(cN_MenuN == 0)
	{
		...
		dsTextLineOut(0, menuPos[mSu].left + 20, menuPos[mSu].top - 24, HoTextFriendNear, lstrlen(HoTextFriendNear));
		...
		for(i = 0; i < 7; i++)
		{
			...
			dsTextLineOut(0, menuPos[mSu].left, menuPos[mSu].top, latestb[i], lstrlen(latestb[i]));
	...
	else if(cN_MenuN == 1)
	{
		...
		SetFontTextColor( RGB(255, 220, 0));
		dsTextLineOut(0, menuPos[mSu].left + 15, menuPos[mSu].top - 24, buf, strlen(buf));
		...
		for(i = fNameS; i < fNameE; i++)
		{
			...
			else if(j == i)
			{
				...
				SetFontTextColor( RGB(255, 255, 255));
			dsTextLineOut(0, menuPos[mSu].left, menuPos[mSu].top, fNameb[i], lstrlen(fNameb[i]));
	...
	else
	{
		...
		SetFontTextColor( RGB(255, 220, 0));
		dsTextLineOut(0, menuPos[mSu].left + 10, menuPos[mSu].top - 24, buf, strlen(buf));
		...
		for(i = dNameS; i < dNameE; i++)
		{
			...
			if(j == i)
			{
				...
				SetFontTextColor( RGB(255, 255, 255));
			dsTextLineOut(0, menuPos[mSu].left, menuPos[mSu].top, dNameb[i], lstrlen(dNameb[i]));
...
void HoParty_KTJ::Draw_questExplane(int x, int y)
{
	...
	SetFontTextColor( RGB(255, 190, 107));
	dsTextLineOut(0, x + 30, y - 20, q_explationTitle, lstrlen(q_explationTitle));

	for(i = q_explationS; i < q_explationS + 8; i++)
	{
		SetFontTextColor( RGB(255, 190, 107));
		dsTextLineOut(0, x, y, q_explation[i], lstrlen(q_explation[i]));
		y += 20;
	}
