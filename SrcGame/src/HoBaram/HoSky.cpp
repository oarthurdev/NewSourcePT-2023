...
int HoSky::Draw(int x, int y, int z, int ax, int ay, int az)
{
	...
	if(smConfig.getSecurity() >= AccountTypes::SEC_MODERATOR)
	{
		...
		if(ColorInfoFlag)
		{
			...
			dsTextLineOut(0, 11, 101, buffer, strlen(buffer));

			SetFontTextColor( RGB(255, 255, 255));
			dsTextLineOut(0, 10, 100, buffer, strlen(buffer));
			...
			SetFontTextColor( RGB(0, 0, 0));
			dsTextLineOut(0, 11, 116, buffer, strlen(buffer));

			SetFontTextColor( RGB(255, 255, 255));
			dsTextLineOut(0, 10, 115, buffer, strlen(buffer));
...
void HoSky::Create()
{
	...
	if (SETTINGSHANDLER->getBitDepth() == 32)
	{
