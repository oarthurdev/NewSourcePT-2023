...
#include "HighLevelUserMerit/HighLevelUserMerit.h"
...
/// <summary>
///VARIAVEIS EXTERNAS RANKING IN GAME
/// </summary>
extern char NameRank[10][32];
extern char NameClanRank[10][32];
extern int LevelRank[10];
extern int ClasseRank[10];
extern int KillsRank[10];
extern char liderClanRank[10][32];
extern int pointsClanRank[10];
extern int pointsRank[10];
extern int membrosclanRank[10];
extern int DeadsRank[10];
extern int Type;
extern int contador;

extern char MsgMix[16][256];
extern int Sheltom[14];
extern DWORD Item[10];
extern float Adicional[10];
...
SocketData* ConnectServer_Main()
{
	...
	strcpy_s(server2, sizeof(server2), SETTINGSHANDLER->getServer2().c_str());
	strcpy_s(server3, sizeof(server3), SETTINGSHANDLER->getServer3().c_str());
	...
	if (!smWsockServer)
	{
		...
		if (SETTINGSHANDLER->getPort())
			dwServPort = SETTINGSHANDLER->getPort();
		else
			dwServPort = 8000;

		smWsockServer = SOCKETGAME->Connect(szServIP, (WORD)dwServPort);
	}

	if (smWsockServer && !smWsockDataServer)
	{
		strcpy_s(server1, sizeof(server1), SETTINGSHANDLER->getServer1().c_str());

		if (server1[0] && lstrcmpi(server1, server2) != 0)
		{
			strcpy_s(szDataServIP, server1);
			dwDataServPort = SETTINGSHANDLER->getPort();
			smWsockDataServer = SOCKETGAME->Connect(szDataServIP, (WORD)dwDataServPort);
		}
		else
		{
	...
	if (smWsockServer && smWsockDataServer && !smWsockUserServer)
	{
		...
		if (server3[0] && lstrcmpi(server3, server2) != 0)
		{
			...
			dwUserServPort = SETTINGSHANDLER->getPort();
			smWsockUserServer = SOCKETGAME->Connect(szUserServIP, (WORD)dwUserServPort);
		}
		else
		{
	...
	if (smWsockServer && smWsockDataServer && !smWsockExtendServer)
	{
		...
		if (server2[0] && lstrcmpi(server2, server2) != 0)
		{
			...
			dwExtendServPort = SETTINGSHANDLER->getPort();
			smWsockExtendServer = SOCKETGAME->Connect(szExtendServIP, (WORD)dwExtendServPort);
		}
		else
		{
...
SocketData* ConnectServer_InfoMain()
{
	if (!smWsockDataServer)
	{
		...
		if (server1[0])
		{
			dwDataServPort = SERVER_TCP_PORT;
			smWsockDataServer = SOCKETGAME->Connect(server1, (WORD)dwDataServPort);
...
SocketData* ConnectServer_GameMain(char* szIP1, DWORD dwPort1, char* szIP2, DWORD dwPort2)
{
	...
	SETTINGSHANDLER->setServer2(szIP1);
	SETTINGSHANDLER->setServer3(szIP2);

	if (smWsockServer && lstrcmp(smWsockServer->szIP, szIP1) != 0)
	{
		DisconnectServer_GameMain();
	}
	if (smWsockUserServer && lstrcmp(smWsockUserServer->szIP, szIP2) != 0)
	{
		DisconnectServer_GameMain();
	}
	...
	if (!smWsockServer)
	{
		...
		if (smWsockDataServer && lstrcmp(smWsockDataServer->szIP, szIP1) == 0 && dwDataServPort == dwPort1)
		{
		...
		{
			smWsockServer = SOCKETGAME->Connect(szIP1, (WORD)dwPort1);
			dwServPort = dwPort1;
		}
		SETTINGSHANDLER->setServer2(szIP1);
	}
	...
	{
		if (smWsockServer && lstrcmp(smWsockServer->szIP, szIP2) == 0 && dwServPort == dwPort2)
		{
		...
		{
			smWsockUserServer = SOCKETGAME->Connect(szIP2, (WORD)dwPort2);
...
SocketData* ConnectServer_GameMain(char* szIP1, DWORD dwPort1, char* szIP2, DWORD dwPort2, char* szIP3, DWORD dwPort3)
{
	...
	SETTINGSHANDLER->setServer2(szIP3);
	...
	if (smWsockExtendServer && lstrcmp(smWsockExtendServer->szIP, szIP3) != 0)
	{
		DisconnectServer_GameMain();
	}
	...
	{
		if ((smWsockServer && lstrcmp(smWsockServer->szIP, szIP3) == 0 && dwServPort == dwPort3) || !szIP3[0])
		{
			smWsockExtendServer = smWsockServer;
			dwExtendServPort = dwServPort;
		}
		...
		{
			smWsockExtendServer = SOCKETGAME->Connect(szIP3, (WORD)dwPort3);
...
public:
	...
	int RecvMessage(SocketData* pcSocketData, char* psPacket);
...
//Rubinho
std::vector<DWORD> QuestCodes =
{
	SIN_QUEST_CODE_CHANGEJOB,
	SIN_QUEST_CODE_CHANGEJOB2_NPC_M,
	SIN_QUEST_CODE_CHANGEJOB2_NPC_D,
	SIN_QUEST_CODE_CHANGEJOB3,
	SIN_QUEST_CODE_CHANGEJOB4,
	SIN_QUEST_CODE_LEVEL30,
	SIN_QUEST_CODE_LEVEL55,
	SIN_QUEST_CODE_LEVEL55_2,
	SIN_QUEST_CODE_LEVEL70,
	SIN_QUEST_CODE_LEVEL80,
	//SIN_QUEST_CODE_LEVEL85,
	SIN_QUEST_CODE_LEVEL90,
	//SIN_QUEST_CODE_LEVEL80_2,
	//SIN_QUEST_CODE_LEVEL90_2,
};
std::vector<BYTE> QuestBits =
{
	QUESTBIT_LEVEL_30,
	QUESTBIT_LEVEL_55,
	QUESTBIT_LEVEL_70,
...
//Rubinho
DWORD WINAPI InitCharStatus()
{
	Sleep(2000);
	ReformCharStatePoint(&lpCurPlayer->smCharInfo, sinQuest_levelLog);
	ReformCharForm();

	RECORD_SKILL nSkill = { 0 };
	RestoreSkill(&nSkill, sinQuest_levelLog);
...
int	CheckStartCharInfo(int lvl)
{
	...
	if (lvl > 0)
	{
		...
		lpCurPlayer->smCharInfo.Next_Exp = (int)ExpLevelTable[lvl + 1];
	}
	...
	for (BYTE pQuest : QuestBits)
		sinQuest_levelLog |= pQuest;

	for (DWORD pQuestCode : QuestCodes)
		EndQuest_Code(pQuestCode);
	...
	DWORD dwMsgId;
	CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)InitCharStatus, 0, 0, &dwMsgId);

	pcAntiCheat->AddNewThreadException(dwMsgId);
	return TRUE;
}

int rsTRANS_SERVER::RecvMessage(SocketData* pcSocketData, char* psPacket)
{
	...
	case smTRANSCODE_SHAMAN_INFO:
		lpShamanInfo = (SHAMAN_INFO*)psPacket;
	...
	case smTRANSCODE_ASSASSIN_INFO:
		lpAssassinInfo = (ASSASSIN_INFO*)psPacket;
	...
	case smTRANSCODE_FIGHTER_INFO:
		lpFighterInfo = (FIGHTER_INFO*)psPacket;
	...
	case smTRANSCODE_ARCHER_INFO:
		lpArcherInfo = (ARCHER_INFO*)psPacket;
	...
	case smTRANSCODE_SACER_INFO:
		lpSacerInfo = (SACER_INFO*)psPacket;
	...
	{
		pcAntiCheat->ReceivePacketHandler((PacketChecksumFunctionList*)psPacket);
	}
	break;
	case OpCode::INIT_CHEAT_PROTECTION:
	{
		pcAntiCheat->HandlePacket((PacketWindowList*)psPacket);
	}
	break;
	...
	case OpCode::SET_ITEM_PREMIUM:
	{
		...
		smPacket = (smTRANS_COMMAND*)psPacket;
	...
	case OpCode::OPCODE_CHATMESSAGE:
		...
		strcpy_s(ChatBuff[ChatCnt].szMessage, &psPacket[3 * 4]);
		ChatBuffCnt++;
		ChatDispCnt = 70 * 10;
		if (ChatScrollPoint)
		{
		...
		/*case OpCode::CHANGE_WINDOW_TITLE:
		{
			ChangeTitle *packet = (ChangeTitle *)psPacket;

			SetWindowText(hwnd, packet->buffer);
	...
	{
		smTRANS_COMMAND* TransCommand = (smTRANS_COMMAND*)psPacket;
	...
	case OpCode::OPCODE_PLAYDATAGROUP:
		...
		lpTransBuff = psPacket + sizeof(smPLAYDATA_GROUP);
		...
		if (smWsockUserServer == pcSocketData)
		{
			ServerCode = PLAY_SERVER_CODE_USER;
		}
		else
		{
			if (smWsockExtendServer == pcSocketData)
		...
		{
			if (lpWSockServer_DispArea[0] == pcSocketData)
			{
				ServerCode = PLAY_SERVER_CODE_AREA1;
			}
			else  if (lpWSockServer_DispArea[1] == pcSocketData)
			{
				...
				if (RcvCnt < 8 || (RcvCnt < 16 && ((int*)lpTransBuff)[1] == OpCode::OPCODE_TRANSPLAYDATA))
				{
					...
					if (lpChar)
					{
						...
						else
						{
							...
							if (pcSocketData->s)
							{
								pcSocketData->Send((char*)&TransCommand, TransCommand.size, TRUE);
							}
						}
					}
				}
		...
		case  OpCode::MSG_PING:
		{
			PacketPing* pPing = (PacketPing*)psPacket;
	...
	case OpCode::OPCODE_ATTACKDATA2:
		...
		if (fnDecodeDamagePacket)
			fnDecodeDamagePacket(0, psPacket);

		if (((TRANS_ATTACKDATA*)psPacket)->dwDamageChkSum != dm_GetDamgeChkSum_S2V((TRANS_ATTACKDATA*)psPacket))
		{
			SendSetHackUser3(8580, ((TRANS_ATTACKDATA*)psPacket)->Power, (DWORD)fnDecodeDamagePacket);
			break;
		}
#else
		dm_DecodePacket(0, psPacket);

		if (((TRANS_ATTACKDATA*)psPacket)->dwDamageChkSum != dm_GetDamgeChkSum_S2V((TRANS_ATTACKDATA*)psPacket))
		{
			SendSetHackUser3(8580, ((TRANS_ATTACKDATA*)psPacket)->Power, (DWORD)dm_DecodePacket);
			break;
		}
#endif
	...
	case OpCode::OPCODE_TRANSPLAYDATA:
		...
		{
			if (smWsockExtendServer != smWsockServer && smWsockExtendServer == pcSocketData)
				ServerCode = PLAY_SERVER_CODE_EXTEND;
			else
				ServerCode = PLAY_SERVER_CODE_MAIN;
		}
		...
		lpPlayData1 = (smPLAYDATA*)psPacket;
		lpChar = FindAutoPlayer(lpPlayData1->dwObjectSerial);
		if (lpChar)
		{
			lpChar->RecvPlayData(pcSocketData, psPacket);
	...
	case OpCode::OPCODE_STARTUSER:
	{
		chaPremiumitem.UsePremiumItem(170);
		AddChatBuff("Você Recebeu: Kit Inicial");
	}
	break;
	...
	case OpCode::OPCODE_ROLL:
		ROLLHANDLER->HandlePacket((PacketRollInfo*)psPacket);
		break;
	case OpCode::OPCODE_ENDROLL:
		ROLLHANDLER->HandlePacket((smTRANS_COMMAND*)psPacket);
		break;
	...
	case OpCode::SEND_ALERT:
		lpTransChatMessage = (TRANS_CHATMESSAGE*)psPacket;
		cMessageBox.ShowMessage1(lpTransChatMessage->szMessage);
		break;
	...
	case OpCode::OPCODE_WHISPERMESSAGE:
		...
		if (lpTransChatMessage->dwIP == 1 && SETTINGSHANDLER->getWhisper())
			break;
	...
	{
		smTRANS_COMMAND* smTransCommand = (smTRANS_COMMAND*)psPacket;
	...
	case OpCode::HIGH_LEVEL_USER:
		lpTransCommand = (smTRANS_COMMAND*)psPacket;
		if (lpTransCommand->LParam == lpCurPlayer->smCharInfo.dwObjectSerial && lpTransCommand->WParam == lpCurPlayer->smCharInfo.JOB_CODE)
		{
			if (!lpCurPlayer->dwTopRankUserEffectTime)
			{
				sSKILL highLevelSkill;

				for (int j = 0; j < SIN_MAX_SKILL; j++)
				{
					if (sSkill[j].CODE == HIGH_LEVEL_MERIT_SKILL)
					{
						memcpy(&highLevelSkill, &sSkill[j], sizeof(sSKILL));
						highLevelSkill.UseTime = 604800;
						sinContinueSkillSet(&highLevelSkill);
						break;
					}
				}

				g_bTopRanker = true;
				lpCurPlayer->dwTopRankUserEffectTime = dwPlayTime + 604800;
				AssaParticle_KeepSkillEffect("HighLevelUserMerit_Star", lpCurPlayer, 604800, HIGH_LEVEL_MERIT_SKILL);
			}
		}
		else if (lpCurPlayer->dwTopRankUserEffectTime && lpTransCommand->WParam == lpCurPlayer->smCharInfo.JOB_CODE)
		{
			cSkill.CancelContinueSkill(HIGH_LEVEL_MERIT_SKILL);
			g_bTopRanker = false;
			lpCurPlayer->dwTopRankUserEffectTime = 0;
			StopAssaCodeEffect(lpCurPlayer, HIGH_LEVEL_MERIT_SKILL);
	...
	case OpCode::START_CARAVAN:
		...
		chaPremiumitem.m_CaravanTime = lpTransCommand->WParam;

		chaPremiumitem.SetUpKeepItem(nsPremiumItem::CARAVAN, lpTransCommand->WParam, true, "Caravan");
		break;
	...
	case OpCode::OPCODE_CONNECTED:
		...
		if (pcSocketData == smWsockServer)
		{
			...
			SendNetworkQuality(SETTINGSHANDLER->getNetworkQuality());
			...
			TransCommand.size = sizeof(smTRANS_COMMAND);
			TransCommand.code = OpCode::HIGH_LEVEL_USER;
	...
	case OpCode::OPCODE_FAILRECORDDATA:
		...
		{//LOG_GAME
		//	Logger::getInstance()->output(LogType::Game, "SetDisconnectFlag: 11");
		...
		if (!AdminCharDisable)
		{
			CheckStartCharInfo(lpTransCharCommand->SParam); //Rubinho
			rsRecorder.dwNextRecordTime = dwPlayTime + RECORD_TIME_STEP;
			dwLoadingTime = 0;
			lpCurPlayer->SendCharInfo(smWsockServer);
	...
	case OpCode::OPCODE_ADDEXP_PARTY:
		...
		if (lpChar && !lpChar->RecvExp)
		{
			if (lpTransCommand->SParam == ((lpTransCommand->WParam * 3 + lpTransCommand->LParam * 13) * 2002))
			{
				...
				//CheckServerExp(pcSocketData, lpTransCommand);
	...
	case OpCode::OPCODE_SERVER_INFO:
		...
		lpTransServerList = (TRANS_SERVER_LIST*)psPacket;
		...
		HoRecvMessage(IPData[1], psPacket);
		break;
	...
	case OpCode::OPCODE_SHOP_ITEMLIST:
		...
		sinRecvMessage(IPData[1], psPacket);

		Send_GetBlessCastleTax();

		break;

	//RUBINHO ABRIR MESTRE DAS HABILIDADES NESSA FUNÇÂO
	case OpCode::OPCODE_SKILL_MENU:
		...
		if (lpCurPlayer->OnStageField >= 0)
		{
			...
			if (StageField[lpCurPlayer->OnStageField]->FieldCode == 21)
			{
				...
				if (lpCurPlayer->smCharInfo.JOB_CODE > 4 && lpCurPlayer->smCharInfo.JOB_CODE <= 8 || lpCurPlayer->smCharInfo.JOB_CODE == 10)
				{
					sinSkillMaster(lpTransCommand->WParam);
				}
			}
	...
	case OpCode::OPCODE_VERSION:
		...
		if (pcSocketData == smWsockDataServer || pcSocketData == smWsockServer)
		{
	...
	case OpCode::OPCODE_GET_RESTAURE:
		cRestaure.Open();
	case OpCode::OPCODE_RESTAURE_LIST:
		cRestaure.RecvRestaureList((RESTAURE_LIST*)psPacket);
		break;
	...
	case OpCode::OPCODE_DEF_INFO:
		lpTransCommand = (smTRANS_COMMAND*)psPacket;

		if (lpTransCommand->WParam == lpCurPlayer->dwObjectSerial)
			lpChar = lpCurPlayer;
		else
			lpChar = FindAutoPlayer(lpTransCommand->WParam);

		if (lpChar)
		{
			ParkAssaParticleDamage(lpChar, lpTransCommand->LParam);
		}
		break;
		...
		cTrade.CancelTradeItem();
		break;
	case OpCode::OPCODE_SHOW_DMG:
		lpTransCommand = (smTRANS_COMMAND*)psPacket;

		if (lpTransCommand->LParam)
		{
			cSHOW_DMG::getInstance()->AddDmg(lpTransCommand->LParam, lpTransCommand->WParam, lpTransCommand->SParam);
		}
		break;

	case OpCode::OPCODE_TRADE_REQUST:
		if (SETTINGSHANDLER->getTradeRequest())
			break;
	...
	case OpCode::OPCODE_FINDCRACK:
		...
		CheckCracker((TRANS_FIND_CRACKER*)psPacket);
		break;


	case OpCode::OPCODE_PARTY_REQUEST:
		if (SETTINGSHANDLER->getPartyRequest())
			break;
	...
	case OpCode::OPCODE_PARTY_UPDATE:
		...
		lpTransPartyUsers = (TRANS_PARTY_USERS*)psPacket;
...

	{
		...
		lpTransPartyPlayInfo = (TRANS_PARTY_PLAYINFO*)psPacket;

		if (lpTransPartyPlayInfo->PartyUserCount > 6) break;
	...
	case OpCode::OPCODE_WAREHOUSE:
		...
		{
			if (LoadWareHouse((TRANS_WAREHOUSE*)psPacket, &sWareHouse) == TRUE)
			{
	...
	{
		PACKET_VIP* packet = (PACKET_VIP*)psPacket;
		if (packet->param)
		{
	...
	{
		PACKET_CROWN* packet = (PACKET_CROWN*)psPacket;
	...
	{
		PACKET_BUFFS* packet = (PACKET_BUFFS*)psPacket;
	...
	case OpCode::OPCODE_OPEN_BATTLE_EVENT:
		BattleEventWindow->Open();
		break;
	case OpCode::OPCODE_HG_NEXT_STAGE:
		CHellsGateHandler->RecvNextStage((smTRANS_HGINFO*)psPacket);
		break;
	case OpCode::OPCODE_HG_ENTER:
		lpTransCommand = (smTRANS_COMMAND*)psPacket;
		CloseEachPlayer();
	...
	case OpCode::OPCODE_CLOSECLIENT:
		...
		//Logger::getInstance()->output(LogType::Game, "SetDisconnectFlag: 13");
		DisconnectFlag = GetCurrentTime();
		break;
	...
	case OpCode::OPCODE_SEND_DATASERVER:
		...
		if (smWsockDataServer)
		{
			smWsockDataServer->Send(((TRANS_SEND_DATASERVER*)psPacket)->szBuff, ((smTRANS_COMMAND*)(((TRANS_SEND_DATASERVER*)psPacket)->szBuff))->size, TRUE);
		}
		break;
	...
	case OpCode::OPCODE_SEND_GAMESERVER:
		...
		if (smWsockServer)
		{
			smWsockServer->Send(((TRANS_SEND_DATASERVER*)psPacket)->szBuff, ((smTRANS_COMMAND*)(((TRANS_SEND_DATASERVER*)psPacket)->szBuff))->size, TRUE);
		}
		if (smWsockUserServer && smWsockUserServer != smWsockServer)
		{
			smWsockUserServer->Send(((TRANS_SEND_DATASERVER*)psPacket)->szBuff, ((smTRANS_COMMAND*)(((TRANS_SEND_DATASERVER*)psPacket)->szBuff))->size, TRUE);
		}
		if (smWsockExtendServer && smWsockExtendServer != smWsockServer)
		{
			smWsockExtendServer->Send(((TRANS_SEND_DATASERVER*)psPacket)->szBuff, ((smTRANS_COMMAND*)(((TRANS_SEND_DATASERVER*)psPacket)->szBuff))->size, TRUE);
		}
		break;
	...
	case OpCode::OPCODE_SEND_AREASERVER:
		...
		if (lpsmSock)
			lpsmSock->Send(((TRANS_SEND_DATASERVER*)psPacket)->szBuff, ((smTRANS_COMMAND*)(((TRANS_SEND_DATASERVER*)psPacket)->szBuff))->size, TRUE);
		else
			smWsockServer->Send(((TRANS_SEND_DATASERVER*)psPacket)->szBuff, ((smTRANS_COMMAND*)(((TRANS_SEND_DATASERVER*)psPacket)->szBuff))->size, TRUE);
	...
	case OpCode::OPCODE_SEND_AGAME_SERVER:
		...
		for (int i = 0; i < SINUPKEEPITEM_MAX; i++)
		{
			...
			else if (chaPremiumitem.UpKeepItem[i].TGAImageNumber == nsPremiumItem::AWELLPOWER)
			{
				if (chaPremiumitem.UpKeepItem[i].IconTime[1] - chaPremiumitem.UpKeepItem[i].IconTime[0] <= 0)
				{
			...
			else if (chaPremiumitem.UpKeepItem[i].TGAImageNumber == nsPremiumItem::MIGALPOWER)
			{
				if (chaPremiumitem.UpKeepItem[i].IconTime[1] - chaPremiumitem.UpKeepItem[i].IconTime[0] <= 0)
				{
			...
			else if (chaPremiumitem.UpKeepItem[i].TGAImageNumber == nsPremiumItem::HPBOOSTER)
			{
				if (chaPremiumitem.UpKeepItem[i].IconTime[1] - chaPremiumitem.UpKeepItem[i].IconTime[0] <= 0)
				{
			...
			else if (chaPremiumitem.UpKeepItem[i].TGAImageNumber == nsPremiumItem::MPBOOSTER)
			{
				if (chaPremiumitem.UpKeepItem[i].IconTime[1] - chaPremiumitem.UpKeepItem[i].IconTime[0] <= 0)
				{
			...
			else if (chaPremiumitem.UpKeepItem[i].TGAImageNumber == nsPremiumItem::SPBOOSTER)
			{
				if (chaPremiumitem.UpKeepItem[i].IconTime[1] - chaPremiumitem.UpKeepItem[i].IconTime[0] <= 0)
				{
			...
			else if (chaPremiumitem.UpKeepItem[i].TGAImageNumber == nsPremiumItem::RECOVERSP)
			{
				if (chaPremiumitem.UpKeepItem[i].IconTime[1] - chaPremiumitem.UpKeepItem[i].IconTime[0] <= 0)
				{
			...
			else if (chaPremiumitem.UpKeepItem[i].TGAImageNumber == nsPremiumItem::BDIVINA)
			{
				if (chaPremiumitem.UpKeepItem[i].IconTime[1] - chaPremiumitem.UpKeepItem[i].IconTime[0] <= 0)
				{
			...
			else if (chaPremiumitem.UpKeepItem[i].TGAImageNumber == nsPremiumItem::FORCE)
			{
				if (chaPremiumitem.UpKeepItem[i].IconTime[1] - chaPremiumitem.UpKeepItem[i].IconTime[0] <= 0)
				{
	...
	case OpCode::OPCODE_FUNCTION_MEM2:
		...
		RecvMemFuncData2((TRANS_FUNC_MEMORY*)psPacket);
		break;
	...
	case OpCode::OPCODE_CRAFTITEM:
		...
		if (((sCRAFTITEM_SERVER*)psPacket)->Result == TRUE)
		{
			...
			cInvenTory.DeleteInvenItemToServer(((sCRAFTITEM_SERVER*)psPacket)->DesCraftItem.sItemInfo.CODE,
				((sCRAFTITEM_SERVER*)psPacket)->DesCraftItem.sItemInfo.BackUpKey,
				((sCRAFTITEM_SERVER*)psPacket)->DesCraftItem.sItemInfo.BackUpChkSum);



			if (FindInvenItem(((sCRAFTITEM_SERVER*)psPacket)->DesCraftItem.sItemInfo.CODE,
				((sCRAFTITEM_SERVER*)psPacket)->DesCraftItem.sItemInfo.BackUpKey,
				((sCRAFTITEM_SERVER*)psPacket)->DesCraftItem.sItemInfo.BackUpChkSum) != 0)
			{

				SendInvenItemError(0, ((sCRAFTITEM_SERVER*)psPacket)->DesCraftItem.sItemInfo.CODE,
					((sCRAFTITEM_SERVER*)psPacket)->DesCraftItem.sItemInfo.BackUpKey,
					((sCRAFTITEM_SERVER*)psPacket)->DesCraftItem.sItemInfo.BackUpChkSum);
				break;
			}
	...
	case OpCode::OPCODE_AGING_UPGRADE:
		...
		sinRecvAgingIncreStateItem(&((TRANS_ITEMINFO*)psPacket)->Item);
		...
		if ((((TRANS_ITEMINFO*)psPacket)->Item).CODE == cInvenTory.InvenItem[sInven[0].ItemIndex - 1].CODE &&
			cInvenTory.InvenItem[sInven[0].ItemIndex - 1].sItemInfo.ItemName[0])
		{
			...
			if ((((TRANS_ITEMINFO*)psPacket)->Item).CODE == cInvenTory.InvenItem[sInven[1].ItemIndex - 1].CODE &&
	...
	case OpCode::OPCODE_MAKE_LINKCORE:
		...
		cInvenTory.RecvLinkCore(&((TRANS_ITEMINFO*)psPacket)->Item);
		ResetInvenItemCode();
		SaveGameData();
		break;
	...
	case OpCode::OPCODE_SHOPTITEM:
		...
		if ((((TRANS_BUY_SHOPITEM*)psPacket)->sItem.sItemInfo.CODE & sinITEM_MASK1) == (sinPM1 & sinITEM_MASK1))
		{
			ComparePotion();
		}
		...
		ResetInvenItemCode();
		break;
	case OpCode::OPCODE_GET_OPENCOINSHOP:
		cCoinShop.Open();
		break;

	case OpCode::OPCODE_GET_OPENRESTAURE:
		cRestaure.Open();
		break;

	case OpCode::OPCODE_GET_SHOPITEM:
		cCoinShop.RecvItemFromServer((smTRANS_SHOPDATA*)psPacket);
		break;

	case OpCode::OPCODE_GET_SHOPCOIN:
		cCoinShop.RecvCoinFromServer((smTRANS_COMMAND*)psPacket);
		break;

	case OpCode::OPCODE_GET_SHOPITEM_T:
		cCoinShop_T.RecvItemFromServer((smTRANS_SHOPDATA*)psPacket);
		break;

	case OpCode::OPCODE_GET_SHOPCOIN_T:
		cCoinShop_T.RecvCoinFromServer((smTRANS_COMMAND*)psPacket);
		break;
	...
	case OpCode::OPCODE_SHOWITEMINFO_PERF:
		RecvItemInfoPerf((TRANS_ITEMINFO*)psPacket);
		break;
	...
	case OpCode::OPCODE_FUNCTION_MEM:
		...
		RecvMemFuncData((TRANS_FUNC_MEMORY*)psPacket);
		break;
	...
	{
		BATTLE_ARENA_POINT* packet = (BATTLE_ARENA_POINT*)psPacket;
	...
	break;
	case OpCode::OPCODE_CANCEL_FORCE_ORB:
		lpTransCommand = (smTRANS_COMMAND*)psPacket;
		switch (lpTransCommand->LParam)
		{
		case 0:
	...
	case OpCode::OPCODE_OPEN_SKINCHANGE:
		cSkinChanger.Open();
		break;

	case OpCode::OPCODE_OPEN_SORTEIO_ITENS:
		Sorteio->Open();
		break;

	case OpCode::OPCODE_RECVKEYS_SORTEIO_ITENS:
		Sorteio->RecvKeysFromServer((smTRANS_COMMAND*)psPacket);
		break;
	...
	case OpCode::OPCODE_AREA_SERVER_MAP:
		...
		RecvAreaServerMap((TRANS_SEVER_MAP*)psPacket);
		break;
	...
	case OpCode::OPCODE_ITEM_EXPRESS:
		...
		{
			ComparePotion();
			cInvenTory.SetInvenToItemInfo((sITEMINFO*)(psPacket + sizeof(TRANS_POST_ITEM)));
			ResetInvenItemCode();

			if ((((sITEMINFO*)(psPacket + sizeof(TRANS_POST_ITEM)))->CODE & sinITEM_MASK1) == (sinPM1 & sinITEM_MASK1))
			{
	...
	case OpCode::OPCODE_SOD_INFOMATION:
		...
		break;
	case OpCode::OPCODE_RANKING:
	{
		lpTransRank = (smRANKING*)psPacket;
		for (int i = 0; i < 10; i++) {
			wsprintf(NameRank[i], "%s", lpTransRank->rankName[i]);
			LevelRank[i] = lpTransRank->rankLevel[i];
			ClasseRank[i] = lpTransRank->rankClasse[i];
		}
		Type = lpTransRank->type;
		contador = lpTransRank->contador;
		RankGame::GetInstance()->Open();
		if (lpTransRank->type == 1) {
			ZeroMemory(NameRank, 0);
			ZeroMemory(LevelRank, 0);
			ZeroMemory(ClasseRank, 0);
			for (int i = 0; i < 10; i++) {
				wsprintf(NameRank[i], "%s", lpTransRank->rankName[i]);
				LevelRank[i] = lpTransRank->rankLevel[i];
				ClasseRank[i] = lpTransRank->rankClasse[i];
			...
			for (int i = 0; i < 10; i++) {
				...
				LevelRank[i] = lpTransRank->rankLevel[i];
				ClasseRank[i] = lpTransRank->rankClasse[i];
				KillsRank[i] = lpTransRank->rankKills[i];
				DeadsRank[i] = lpTransRank->rankDeads[i];
			}
			contador = lpTransRank->contador;
		}
		else if (lpTransRank->type == 3) {
			ZeroMemory(NameRank, 0);
			ZeroMemory(ClasseRank, 0);
			ZeroMemory(pointsRank, 0);
			for (int i = 0; i < 10; i++) {
				wsprintf(NameRank[i], "%s", lpTransRank->rankName[i]);
				ClasseRank[i] = lpTransRank->rankClasse[i];
				pointsRank[i] = lpTransRank->rankPointsSod[i];
			}
			contador = lpTransRank->contador;
		}
		else if (lpTransRank->type == 4) {
			ZeroMemory(liderClanRank, 0);
			ZeroMemory(NameClanRank, 0);
			ZeroMemory(pointsClanRank, 0);
			for (int i = 0; i < 10; i++) {
				wsprintf(NameClanRank[i], "%s", lpTransRank->rankNameClan[i]);
				wsprintf(liderClanRank[i], "%s", lpTransRank->rankLiderClan[i]);
				pointsClanRank[i] = lpTransRank->rankPointsSod[i];
			}
			contador = lpTransRank->contador;
		}
		else if (lpTransRank->type == 5) {
			ZeroMemory(NameClanRank, 0);
			ZeroMemory(liderClanRank, 0);
			ZeroMemory(membrosclanRank, 0);
			for (int i = 0; i < 10; i++) {
				wsprintf(NameClanRank[i], "%s", lpTransRank->rankNameClan[i]);
				wsprintf(liderClanRank[i], "%s", lpTransRank->rankLiderClan[i]);
				membrosclanRank[i] = lpTransRank->rankmembros[i];
			}
			contador = lpTransRank->contador;
		}
	}
	...
	case OpCode::OPCODE_XTRAP_PACKET:
		XTrap_Recv((smTRANS_COMMAND*)psPacket, smWsockDataServer);
		break;
#endif
	...
	case OpCode::OPCODE_UPDATE_PREMIUMITEM_TIME:
		...
		lpTransPremiumTime = (TRANS_PREMIUM_TIME*)psPacket;
		...
		chaPremiumitem.m_CaravanTime = lpTransPremiumTime->Caravan;
		chaPremiumitem.m_BigHeadTime = lpTransPremiumTime->BigHead;
		chaPremiumitem.m_AwellPowerTime = lpTransPremiumTime->AwellPower;
		chaPremiumitem.m_MigalPoweTime = lpTransPremiumTime->MigalPower;
		chaPremiumitem.m_HPBoosterTime = lpTransPremiumTime->HPBooster;
		chaPremiumitem.m_MPBoosterTime = lpTransPremiumTime->MPBooster;
		chaPremiumitem.m_SPBoosterTime = lpTransPremiumTime->SPBooster;
		chaPremiumitem.m_RecoverSPTime = lpTransPremiumTime->SPRecover;
		chaPremiumitem.m_BDivinaTime = lpTransPremiumTime->BDivina;
		chaPremiumitem.m_ForceTime = lpTransPremiumTime->Force;
		...
		if (chaPremiumitem.m_ThirdEyesTime > PRIME_ITEM_TIME_7D || chaPremiumitem.m_ExpUpPotionTime > PRIME_ITEM_TIME_7D ||
			...
			chaPremiumitem.m_HelpPetTime > PRIME_ITEM_TIME_7D || chaPremiumitem.m_StaminaReducePotiontime > PRIME_ITEM_TIME_7D
			|| chaPremiumitem.m_VampiricCuspidEXTime > PRIME_ITEM_TIME_7D || chaPremiumitem.m_CaravanTime > PRIME_ITEM_TIME_7D ||
			chaPremiumitem.m_BigHeadTime > PRIME_ITEM_TIME_7D || chaPremiumitem.m_AwellPowerTime > PRIME_ITEM_TIME_7D || chaPremiumitem.m_MigalPoweTime > PRIME_ITEM_TIME_7D ||
			chaPremiumitem.m_HPBoosterTime > PRIME_ITEM_TIME_7D || chaPremiumitem.m_MPBoosterTime > PRIME_ITEM_TIME_7D || chaPremiumitem.m_SPBoosterTime > PRIME_ITEM_TIME_7D ||
			chaPremiumitem.m_RecoverSPTime > PRIME_ITEM_TIME_7D || chaPremiumitem.m_BDivinaTime > PRIME_ITEM_TIME_7D || chaPremiumitem.m_ForceTime > PRIME_ITEM_TIME_7D ||
			chaPremiumitem.m_CaravanTime > PRIME_ITEM_TIME_7D)
		{
			break;
		}
		...
		for (int i = 0; i < SINUPKEEPITEM_MAX; i++)
		{
			...
			case nsPremiumItem::CARAVAN:
				chaPremiumitem.UpKeepItem[i].IconTime[1] = chaPremiumitem.m_CaravanTime;
				break;

			case nsPremiumItem::BIG_HEAD:
				chaPremiumitem.UpKeepItem[i].IconTime[1] = chaPremiumitem.m_BigHeadTime;
				break;

			case nsPremiumItem::AWELLPOWER:
				chaPremiumitem.UpKeepItem[i].IconTime[1] = chaPremiumitem.m_AwellPowerTime;
				break;

			case nsPremiumItem::MIGALPOWER:
				chaPremiumitem.UpKeepItem[i].IconTime[1] = chaPremiumitem.m_MigalPoweTime;
				break;

			case nsPremiumItem::HPBOOSTER:
				chaPremiumitem.UpKeepItem[i].IconTime[1] = chaPremiumitem.m_HPBoosterTime;
				break;

			case nsPremiumItem::MPBOOSTER:
				chaPremiumitem.UpKeepItem[i].IconTime[1] = chaPremiumitem.m_MPBoosterTime;
				break;

			case nsPremiumItem::SPBOOSTER:
				chaPremiumitem.UpKeepItem[i].IconTime[1] = chaPremiumitem.m_SPBoosterTime;
				break;

			case nsPremiumItem::RECOVERSP:
				chaPremiumitem.UpKeepItem[i].IconTime[1] = chaPremiumitem.m_RecoverSPTime;
				break;
			case nsPremiumItem::BDIVINA:
				chaPremiumitem.UpKeepItem[i].IconTime[1] = chaPremiumitem.m_BDivinaTime;
				break;
			case nsPremiumItem::FORCE:
				chaPremiumitem.UpKeepItem[i].IconTime[1] = chaPremiumitem.m_ForceTime;
				break;
			}
	...
	case OpCode::OPCODE_XIGNCODE_PACKET:
		XIGNCODE_PACKET* pack = (XIGNCODE_PACKET*)psPacket;
	...
	else
	{
		...
		if (szMessage[0] == '/')
		{
			...
			if (StageField[lpCurPlayer->OnStageField]->FieldCode != 50)
			{
				...
				if (len)
				{
					AddChatBuff("> /killme -> para se matar", 5);
					AddChatBuff("> /city -> Teletransportar para Ricartem", 5);
					AddChatBuff("> /cancelar_force -> Cancelar a força Orb", 5);
					return TRUE;
				}*/

			}

			len = rsCompString("/defenseinfo", szMessage);
			if (len)
			{
				defenseinfo = !defenseinfo;

				if (defenseinfo)
					AddChatBuff("Defense info debug ON!");
				else
					AddChatBuff("Defense info debug OFF!");
				return TRUE;
			}
...
int DisplayChatMessage(HDC hdc, int x, int y, int MaxLine)
{
	...
	for (cnt = start; cnt < end; cnt++)
	{
		if (cnt >= 0)
		{
			...
			dsTextLineOut(0, x + 1, y + 1, lpString, lstrlen(lpString));
			...
			dsTextLineOut(0, x, y, lpString, lstrlen(lpString));
		}
		y += 16;
	}
...
int SetIpColor(DWORD dwIP)
{
	...
	case 10:	SetFontTextColor(RGB(255, 180, 0));		break;
	...
	case 15:	SetFontTextColor(RGB(255, 0, 255));		break;
	case 16:	SetFontTextColor(RGB(0, 255, 0));		break;
...
int DisplayChatMessage2(int x, int y, int StrMax, int LineMax)
{
	...
	for (int cnt = start; cnt >= end; cnt--) {
		if (cnt >= 0) {
			...
			lstrcpy(szBuff, ChatBuff2[fl_Count][chatCnt].szMessage);
			if (!szBuff[0]) continue;
			lpString = szBuff;

			SetIpColor(ChatBuff2[fl_Count][chatCnt].dwIP);

			if (cEmoji.getTextSize(lpString) > StrMax) {
				int cutPos = cEmoji.getCutPos(lpString, StrMax);
				dsTextLineOut(0, x, y, &lpString[cutPos], lstrlen(&lpString[cutPos]));
				lpString[cutPos] = 0;
			...
			lpString = strstr(szBuff, ":");
			if (lpString) {
				dsTextLineOut(0, x, y, szBuff, lstrlen(szBuff));
				lpString[1] = 0;
...
int DisconnectPlayer(SocketData * lpsmSock)
{
	...
	if (smWsockServer && lpsmSock->s == smWsockServer->s)
	{
		...
		smWsockServer = 0;
		//Utils_Log(LOG_DEBUG_, "SetDisconnectFlag: 14");
	...
	if (smWsockDataServer && lpsmSock->s == smWsockDataServer->s)
	{
	...
	if (smWsockUserServer && lpsmSock->s == smWsockUserServer->s)
	{
		AddChatBuff("Connection lost to Server");
		smWsockUserServer = 0;
	...
	if (smWsockExtendServer && lpsmSock->s == smWsockExtendServer->s)
	{
		AddChatBuff("Connection lost to Server");
		smWsockExtendServer = 0;
	...
	if (AreaServerMode)
	{
		if (lpWSockServer_Area[0] && lpWSockServer_Area[0]->s == lpsmSock->s)
		{
			if (smConfig.getSecurity() >= AccountTypes::SEC_MODERATOR)
			{
				wsprintf(szBuff, "Disconnect Area Server (%s)", lpWSockServer_Area[0]->szIP);
		...
		if (lpWSockServer_Area[1] && lpWSockServer_Area[1]->s == lpsmSock->s)
		{
			if (smConfig.getSecurity() >= AccountTypes::SEC_MODERATOR)
			{
				wsprintf(szBuff, "Disconnect Area Server (%s)", lpWSockServer_Area[1]->szIP);
				AddChatBuff(szBuff, 0);
			}
...
void RecvMessage(SocketData * pcSocketData, char* psPacket)
{
...
int NetWorkPlay()
{
	...
	bool connected = InternetGetConnectedState(&state, 0);
	if (!connected)
	{
		DisconnectFlag = 1;
	}
	...
	if (NetWorkInitFlag == 0)
	{
		...
		switch (SETTINGSHANDLER->getNetworkQuality())
		{
		case 0:
			NetScoopTime = 3000;
			break;
	...
	else
	{
		...
		/*if (dwLastPing + 150 < GetCurrentTime())
		{
			smTRANS_COMMAND sPacket;
			sPacket.code = OpCode::MSG_PING;
			sPacket.size = sizeof(smTRANS_COMMAND);
			sPacket.WParam = GetCurrentTime();
			if (smWsockServer)
				smWsockServer->Send((char*)&sPacket, sPacket.size, true);

			dwLastPing = GetCurrentTime();
		}*/
		...
		if (dwLoadingTime)
		{
			...
			{
				//LOG_GAME
				//Logger::getInstance()->output(LogType::Game, "SetDisconnectFlag: 19");
				DisconnectFlag = GetCurrentTime();
				dwLoadingTime = 0;
			}
		}
		...
		TransThrowItem.Item.ItemHeader.dwChkSum == (DWORD)lpTransCommand->SParam)
	{
		...
		if (!TransThrowItem.Item.SpecialItemFlag[0])
		{
				...
				(dwItemCodeMask == sinBI4 && TransThrowItem.Item.PotionCount == 300))
			{
				...
				if (dwItemCodeMask == sinBI1 || dwItemCodeMask == sinBI2 || dwItemCodeMask == sinBI3 || dwItemCodeMask == sinBI4)
				{
					...
					{
						chaPremiumitem.SetVampiricCuspidTime(PRIME_ITEM_TIME_12H);
					}
					...
					{
						chaPremiumitem.SetManaReducePotionValue(PRIME_ITEM_TIME_12H);
					}
					...
					else if (TransThrowItem.Item.CODE == (sinBI1 | sin26))
					{
						...
						chaPremiumitem.SetMightOfAwellWeight(1000);
					}
					...
					else if (TransThrowItem.Item.CODE == (sinBI3 | sin20))
					{
						chaPremiumitem.SetCaravanTime((60 * 60 * 24 * 3));
					}
					else if (TransThrowItem.Item.CODE == (sinBI3 | sin21))
					{
						chaPremiumitem.SetCaravanTime((60 * 60 * 24 * 7));
					}
...
int ModoPVP()
{
	...
	if (!PLayerPvP)
	{
		if (StageField[lpCurPlayer->OnStageField]->State != FIELD_STATE_CASTLE)
		{
			...
			{
				AddChatBuff("> Espere 30Seg para ativar Novamente!", 14);
			}
			else
			{
				ModoPvPTimer = GetTickCount();
				AddChatBuff("> Modo PvP Ativado", 14);
				PLayerPvP = TRUE;
	...
	{
		if (GetTickCount() - ModoPvPTimer < 60000)

		{
			//if (StageField[lpCurPlayer->OnStageField]->State != FIELD_STATE_CASTLE)
			//{
			AddChatBuff("> Aguarde 60s para Desativar", 14);
		}
		else
		{
			//DESATIVAR WarMode
			ModoPvPTimer = GetTickCount();
			AddChatBuff("> Modo PvP Desativado", 14);
			PLayerPvP = FALSE;
...
int	TransCheckNetwork()
{
		...
		!smWsockServer)
	{
		...
		SOCKETGAME->SocketClose(smWsockServer);
		smWsockServer = 0;
		ConnectServer_Main();
		...
		Logger::getInstance()->output(LogType::Game, "SetDisconnectFlag: 20");
		DisconnectFlag = GetCurrentTime();
		return FALSE;
	}
	...
	if (smWsockDataServer && smWsockServer != smWsockDataServer)
	{
		...
		{//LOG_GAME
			//Logger::getInstance()->output(LogType::Game, "SetDisconnectFlag: 21");
			DisconnectFlag = GetCurrentTime();
	...
	if (smWsockUserServer && smWsockServer != smWsockUserServer)
	{
			...
			(dwUserServ_RecvTime && (dwUserServ_RecvTime + RECONNECT_LIMIT_TIME) < dwTime2))
		{
			...
			//Logger::getInstance()->output(LogType::Game, "SetDisconnectFlag: 22");
			DisconnectFlag = GetCurrentTime();
			return FALSE;
		}
	...
	if (smWsockExtendServer && smWsockServer != smWsockExtendServer)
	{
			...
			(dwExtendServ_RecvTime && (dwExtendServ_RecvTime + RECONNECT_LIMIT_TIME) < dwTime2))
		{
			...
			//LOG_GAME
			DisconnectServerCode = 3;
			//Logger::getInstance()->output(LogType::Game, "SetDisconnectFlag: 23");
			DisconnectFlag = GetCurrentTime();
			return FALSE;
		}
...
{
	Logger::getInstance()->output(LogType::Debug, "SEND POWER USER TO SERVER");
	smTRANS_COMMAND	TransCommand;
...
{
	Logger::getInstance()->output(LogType::Debug, "SendSetHackUser3: Type(%d)  LParam(%d)  SParam(%d)", Type, LParam, SParam);
...
int SendCrackWindow(HWND hWnd)
{
	Logger::getInstance()->output(LogType::Debug, "SendCrackWindow");
...
{
	Logger::getInstance()->output(LogType::Debug, "SendSetHackUser: StopGame(%d)", StopGame);
	...
	{//LOG_GAME
		...
		//Logger::getInstance()->output(LogType::Game, "SetDisconnectFlag: 25");
		DisconnectFlag = GetCurrentTime();
	}
...
{
	Logger::getInstance()->output(LogType::Debug, "SendSetHackUser2: Type(%d)  LParam(%d)", Type, LParam);
...
int	SendRecordTradeItems(sTRADE * lpTrade)
{
	...
	TransRecordTradeItems.Coin = lpTrade->Coin;
...
int	LoadWareHouse(TRANS_WAREHOUSE * lpTransWareHouse, sWAREHOUSE * lpWareHouse, int flag)
{
	...
	if (dwChkSum == lpTransWareHouse->dwChkSum)
	{
		...
		cWareHouse.Page = lpTransWareHouse->dwTemp[0];
		return TRUE;
	}
...
int DeleteEventItem_TimeOut(sITEMINFO * lpsItem)
{
	...
	//Rubinho
	if (lpsItem->CODE == (sinDB1 | sin31)|| lpsItem->CODE == (sinOR2 | sin50) ||
		lpsItem->CODE == (sinOA1 | sin50)|| lpsItem->CODE == (sinDS1 | sin31) ||
		lpsItem->CODE == (sinOA2 | sin31)|| lpsItem->CODE ==( sinBI2 | sin83) )
	{
		if (lpsItem->dwCreateTime + (60 * 60 * 24 * 7) < NowTime)
			return TRUE;
	}
	...
	if (lpsItem->CODE == (sinDA1 | sin44)|| lpsItem->CODE == (sinDA2 | sin44))
	...
	if (lpsItem->CODE == (sinCA1 | sin31) || lpsItem->CODE == (sinCA2 | sin32) ||
		...
		lpsItem->CODE == (sinCA1 | sin66) || lpsItem->CODE == (sinCA2 | sin66) ||
		lpsItem->CODE == (sinCA1 | sin68) || lpsItem->CODE == (sinCA2 | sin68) ||
	...
	if (lpsItem->CODE == (sinCA1 | sin67) || lpsItem->CODE == (sinCA2 | sin67) ||
		...
		lpsItem->CODE == (sinCA5 | sin04) || lpsItem->CODE == (sinCA6 | sin04) ||
		lpsItem->CODE == (sinCA5 | sin06) || lpsItem->CODE == (sinCA6 | sin06) ||
		...
		lpsItem->CODE == (sinCA5 | sin26) || lpsItem->CODE == (sinCA6 | sin26) ||
		lpsItem->CODE == (sinCA5 | sin28) || lpsItem->CODE == (sinCA6 | sin28) ||
		lpsItem->CODE == (sinCA5 | sin32) || lpsItem->CODE == (sinCA6 | sin32) ||
		lpsItem->CODE == (sinCA5 | sin34) || lpsItem->CODE == (sinCA6 | sin34) ||
		lpsItem->CODE == (sinCA5 | sin36) || lpsItem->CODE == (sinCA6 | sin36) ||
		lpsItem->CODE == (sinCA5 | sin38) || lpsItem->CODE == (sinCA6 | sin38) ||
		lpsItem->CODE == (sinCA5 | sin40) || lpsItem->CODE == (sinCA6 | sin40) ||
		lpsItem->CODE == (sinCA5 | sin42) || lpsItem->CODE == (sinCA6 | sin42) ||
		lpsItem->CODE == (sinCA5 | sin44) || lpsItem->CODE == (sinCA6 | sin44) ||
		lpsItem->CODE == (sinCA5 | sin46) || lpsItem->CODE == (sinCA6 | sin46))
	{
		if (lpsItem->dwCreateTime + (60 * 60 * 24 * 30) < NowTime)
			return true;
	}
	...
	//Trajes da arena
	if (lpsItem->CODE == (sinCA5 | sin39) || lpsItem->CODE == (sinCA6 | sin39) ||
