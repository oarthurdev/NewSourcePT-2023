...
sINVENTORY sInven[INVENTORY_MAX_POS] = {
	...
	{INVENTORY_POS_WingRight  ,{ 436 + 1 , 490 + 1 , 458 , 512 } },
};
...
int ForceOrbUseLevel[][2] = {
	...
	{ 50,CHAR_LEVEL_MAX },  // Devine
	{ 50,CHAR_LEVEL_MAX },  // Celesto
	{ 50,CHAR_LEVEL_MAX },  // Miraje
	{ 50,CHAR_LEVEL_MAX },  // Inferna
	{ 80,CHAR_LEVEL_MAX },  // Enigma
	{ 90,CHAR_LEVEL_MAX },  // Bellum
...
void cINVENTORY::RButtonDown(int x, int y)
{
	...
	if (SelectInvenItemIndex)
	{
		...
		if ((InvenItem[SelectInvenItemIndex - 1].CODE &sinITEM_MASK2) == sinGP1)
		{
			...
			if (DelayTime && (GetCurrentTime() - DelayTime < 3000)) {

				if (chaPremiumitem.haItemUseDelayFlag)
				{
					cMessageBox.ShowMessage("Espera 3 segundos para usar un nuevo cristal");
					return;
				}
...
int cINVENTORY::LoadItemInfo()
{
	...
	for (i = 0; i < INVENTORY_MAXITEM; i++)
	{
		if (InvenItemTemp[i].Flag)
		{
			...
			else if (InvenItemTemp[i].ItemPosition == 1)
			{
				if (InvenItemTemp[i].Class == ITEM_CLASS_WING)
					break;
			}
		...
		if (InvenItem[i].Flag)
		{
			...
			if (InvenItem[i].ItemPosition > 0)
			{
				...
				else
				{
					...
					else if (InvenItem[TempItemIndex[InvenItem[i].ItemPosition] - 1].Class == ITEM_CLASS_WING)
					{
						if (InvenItem[TempItemIndex[InvenItem[i].ItemPosition] - 1].ItemPosition == 15)
						{
							if (InvenItem[TempItemIndex[InvenItem[i].ItemPosition] - 1].CODE == InvenItem[sInven[1].ItemIndex - 1].CODE)
							{
						...
						if (InvenItem[TempItemIndex[InvenItem[i].ItemPosition] - 1].ItemPosition == 16)
						{
							if (InvenItem[TempItemIndex[InvenItem[i].ItemPosition] - 1].CODE == InvenItem[sInven[0].ItemIndex - 1].CODE)
							{
								InvenItem[sInven[14].ItemIndex - 1].Flag = 0;
			...
			if (InvenItem[i].Class == ITEM_CLASS_WEAPON_TWO)
			{
				...
				{
					if (InvenItem[i].CODE != InvenItem[sInven[14].ItemIndex - 1].CODE)
					{
						InvenItem[sInven[14].ItemIndex - 1].Flag = 0;
						InvenItem[i].Flag = 0;
						sInven[15].ItemIndex = 0;
			...
			else if (InvenItem[i].ItemPosition == 15)
			{
				if (InvenItem[i].Class == ITEM_CLASS_WING)
					continue;
			}
	...
	for (int i = 0; i < INVENTORY_MAX_POS; i++)
	{
		if (pItem->Class & sInven[i].Position)
		{
			if (!sInven[i].ItemIndex)
			{
				...
				else if (pItem->Class == ITEM_CLASS_WING)
				{
	...
	if (pItem->Class == ITEM_CLASS_WEAPON_TWO)
	{
		...
		else if (pItem->Class == ITEM_CLASS_WING)
		{
			if (sInven[14].ItemIndex)
			{
				InvenItem[sInven[14].ItemIndex - 1].Flag = 0;
				sInven[14].ItemIndex = 0;
				if (InvenItem[sInven[14].ItemIndex - 1].ItemPosition)
				{
					if (InvenItem[sInven[0].ItemIndex - 1].SetModelPosi)

					{
						sinSetCharItem(InvenItem[sInven[14].ItemIndex - 1].CODE, InvenItem[sInven[14].ItemIndex - 1].SetModelPosi, FALSE);
					}
				}
			}
		}
			if (sInven[15].ItemIndex)
			{
				InvenItem[sInven[15].ItemIndex - 1].Flag = 0;
				sInven[15].ItemIndex = 0;
				if (InvenItem[sInven[15].ItemIndex - 1].ItemPosition)
				{
					if (InvenItem[sInven[15].ItemIndex - 1].SetModelPosi)
					{
						sinSetCharItem(InvenItem[sInven[15].ItemIndex - 1].CODE, InvenItem[sInven[15].ItemIndex - 1].SetModelPosi, FALSE);
					}
			}
		}
...
int cINVENTORY::SetTwoHandItem(sITEM *pItem)
{
	...
	else if (pItem->ItemPosition == ITEM_CLASS_WING)
	{
		if (pItem->ItemPosition == 15)
		{
			pItem->ItemPosition = 15;
			pItem->x = sInven[14].Rect.left + (((sInven[14].Rect.right - sInven[14].Rect.left) - pItem->w) / 2);
			pItem->y = sInven[14].Rect.top + (((sInven[14].Rect.bottom - sInven[14].Rect.top) - pItem->h) / 2);
		}
		TempTwoHandItem.x = sInven[15].Rect.left + (((sInven[15].Rect.right - sInven[15].Rect.left) - pItem->w) / 2);
		TempTwoHandItem.y = sInven[15].Rect.top + (((sInven[15].Rect.bottom - sInven[15].Rect.top) - pItem->h) / 2);
		TempTwoHandItem.w = pItem->w;
		TempTwoHandItem.h = pItem->h;
		TempTwoHandItem.ItemPosition = 15;
		TempTwoHandItem.Class = pItem->Class;
		TempTwoHandItem.sItemInfo.CODE = pItem->CODE;
		TempTwoHandItem.sItemInfo.Price = pItem->sItemInfo.Price;
		memcpy(&TempTwoHandItem.lpItem, &pItem->lpItem, sizeof(LPDIRECT3DTEXTURE9));
...
int cINVENTORY::PickUpTwoHandItem(sITEM *pItem)
{
	...
	else if (pItem->ItemPosition == 15)
	{
		if (sInven[14].ItemIndex && sInven[15].ItemIndex)
		{
			memcpy(&TempPickItem, &InvenItem[sInven[14].ItemIndex - 1], sizeof(sITEM));
			InvenItem[sInven[14].ItemIndex - 1].Flag = 0;
			InvenItem[sInven[15].ItemIndex - 1].Flag = 0;
			sInven[14].ItemIndex = 0;
			sInven[15].ItemIndex = 0;
...
int cINVENTORY::OverlapTwoHandItem(sITEM *pItem)
{
	...
	else if (pItem->Class ==  ITEM_CLASS_WING)
	{
		if (sInven[15].ItemIndex)
		{
			if (InvenItem[sInven[15].ItemIndex - 1].Class != ITEM_CLASS_WING)
			{
				if (pItem->ItemPosition == 15)
				{
					if (sInven[15].ItemIndex)
		...
		if (InvenItem[i].Flag)
		{
				...
				desRect.top > rect.top - desRect.bottom && desRect.top < rect.bottom)
			{
				...
				if (!CostumeFlag && InvenItem[i].ItemPosition == 14)
					continue;
				if (InvenItem[i].ItemPosition == ITEM_CLASS_WING)
				return i + 1;
			}
		}
	}
...
int cINVENTORY::ChangeInvenItem(sITEM *pItem)
{
	...
	if (CrashItemIndex[0])
	{
		...
		if (InvenItem[CrashItemIndex[0] - 1].Class == ITEM_CLASS_WEAPON_TWO || InvenItem[CrashItemIndex[0] - 1].Class == ITEM_CLASS_WING)
...
int cINVENTORY::DrawInvenText()
{
	...
	if (OpenFlag)
	{
		...
		dsTextLineOut(390, 495, strBuff, lstrlen(strBuff));
...
int cINVENTORY::SetItemToChar(int CheckHackFlag)
{
	...
	//Set Boss #2
	if (cInvenTory.InvenItem[sInven[4].ItemIndex - 1].sItemInfo.CODE == (sinOR1 | sin27)) {
		LShyRing = 1;
	}
	else { LShyRing = 0; }
	if (cInvenTory.InvenItem[sInven[5].ItemIndex - 1].sItemInfo.CODE == (sinOR1 | sin27)) {
		RShyRing = 1;
	}
	else { RShyRing = 0; }
	if (cInvenTory.InvenItem[sInven[3].ItemIndex - 1].sItemInfo.CODE == (sinOA1 | sin27)) {
		TullaAmulet = 1;
	}
	else { TullaAmulet = 0; }
	...
	int baseWeight = int((((fstrength * 2.0f) + (fhealth * 1.5f) + fLevel * 3.0f) + 60 + fDexterity * 1.2f + fSpirit * 2.0f));
	...
	/*if (sinChar->GravityScroolCheck[0] == 0 && chaPremiumitem.m_MightofAwellTime < 0)
	{
		sinChar->Weight[1] = (short)baseWeight;
		...
		if (InvenItemTemp[i].Flag)
		{
			if (CompareItems(&pItem->sItemInfo, &InvenItemTemp[i].sItemInfo))
			{
				...
				if (InvenItemTemp[i].Class == ITEM_CLASS_WEAPON_TWO)
				{
					...
					else if (InvenItemTemp[i].Class == ITEM_CLASS_WING)
					{
						if (InvenItemTemp[i].ItemPosition == 15)
						{
							for (int k = 0; k < INVENTORY_MAXITEM; k++)
							{
								if (InvenItemTemp[k].Flag)
								{
									if (InvenItemTemp[k].ItemPosition == 15)
									{
										if (InvenItemTemp[i].sItemInfo.CODE == InvenItemTemp[k].sItemInfo.CODE)
											InvenItemTemp[k].Flag = 0;
									}
