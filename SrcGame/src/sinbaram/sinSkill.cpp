...
{ "Elixir da Ira", ELIXIR_IRA, "", "Elixir_Ira.tga" },
	...
	if (SkillInfoShowFlag || sinMoveKindInter[SIN_SKILL])
	{
		...
		if (OpenFlag)
		{
			...
			if (sinSkill.SkillPoint >= 0 || sinSkill.SkillPoint4 >= 0)
			{
				...
				dsTextLineOut(0, CheckEditSize(165, 253, strBuff), 578 + (256 - sinMoveKindInter[SIN_SKILL]),
					strBuff, lstrlen(strBuff));


				wsprintf(strBuff, "%d", sinSkill.SkillPoint4);
				dsTextLineOut(0, CheckEditSize(250, 253, strBuff), 578 + (256 - sinMoveKindInter[SIN_SKILL]),
					strBuff, lstrlen(strBuff));
			...
			if (SkillMasteryShowFlag)
			{
				...
				dsTextLineOut(0, MasteryShowX, UseSkillMasteryPosi.y + 6 + (256 - sinMoveKindInter[SIN_SKILL]),
					strBuff, lstrlen(strBuff));
				...
				for (int j = 0; j < len2; j++)
				{
					if (szSkillInfoBuffBack[i] == '\r')
					{
						if (szSkillInfoBuff2Back[j] == '\r')
						{
							...
							SIZE sizecheck = { 0 };
							GetFontTextExtentPoint(0, pSkillInfo[Count], strlen(pSkillInfo[Count]), &sizecheck);
							...
							if (Count + 1 == 1)
							{
								...
								dsTextLineOut(0, SetTextXposi, SkillInfoBoxPosi.y + 27 + ((Count - 1) * 14), pSkillInfo[Count], Textlen);
								BoldFlag = 1;
							...
							if (!BoldFlag)
							{
								dsTextLineOut(0, SetTextXposi, SkillInfoBoxPosi.y + 28 + ((Count - 1) * 15), pSkillInfo[Count], Textlen);

								COLORREF colour = GetTextColor(hdc);
								SetFontTextColor( RGB(173, 51, 243));
								dsTextLineOut(0, SetTextXposi2, SkillInfoBoxPosi.y + 28 + ((Count - 1) * 15), pSkillInfo2[Count], Textlen2);
					...
					if (TempSkillInfo[i] == '\r')
					{
						if (TempSkillInfo2[j] == '\r')
						{
							...
							if (Count + 1 == 1)
							{
								...
								SetFontTextColor( RGB(222, 231, 255));
								dsTextLineOut(0, SetTextXposi, SkillUpInfoPosi.y + 27 + ((Count - 1) * 14), pSkillMasterInfo[Count], Textlen);
								BoldFlag = 1;
							...
							if (!BoldFlag)
							{
								dsTextLineOut(0, SetTextXposi, SkillUpInfoPosi.y + 28 + ((Count - 1) * 15), pSkillMasterInfo[Count], Textlen);
								dsTextLineOut(0, SetTextXposi2, SkillUpInfoPosi.y + 28 + ((Count - 1) * 15), pSkillMasterInfo2[Count], Textlen2);
								dsTextLineOut(0, SetTextXposi2 + 1, SkillUpInfoPosi.y + 28 + ((Count - 1) * 15), pSkillMasterInfo2[Count], Textlen2);
							}
							Count++;
							break;
						}
...
string convertSecondsToDate(long int seconds)
{
...
int cSKILL::ShowSkillInfo(sSKILL *pSkill, sSKILLBOX *pSkillBox, int Icon)
{
	...
	if (pSkill->Point)
	{
		if (pSkill->CODE == PLAYER_VIP || pSkill->CODE == DELAY_GLOBAL || pSkill->CODE == QUEST_DIARY || pSkill->CODE == SKILL_FORCE_ORB || pSkill->CODE == CHANGE_JOB3_QUEST || pSkill->CODE == CHANGE_ELEMENTARY_QUEST)
		{
	...
	else if (pSkill->CODE == SOD_SKILL_GOLD)
	{
		...
		lstrcat(szSkillInfoBuff, "Habilidade concedida ao \rClan que sobreviviu a Bellatra.\r\r");
		lstrcat(szSkillInfoBuff2, "\r");
	...
	{
		lstrcat(szSkillInfoBuff2, "\r");
		lstrcat(szSkillInfoBuff, "Habilidade concedida ao \rClan que sobrevivio a Bellatra.\r\r");
		lstrcat(szSkillInfoBuff2, "\r");
		lstrcat(szSkillInfoBuff, "+ 60 de vida\r");
		lstrcat(szSkillInfoBuff2, "\r");
		lstrcat(szSkillInfoBuff2, "\r");
		lstrcat(szSkillInfoBuff2, "\r");
	...
	else if (pSkill->CODE == SOD_SKILL_BRONZE)
	{
		...
		lstrcat(szSkillInfoBuff, "Habilidade concedida ao \rClan que sobrevivio a Bellatra.\r\r");
		lstrcat(szSkillInfoBuff2, "\r");
		lstrcat(szSkillInfoBuff, "+ 30 de vida\r");
		lstrcat(szSkillInfoBuff2, "\r");
		lstrcat(szSkillInfoBuff2, "\r");
		lstrcat(szSkillInfoBuff2, "\r");
	...
	else if (pSkill->CODE == HIGH_LEVEL_MERIT_SKILL)
	{
		// ID
		lstrcpy(szSkillInfoBuff, "Top Classe\r");
		lstrcat(szSkillInfoBuff2, "\r");
		lstrcat(szSkillInfoBuff, "\r");
		lstrcat(szSkillInfoBuff, "Add +30 HP/MP/STM\r");
		//lstrcat(szSkillInfoBuff, "Add +1 ATK Speed\r\r");
	...
	{
		// ID
		lstrcpy(szSkillInfoBuff, "Top PVP\r");
		lstrcat(szSkillInfoBuff2, "\r");
		lstrcat(szSkillInfoBuff, "\r");
		lstrcat(szSkillInfoBuff, "+ 5% attack power\r");
		//lstrcat(szSkillInfoBuff, "+ 150 HP\r");
		//lstrcat(szSkillInfoBuff2, "\r");
		//lstrcat(szSkillInfoBuff2, "\r");
	...
	/*else if (pSkill->CODE == PLAYER_ARENA)
	{
		...
		lstrcat(szSkillInfoBuff2, "\r");
		lstrcat(szSkillInfoBuff, "+100 de HP\r");

		LineCount = 4;
	}*/

	else if (pSkill->CODE == BUFF_WARMODE)
	{
		lstrcpy(szSkillInfoBuff, "Modo PvP Ativo\r");
		lstrcat(szSkillInfoBuff2, "\r");
		lstrcat(szSkillInfoBuff, "\r");
		lstrcat(szSkillInfoBuff, "Adicional Drop 5% \r");
		lstrcat(szSkillInfoBuff, "Adicional Exp 5% \r");

		LineCount = 4;
	}

	else if (pSkill->CODE == ELIXIR_IRA)
	{

		lstrcpy(szSkillInfoBuff, "Elixir da Ira\r");
		lstrcat(szSkillInfoBuff2, "\r");
		lstrcat(szSkillInfoBuff, "+ 3% de poder de ataque\r");

		LineCount = 2;
	}
	else if (pSkill->CODE == ELIXIR_GLORIA)
	{

	lstrcpy(szSkillInfoBuff, "Elixir da Gloria\r");
	lstrcat(szSkillInfoBuff2, "\r");
	...
	else if (pSkill->CODE == PLAYERVIP)
	{
		...
		lstrcat(szSkillInfoBuff2, "\r");
		lstrcat(szSkillInfoBuff, "Seu vip expira dia\r");

		data = hoje + (86400 * pSkill->UseTime);

		std::string dataFormatada = convertSecondsToDate(data);
		strcpy_s(szBuff, dataFormatada.c_str());

		lstrcat(szSkillInfoBuff, szBuff);
		lstrcat(szSkillInfoBuff, "\r");
	...
	case SKILL_AVANGING_CRASH:
		...
		wsprintf(szTempSkillDoc, "Bônus de 30%% contra monstros\r"); //½ºÅ³ ÀÌ¸§  
	...
	for (i = 0; i < 2; i++)
	{
		...
		case PLAYER_VIP:
		{
			lstrcpy(szTempSkillDoc, "ATK Percent Add:\r");
			lstrcat(szSkillInfoBuff, szTempSkillDoc);
			wsprintf(szTempSkillDoc2, "%d%s\r", IsVIP.atk, "%");
			lstrcat(szSkillInfoBuff2, szTempSkillDoc2);
			LineCount++;

			lstrcpy(szTempSkillDoc, "EXP Percent Add:\r");
			lstrcat(szSkillInfoBuff, szTempSkillDoc);
			wsprintf(szTempSkillDoc2, "%d%s\r", IsVIP.exp, "%");
			lstrcat(szSkillInfoBuff2, szTempSkillDoc2);
			LineCount++;

			lstrcpy(szTempSkillDoc, "Drop Percent Add:\r");
			lstrcat(szSkillInfoBuff, szTempSkillDoc);
			wsprintf(szTempSkillDoc2, "%d%s\r", IsVIP.drop, "%");
			lstrcat(szSkillInfoBuff2, szTempSkillDoc2);
			LineCount++;

			lstrcpy(szTempSkillDoc, "HP Percent Add:\r");
			lstrcat(szSkillInfoBuff, szTempSkillDoc);
			wsprintf(szTempSkillDoc2, "%d%s\r", IsVIP.hp, "%");
			lstrcat(szSkillInfoBuff2, szTempSkillDoc2);
			LineCount++;

			lstrcpy(szTempSkillDoc, "MP Percent Add:\r");
			lstrcat(szSkillInfoBuff, szTempSkillDoc);
			wsprintf(szTempSkillDoc2, "%d%s\r", IsVIP.mp, "%");
			lstrcat(szSkillInfoBuff2, szTempSkillDoc2);
			LineCount++;

			lstrcpy(szTempSkillDoc, "STM Percent Add:\r");
			lstrcat(szSkillInfoBuff, szTempSkillDoc);
			wsprintf(szTempSkillDoc2, "%d%s\r", IsVIP.res, "%");
			lstrcat(szSkillInfoBuff2, szTempSkillDoc2);
			LineCount++;

			UINT Days = VIPTime / 60 / 60 / 24;
			UINT Hours = (VIPTime / 60 / 60) % 24;
			UINT Minutes = (VIPTime / 60) % 60;
			UINT Seconds = VIPTime % 60;

			lstrcpy(szTempSkillDoc, "Tempo Restante:\r");
			lstrcat(szSkillInfoBuff, szTempSkillDoc);
			wsprintf(szTempSkillDoc2, "%02d:%02d:%02d\r", Days, Hours, Minutes);
			lstrcat(szSkillInfoBuff2, szTempSkillDoc2);
			LineCount++;
