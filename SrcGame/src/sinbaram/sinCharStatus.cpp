...
sCHARRECTPOSI	sCharRectPosi[MAX_CHAR_RECT_POSI] = {
	...
	{"Life"		,{ 364  , 430 + ReHeight3  , 455 , 440  }},
	{"Mana"     ,{ 364  , 450 + ReHeight3  , 455 , 466  }},
	{"Stamina"  ,{ 364  , 471 + ReHeight3  , 455 , 490  }},
	...
	{"Bionic"   ,{ 215 , 448 , 215  , 468 }},
	{"Poison"   ,{ 260 , 448 , 260  , 469 }},
	...
	{"LifeRegen"      ,{ 424 , 430 + ReHeight3 , 515  , 440 }},
	{"ManaRegen"      ,{ 424 , 450 + ReHeight3 , 515  , 466 }},
	{"StaminaRegen"      ,{ 424 , 471 + ReHeight3 , 515  , 490 }},

	{"Critical"      ,{ 264 , 496, 264  , 543 }},
	{"Block"      ,{ 264 , 516, 264  , 543 }},
	{"Evade"      ,{ 264 , 537, 264  , 543 }},
};
//sCHARRECTPOSI	sCharRectPosi[MAX_CHAR_RECT_POSI] = {
//
//	{ "Class"    ,{ 75   , 446 + ReHeight3  , 160 , 454 } },
//	{ "Name"     ,{ 75   , 467 + ReHeight3  , 160 , 479 } },
//	{ "Level"    ,{ 75   , 507 + ReHeight3  , 128 , 504 } },
//	{ "Exp"      ,{ 75   , 543 + ReHeight3  , 160 , 549 } },
//	{ "Next_Exp" ,{ 75   , 563 + ReHeight3  , 160 , 574 } },
//	{ "Life"		,{ 345  , 431 + ReHeight3  , 470 , 440 } },
//	{ "Mana"     ,{ 345  , 451 + ReHeight3  , 470 , 466 } },
//	{ "Stamina"  ,{ 345  , 470 + ReHeight3  , 470 , 490 } },
//
//
//	{ "Strength" ,{ 585 , 433 + ReHeight3 , 632 , 444 } },
//	{ "Spirit"   ,{ 585 , 454 + ReHeight3 , 632 , 464 } },
...
//
//
//	{ "Bionic"   ,{ 200 + 3 , 432 + 8 + ReHeight3 , 231  , 468 } },
//	{ "Poison"   ,{ 248 + 3 , 432 + 8 + ReHeight3 , 279  , 469 } },
//	{ "Fire"     ,{ 293 , 432 + 8 + ReHeight3 , 320  , 469 } },
//	{ "Lighting" ,{ 200 + 3 , 456 + 8 + ReHeight3 , 231  , 505 } },
//	{ "Ice"      ,{ 248 + 3 , 456 + 8 + ReHeight3 , 279  , 543 } },
//
//
//	{ "Attack_Rating" ,{ 710 , 455 + ReHeight3 - 1 , 769 , 464 } },
//	{ "Attack_Damage" ,{ 710 , 474 + ReHeight3 - 1 , 769 , 484 } },
...
void cCHARSTATUS::Release()
{
	...
	if(lpStatusPoint[0])
	{
		lpStatusPoint[0]->Release();
		lpStatusPoint[0] = 0;
	}
	if (lpStatusPoint[1])
	{
		lpStatusPoint[1]->Release();
		lpStatusPoint[1] = 0;
	}
}
...
void cCHARSTATUS::Main()
{
	...
	if(OpenFlag)
	{
		...
		if (pCursorPos.x > 620 && pCursorPos.x < 639 && pCursorPos.y > 531 && pCursorPos.y < 550 && sinChar->StatePoint >= 10)
			ShowFastAddPoint[0] = TRUE;
		else
			ShowFastAddPoint[0] = FALSE;
		if (sinChar->StatePoint < 10)
...
void cCHARSTATUS::LButtonDown(int x, int y)
{
	...
	if(OpenFlag)
	{
		if(x > 251 && x < 269 && y > 565 && y < 583)
			OpenFlag = SIN_CLOSE;
		if (x > 620 && x < 639 && y > 531 && y < 550 && sinChar->StatePoint >= 10)
		{
...
void cCHARSTATUS::DrawCharText()
{
	...
	dsTextLineOut(0, CheckEditSize(sCharRectPosi[0].Rect.left, sCharRectPosi[0].Rect.right, strBuff), sCharRectPosi[0].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
		strBuff, lstrlen(strBuff));

	wsprintf(strBuff, "%s", sinCharDisplay.szName);
	dsTextLineOut(0, CheckEditSize(sCharRectPosi[1].Rect.left, sCharRectPosi[1].Rect.right, strBuff), sCharRectPosi[1].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
		strBuff, lstrlen(strBuff));
		...
		cldata.myPosition == 101 || cldata.myPosition == 104)
	{
		...
		dsTextLineOut(0, CheckEditSize(sCharRectPosi[1].Rect.left, sCharRectPosi[1].Rect.right, strBuff) + TempLen, sCharRectPosi[1].Rect.top + 20 + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
			strBuff, lstrlen(strBuff));
	}

	wsprintf(strBuff, "%d", sinCharDisplay.Level);
	dsTextLineOut(0, CheckEditSize(sCharRectPosi[2].Rect.left, sCharRectPosi[2].Rect.right, strBuff), sCharRectPosi[2].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
		strBuff, lstrlen(strBuff));
	...
	if(sinChar->Level > 79)
	{
		...
		dsTextLineOut(0, CheckEditSize(sCharRectPosi[3].Rect.left, sCharRectPosi[3].Rect.right, strBuff), sCharRectPosi[3].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
			strBuff, lstrlen(strBuff));
	...
	if(sinChar->Level > 79)
	{
		...
		dsTextLineOut(0, CheckEditSize(sCharRectPosi[4].Rect.left, sCharRectPosi[4].Rect.right, strBuff), sCharRectPosi[4].Rect.top + (254 - sinMoveKindInter[SIN_CHARSTATUS]),
			strBuff, lstrlen(strBuff));
	}
	else
	{
	...
	int elixirgloria = 0;
	if (cSkill.SearchContiueSkillCODE(ELIXIR_GLORIA))
		elixirgloria += 150;


	SetFontTextColor( CharStatusColor[SkillTextColor[SIN2_LIFE]]);
	wsprintf(strBuff, "%d/%d", sinGetLife() + AddVirtualLife[0], sinCharDisplay.Life[1] + AddVirtualLife[1] + elixirgloria);
	dsTextLineOut(0, CheckEditSize(sCharRectPosi[5].Rect.left, sCharRectPosi[5].Rect.right, strBuff), sCharRectPosi[5].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
		strBuff, lstrlen(strBuff));

	SetFontTextColor( CharStatusColor[SkillTextColor[SIN2_LIFE]]);
	sprintf(strBuff, "%.1f", sinCharDisplay.Life_Regen);
	dsTextLineOut(0, CheckEditSize(sCharRectPosi[25].Rect.left, sCharRectPosi[24].Rect.right, strBuff), sCharRectPosi[24].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]), strBuff, lstrlen(strBuff));

	SetFontTextColor( CharStatusColor[SkillTextColor[SIN2_MANA]]);
	wsprintf(strBuff, "%d/%d", sinGetMana(), sinCharDisplay.Mana[1]);
	dsTextLineOut(0, CheckEditSize(sCharRectPosi[6].Rect.left, sCharRectPosi[6].Rect.right, strBuff), sCharRectPosi[6].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
		strBuff, lstrlen(strBuff));

	SetFontTextColor( CharStatusColor[SkillTextColor[SIN2_MANA]]);
	sprintf(strBuff, "%.1f", sinCharDisplay.Mana_Regen);
	dsTextLineOut(0, CheckEditSize(sCharRectPosi[26].Rect.left, sCharRectPosi[25].Rect.right, strBuff), sCharRectPosi[25].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]), strBuff, lstrlen(strBuff));

	SetFontTextColor( CharStatusColor[SkillTextColor[SIN2_STAMINA]]);
	wsprintf(strBuff, "%d/%d", sinGetStamina(), sinCharDisplay.Stamina[1]);
	dsTextLineOut(0, CheckEditSize(sCharRectPosi[7].Rect.left, sCharRectPosi[7].Rect.right, strBuff), sCharRectPosi[7].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
		strBuff, lstrlen(strBuff));

	SetFontTextColor( CharStatusColor[SkillTextColor[SIN2_STAMINA]]);
	sprintf(strBuff, "%.1f", sinCharDisplay.Stamina_Regen);
	dsTextLineOut(0, CheckEditSize(sCharRectPosi[26].Rect.left, sCharRectPosi[26].Rect.right, strBuff), sCharRectPosi[26].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]), strBuff, lstrlen(strBuff));

	SetFontTextColor( RGB(255, 255, 255));

	wsprintf(strBuff, "%d", sinCharDisplay.Strength);
	dsTextLineOut(0, CheckEditSize(sCharRectPosi[8].Rect.left, sCharRectPosi[8].Rect.right, strBuff), sCharRectPosi[8].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
		strBuff, lstrlen(strBuff));


	wsprintf(strBuff, "%d", sinCharDisplay.Spirit);
	dsTextLineOut(0, CheckEditSize(sCharRectPosi[9].Rect.left, sCharRectPosi[9].Rect.right, strBuff), sCharRectPosi[9].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
		strBuff, lstrlen(strBuff));



	wsprintf(strBuff, "%d", sinCharDisplay.Talent);
	dsTextLineOut(0, CheckEditSize(sCharRectPosi[10].Rect.left, sCharRectPosi[10].Rect.right, strBuff), sCharRectPosi[10].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
		strBuff, lstrlen(strBuff));



	wsprintf(strBuff, "%d", sinCharDisplay.Dexterity);
	dsTextLineOut(0, CheckEditSize(sCharRectPosi[11].Rect.left, sCharRectPosi[11].Rect.right, strBuff), sCharRectPosi[11].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
		strBuff, lstrlen(strBuff));

	
	wsprintf(strBuff, "%d", sinCharDisplay.Health);
	dsTextLineOut(0, CheckEditSize(sCharRectPosi[12].Rect.left, sCharRectPosi[12].Rect.right, strBuff), sCharRectPosi[12].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
		strBuff, lstrlen(strBuff));

	if(sinChar->StatePoint)
	{
		wsprintf(strBuff, "%d", sinChar->StatePoint);
		dsTextLineOut(0, CheckEditSize(sCharRectPosi[23].Rect.left, sCharRectPosi[23].Rect.right, strBuff), sCharRectPosi[23].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
			strBuff, lstrlen(strBuff));
	...
	dsTextLineOut(0, CheckEditSize(sCharRectPosi[18].Rect.left, sCharRectPosi[18].Rect.right, strBuff), sCharRectPosi[18].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
		strBuff, lstrlen(strBuff));
	...
	if (cSkill.SearchContiueSkillCODE(ELIXIR_IRA))
	...
	dsTextLineOut(0, CheckEditSize(sCharRectPosi[19].Rect.left, sCharRectPosi[19].Rect.right, strBuff), sCharRectPosi[19].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
		strBuff, lstrlen(strBuff));
	...
	dsTextLineOut(0, CheckEditSize(sCharRectPosi[20].Rect.left, sCharRectPosi[20].Rect.right, strBuff), sCharRectPosi[20].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
		strBuff, lstrlen(strBuff));
	...
	dsTextLineOut(0, CheckEditSize(sCharRectPosi[21].Rect.left, sCharRectPosi[21].Rect.right, strBuff), sCharRectPosi[21].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
		strBuff, lstrlen(strBuff));


	SetFontTextColor( CharStatusColor[SkillTextColor[SIN2_SPEED]]);
	wsprintf(strBuff, "%d", sinCharDisplay.Move_Speed);
	dsTextLineOut(0, CheckEditSize(sCharRectPosi[22].Rect.left, sCharRectPosi[22].Rect.right, strBuff), sCharRectPosi[22].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
		strBuff, lstrlen(strBuff));


	SetFontTextColor(RGB(255, 255, 255));
	wsprintf(strBuff, "%d", sinCharDisplay.Resistance[0]);
	dsTextLineOut(0,CheckEditSize(sCharRectPosi[13].Rect.left, sCharRectPosi[13].Rect.right, strBuff) - 5, sCharRectPosi[13].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
		strBuff, lstrlen(strBuff));

	wsprintf(strBuff, "%d", sinCharDisplay.Resistance[5]);
	dsTextLineOut(0,CheckEditSize(sCharRectPosi[14].Rect.left, sCharRectPosi[14].Rect.right, strBuff) - 3, sCharRectPosi[14].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
		strBuff, lstrlen(strBuff));

	wsprintf(strBuff, "%d", sinCharDisplay.Resistance[2]);
	dsTextLineOut(0,CheckEditSize(sCharRectPosi[15].Rect.left, sCharRectPosi[15].Rect.right, strBuff) - 1, sCharRectPosi[15].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
		strBuff, lstrlen(strBuff));

	wsprintf(strBuff, "%d", sinCharDisplay.Resistance[4]);
	dsTextLineOut(0,CheckEditSize(sCharRectPosi[16].Rect.left, sCharRectPosi[16].Rect.right, strBuff) - 5, sCharRectPosi[16].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
		strBuff, lstrlen(strBuff));

	wsprintf(strBuff, "%d", sinCharDisplay.Resistance[3]);
	dsTextLineOut(0,CheckEditSize(sCharRectPosi[17].Rect.left, sCharRectPosi[17].Rect.right, strBuff) - 3, sCharRectPosi[17].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]),
		strBuff, lstrlen(strBuff));

	wsprintf(strBuff, "%d%%", sinCharDisplay.Critical_Hit);
	dsTextLineOut(hdc, CheckEditSize(sCharRectPosi[27].Rect.left, sCharRectPosi[27].Rect.right, strBuff), sCharRectPosi[27].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]), strBuff, lstrlen(strBuff));

	wsprintf(strBuff, "%d%%", sinCharDisplay.Chance_Block);
	dsTextLineOut(hdc, CheckEditSize(sCharRectPosi[28].Rect.left, sCharRectPosi[27].Rect.right, strBuff), sCharRectPosi[28].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]), strBuff, lstrlen(strBuff));


	/*wsprintf(strBuff, "%d%%", sinbaram_Evade);
	dsTextLineOut(hdc, CheckEditSize(sCharRectPosi[29].Rect.left, sCharRectPosi[29].Rect.right, strBuff), sCharRectPosi[29].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]), strBuff, lstrlen(strBuff));*/
	...
	//dsTextLineOut(0,CheckEditSize(sCharRectPosi[30].Rect.left, sCharRectPosi[30].Rect.right, strBuff), sCharRectPosi[30].Rect.top + (253 - sinMoveKindInter[SIN_CHARSTATUS]), strBuff, lstrlen(strBuff));
...
void cCHARSTATUS::OtherStateShow()
{
	...
	dsTextLineOut(0, 100, 300, strBuff, lstrlen(strBuff));

	wsprintf(strBuff, "ºí·°À²:%d", sinCharDisplay.Chance_Block);
	dsTextLineOut(0, 200, 300, strBuff, lstrlen(strBuff));

	wsprintf(strBuff, "Å©¸®Æ¼ÄÃ:%d", sinCharDisplay.Critical_Hit);
	dsTextLineOut(0, 300, 300, strBuff, lstrlen(strBuff));

	wsprintf(strBuff, "½´ÆÃ±æÀÌ:%d", sinCharDisplay.Shooting_Range);
	dsTextLineOut(0, 400, 300, strBuff, lstrlen(strBuff));
