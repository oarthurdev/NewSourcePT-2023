	...
	for (i = 0; i < 20; i++)
	{
		if (sMessageBox2[i].Flag)
		{
			if (MessageArrowPosi == 1)
			{
				...
				{
					if (cCoinShop.Coin < sMessageBox2[i].Count)
					{
						sMessageBox2[i].Count = cCoinShop.Coin;
					}
				}
...
int cMESSAGEBOX::ShowMessageEvent(char *Msg)
{
	...
	sMessageBox[49].x = SETTINGSHANDLER->getWidth() / 2 - ((sMessageBox[49].len * 10) / 2) + 40;
	sMessageBox[49].y = 5;
...
int cMESSAGEBOX::ShowMessage(int Kind)
{
	...
	{
		sMessageBox[Kind].x = SETTINGSHANDLER->getWidth() / 2 - ((sMessageBox[Kind].len * 10) / 2) + 40;
		sMessageBox[Kind].y = SETTINGSHANDLER->getHeight() / 2 - (64 / 2);
...
int cMESSAGEBOX::ShowMessage1(const char* msg)
{
	for (int i = 0; i < 100; i++)
	{
		if (sMessageBox[i].Flag)
	...
	sMessageBox[49].Flag = 50;
	strcpy_s(sMessageBox[49].MessageDoc, msg);
	sMessageBox[49].len = lstrlen(msg);
	sMessageBox[49].x = SETTINGSHANDLER->getWidth() / 2 - ((sMessageBox[49].len * 10) / 2) + 40;
	sMessageBox[49].y = 150;
	sMessageBox[49].Line = (sMessageBox[49].len * 10 / 16) - 6;
...
void cMESSAGEBOX::DrawMessageText()
{
	...
	if (sStarShop.Flag)
	{
		...
		dsTextLineOut(0,BackStartPos.x + 48, BackStartPos.y + 35, HoldStarNumDoc, lstrlen(HoldStarNumDoc));

		wsprintf(HoldStarNum, "%s", "x");
		dsTextLineOut(0,BackStartPos.x + 120, BackStartPos.y + 75, HoldStarNum, lstrlen(HoldStarNum));

		wsprintf(HoldStarNum, "%d", sStarShop.Count);
		dsTextLineOut(0,BackStartPos.x + 160, BackStartPos.y + 75, HoldStarNum, lstrlen(HoldStarNum));

		dsTextLineOut(0,BackStartPos.x + 83, BackStartPos.y + 140, StarItemBuy7, lstrlen(StarItemBuy7));

		SetFontTextColor(RGB(222, 231, 25));
		dsTextLineOut(0,BackStartPos.x + 53, BackStartPos.y + 120, ChangeMoney7, lstrlen(ChangeMoney7));
	...
	if (sLost_Item.Flag)
	{
		...
		if (OverDay_Item_Flag)
		{
			...
			dsTextLineOut(0,BackStartPos.x + 110, BackStartPos.y + 90, szTempBuff5, lstrlen(szTempBuff5));

		}
		else
		{
			dsTextLineOut(0,BackStartPos.x + 110, BackStartPos.y + 90, Quset3ItemDoc15, lstrlen(Quset3ItemDoc15));
		...
		dsTextLineOut(0,BackStartPos.x + 110, BackStartPos.y + 120, Quset3ItemDoc16, lstrlen(Quset3ItemDoc16));
	...
	if (sMessageBox_RecvItem.Flag)
	{
		...
		if ((len * 7) > 256)
		{
			...
			dsTextLineOut(0,BackStartPos.x + 10, BackStartPos.y + 25, szBuffDoc, lstrlen(szBuffDoc));
			dsTextLineOut(0,BackStartPos.x + 10, BackStartPos.y + 40, szBuffDoc2, lstrlen(szBuffDoc2));
		}
		else
		{
			XPosi = ((256 - (len * 6)) / 2);
			if (XPosi < 10)XPosi = 10;
			dsTextLineOut(0,BackStartPos.x + XPosi, BackStartPos.y + 30, sMessageBox_RecvItem.szDoc, lstrlen(sMessageBox_RecvItem.szDoc));
		}
		...
		if (sMessageBox_RecvItem.RecvItem.SellPrice)
		{
			...
			dsTextLineOut(0,BackStartPos.x + 120, BackStartPos.y + 90, szBuffDoc, lstrlen(szBuffDoc));
			...
			SetFontTextColor(RGB(255, 255, 255));
			dsTextLineOut(0,BackStartPos.x + 130, BackStartPos.y + 120, Money5, lstrlen(Money5));
		...
		else if (sMessageBox_RecvItem.RecvItem.OldX)
		{
			...
			SetFontTextColor(RGB(255, 255, 255));
			dsTextLineOut(0,BackStartPos.x + 120, BackStartPos.y + 120, Exp5, lstrlen(Exp5));
		...
		else if (sMessageBox_RecvItem.RecvItem.PotionCount)
		{
			...
			SetFontTextColor(RGB(255, 255, 255));
			dsTextLineOut(0,BackStartPos.x + 120, BackStartPos.y + 120, RecvPotion7, lstrlen(RecvPotion7));
		...
		else
		{
			...
			SetFontTextColor(RGB(128, 255, 128));
			dsTextLineOut(0,BackStartPos.x + 110, BackStartPos.y + 90, sMessageBox_RecvItem.szItem, lstrlen(sMessageBox_RecvItem.szItem));
			...
			SetFontTextColor(RGB(255, 255, 255));
			dsTextLineOut(0,BackStartPos.x + 120, BackStartPos.y + 120, Itemul, lstrlen(Itemul));
		...
		SetFontTextColor(RGB(255, 255, 255));
		dsTextLineOut(0,BackStartPos.x + 110, BackStartPos.y + 140, ItemRecvOk, lstrlen(ItemRecvOk));
	...
	if (sinMessageBoxShowFlag || TempFlag)
	{
		...
		for (i = 0; i < 100; i++)
		{
			if (sMessageBox[i].Flag)
			{
				if (i == MESSAGE_100LVITEM_OK)
				{
					...
					dsTextLineOut(0,sMessageBox[i].x + 43, sMessageBox[i].y + 17, sMessageBox[i].MessageDoc2, sMessageBox[i].len);
				}
				else
					dsTextLineOut(0,sMessageBox[i].x + 43, sMessageBox[i].y + 17, sMessageBox[i].MessageDoc, sMessageBox[i].len);
			}
		}
		...
		for (i = 0; i < 20; i++)
		{
			if (sMessageBox2[i].Flag)
			{
				dsTextLineOut(0,sMessageBox2[i].x + 45, sMessageBox2[i].y + 15, sMessageBox2[i].MessageDoc1, lstrlen(sMessageBox2[i].MessageDoc1));
				dsTextLineOut(0,sMessageBox2[i].x + 45, sMessageBox2[i].y + 40, sMessageBox2[i].MessageDoc2, lstrlen(sMessageBox2[i].MessageDoc2));
				...
				if (i == MESSAGE_MYSHOP_POTION)
				{
					...
					dsTextLineOut(0,sMessageBox2[i].TextSetX - (len2), sMessageBox2[i].y + 58, szBuff15, lstrlen(szBuff15));

					wsprintf(szBuff15, "%s %d", BuyPotionMoney, sMyShop_Server.Price*sMessageBox2[i].Count);
					len2 = lstrlen(szBuff15);
					dsTextLineOut(0,sMessageBox2[i].TextSetX - (len2), sMessageBox2[i].y + 72, szBuff15, lstrlen(szBuff15));
				...
				else
				{
					...
					dsTextLineOut(0,sMessageBox2[i].TextSetX, sMessageBox2[i].y + 72, szBuff5, lstrlen(szBuff5));
				}
			}
		}
		for (i = 0; i < MESSAGE3_MAX; i++)
		{
			if (sMessageBox3[i].Flag)
			{
				...
				SetFontTextColor(RGB(222, 231, 255));
				dsTextLineOut(0,sMessageBox3[i].x + TempLenght, sMessageBox3[i].y + 20, sMessageBox3[i].RecvMessage, lstrlen(sMessageBox3[i].RecvMessage));
				//SelectObject(hdc, sinFont);

				SetFontTextColor(RGB(255, 255, 255));
				TempLenght = (int)((172 - (lstrlen(sMessageBox3[i].MessageDoc1)*5.5)) / 2 - 5);
				dsTextLineOut(0,sMessageBox3[i].x + TempLenght, sMessageBox3[i].y + 35 + 8, sMessageBox3[i].MessageDoc1, lstrlen(sMessageBox3[i].MessageDoc1));
				if (i == MESSAGE_EVENT_SMASHTV)
				{
					wsprintf(szTemp2, "%d%s", sinChar->Level*SMASHTV_MONEY, sMessageBox3[i].MessageDoc2);
					dsTextLineOut(0,sMessageBox3[i].x + 48, sMessageBox3[i].y + 55 + 5, szTemp2, lstrlen(szTemp2));
				}
				else if (i == MESSAGE_SECRET_NUM)
				{
					...
					dsTextLineOut(0,sMessageBox3[i].x + 48, sMessageBox3[i].y + 55 + 5, sMessageBox3[i].MessageDoc2, lstrlen(sMessageBox3[i].MessageDoc2));

				}
				else
				{
					TempLenght = (int)((172 - (lstrlen(sMessageBox3[i].MessageDoc2)*5.5)) / 2 - 5);
					dsTextLineOut(0,sMessageBox3[i].x + TempLenght + 2, sMessageBox3[i].y + 55 + 5, sMessageBox3[i].MessageDoc2, lstrlen(sMessageBox3[i].MessageDoc2));
				}
			}
		}
...
int cMESSAGEBOX::CheckOkMessageBox(int Kind)
{
	...
	case  MESSAGE_MOVE_COIN_TRADE:
		...
		if (cCoinShop.Coin - sMessageBox2[Kind].Count < 0)
		{
			SendSetHackUser2(4100, 7);
			sMessageBox2[Kind].Count = cCoinShop.Coin;
