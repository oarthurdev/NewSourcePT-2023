...
char *HaEffectFilePath[] = {
	0,
	"Effect\\Particle\\defense.tga",
	"Effect\\Particle\\block.tga",
	"Effect\\Particle\\Evade.tga",
	0,
};
...
void cINTERFACE::Init()
{
	...
	MatChatDefault[0] = CreateTextureMaterial("game\\GUI\\Interface\\Chat\\ChatDefault-1.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	MatChatDefault[1] = CreateTextureMaterial("game\\GUI\\Interface\\Chat\\ChatDefault-2.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);

	MatChatSmall[0] = CreateTextureMaterial("game\\GUI\\Interface\\Chat\\ChatSmall-1.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
...
void cINTERFACE::Draw()
{
	...
	//Rubinho Posi豫o barra de exp pra cima
	...
	if (sinPKWarningExpFlag) {
		if (sinPKWarnigExpTime <= 15)
			DrawSprite(575, 592, lpBar_Exp, 0, 0, sinBmpLength[3], 5);
	...
	if(hFocusWnd && sinChatInputMode && !usehFocus)
	{
		...
		dsTextLineOut(0, x + 6, y, lpInput, len);
		SetFontTextColor( RGB(255, 255, 255));
		dsTextLineOut(0, x + 5, y - 1, lpInput, len);
	#else
		dsTextLineOut(0, x + 1, y + 1, lpInput, len);
		SetFontTextColor( RGB(255, 255, 255));
		dsTextLineOut(0, x, y, lpInput, len);
	#endif
...
#include <imm.h>
	...
	if(ImeChatLen != len || ImePositon.x != len || ImePositon.y != y)
	{
		...
		if (SETTINGSHANDLER->getHeight() == 600) addPoint = 0;
		else if (SETTINGSHANDLER->getHeight() == 768) addPoint = 163;
		else if (SETTINGSHANDLER->getHeight() == 1024) addPoint = 420;
	...
	if(SkillMasterFlag == 1)
	{
		...
		if(SkillMasterFlag == 1 && !ShowSkillUpInfo)
		{
			...
			dsTextLineOut(0, 100, 225 - 80, SkillUseInfoText[0], lstrlen(SkillUseInfoText[0]));
			...
			if(sinSkill.SkillPoint || sinSkill.SkillPoint4)
			{
				...
				dsTextLineOut(0, 100, 260 - 80, SkillUseInfoText[1], lstrlen(SkillUseInfoText[1]));
				dsTextLineOut(0, 100, 280 - 80, SkillUseInfoText[2], lstrlen(SkillUseInfoText[2]));
				wsprintf(strBuff, sinMsgSkillPoint, sinSkill.SkillPoint);
				//SelectObject(hdc, sinBoldFont);
				
				SetFontTextColor( RGB(247, 243, 193));
				dsTextLineOut(0, 100, 310 - 80, strBuff, lstrlen(strBuff));

				wsprintf(strBuff, sinMsgEliteSkillPoint, sinSkill.SkillPoint4);
				//SelectObject(hdc, sinBoldFont);
				
				SetFontTextColor( RGB(147, 243, 193));
				dsTextLineOut(0, 103, 310 - 55, strBuff, lstrlen(strBuff));
			}
			else
			{
				//SelectObject(hdc, sinFont);
				...
				dsTextLineOut(0, 100, 260 - 80, SkillUseInfoText[3], lstrlen(SkillUseInfoText[3]));
				dsTextLineOut(0, 100, 280 - 80, SkillUseInfoText[4], lstrlen(SkillUseInfoText[4]));
...
{
	ReStartMainXY.x = (SETTINGSHANDLER->getWidth() / 2) - (187 / 2);
	ReStartMainXY.y = (SETTINGSHANDLER->getHeight() / 2) - (127 / 2) - 100;

	ReStartTextXY.x = ReStartMainXY.x + 23;
	ReStartTextXY.y = ReStartMainXY.y + 73;

	SkillMasterMainXY.x = (SETTINGSHANDLER->getWidth() / 2) - (187 / 2) - 30;
	SkillMasterMainXY.y = (SETTINGSHANDLER->getHeight() / 2) - (127 / 2) - 100;
	...
	if(sinGetKeyClick(VK_RETURN))
	{
		...
		if(smConfig.getSecurity() >= AccountTypes::SEC_MODERATOR)
		{
			if(sinBuff[0])
			{
				...
				if(lstrcmp(sinTestBuff5, "/이방범전업") == 0 || lstrcmp(sinTestBuff5, "/Rank") == 0)
				{
				#ifdef	_WINMODE_DEBUG
...
void cINTERFACE::DrawInterText()
{
	...
	if(InfoMessageBoxPos.x != 0 || InfoMessageBoxPos.y != 0)
		dsTextLineOut(0, InfoMessageBoxPos.x, InfoMessageBoxPos.y, szInfoMsgBuff5, lstrlen(szInfoMsgBuff5));
	if(ShowExpPercentFlag)
	{
		SetFontTextColor( RGB(202, 254, 251));
		dsDrawOffsetArray = dsARRAY_BOTTOM;
		dsTextLineOut(0, 511, 551, szInfoMsgBuff6, lstrlen(szInfoMsgBuff6));
		dsDrawOffsetArray = dsARRAY_TOP;
	...
	if(SkillNpcFlag)
	{
		...
		{
			dsTextLineOut(0, SkillMasterTextXY.x, SkillMasterTextXY.y + 2, szSkillMasterMsg, lstrlen(szSkillMasterMsg));
		}
		if(szReStartMsg2[0])
		{
			dsTextLineOut(0, SkillMasterTextXY.x, SkillMasterTextXY.y + 18, szSkillMasterMsg2, lstrlen(szSkillMasterMsg2));
		}
...

{
	...
	smTransCommand.code = OpCode::OPCODE_DEF_INFO;
