...
void cSINSOD2::DrawText()
{
	...
	if(BoxIndex == 1)
	{
		...
		case 1:
			for(i = 0; i < 3; i++)
			{
				if(i == 2)
				{
					...
					dsTextLineOut(0, 97, 182 + (14 * i), szTempBuff, lstrlen(szTempBuff));
				}
				else
				{
					dsTextLineOut(0, 97, 182 + (14 * i), SodMessage_Etc[i], lstrlen(SodMessage_Etc[i]));
				}
			}

			while(j != sinClanMessage.size())
			{
				dsTextLineOut(0, 154, 281 + (j * 20), sinClanMessage[j].c_str(), lstrlen(sinClanMessage[j].c_str()));
				j++;
			}


			wsprintf(szTempBuff, "%s : %s", sinClanMaster7, szClanMaster);
			dsTextLineOut(0, 230, 355, szTempBuff, lstrlen(szTempBuff));


			//SelectObject(hdc, sinBoldFont);
			SetFontTextColor( RGB(255, 244, 201));
			dsTextLineOut(0, 185, 255, szClanName, lstrlen(szClanName));
		...
		case 2:
			for(i = 0; i < 2; i++)
			{
				dsTextLineOut(0, 97, 182 + (14 * i), SodMessage_Clan[i], lstrlen(SodMessage_Clan[i]));
			}

			memset(&szTempBuff, 0, sizeof(szTempBuff));
			NumLineComa(TotalEMoney, szTempBuff);
			dsTextLineOut(0, 170, 283, szTempBuff, lstrlen(szTempBuff));

			wsprintf(szTempBuff, "%d%s", Tax, "%");
			dsTextLineOut(0, 170, 301, szTempBuff, lstrlen(szTempBuff));

			memset(&szTempBuff, 0, sizeof(szTempBuff));
			NumLineComa(TotalMoney, szTempBuff);
			dsTextLineOut(0, 170, 320, szTempBuff, lstrlen(szTempBuff));

			break;
		case 3:
			for(i = 0; i < 3; i++)
			{
				dsTextLineOut(0, 97, 182 + (14 * i), SodMessage_Master[i], lstrlen(SodMessage_Master[i]));
			}
			memset(&szTempBuff, 0, sizeof(szTempBuff));
			NumLineComa(TotalEMoney, szTempBuff);
			dsTextLineOut(0, 170, 283, szTempBuff, lstrlen(szTempBuff));
		...
		case 6:
			...
			//SelectObject(hdc, sinBoldFont);
			SetFontTextColor( RGB(255, 244, 201));
			dsTextLineOut(0, 185, 255, cldata.name, lstrlen(cldata.name));

			//SelectObject(hdc, sinFont);
			wsprintf(szTempBuff, "%s : ", sinPrize7);
			dsTextLineOut(0, 110, 283, szTempBuff, lstrlen(szTempBuff));

			memset(&szTempBuff, 0, sizeof(szTempBuff));
			NumLineComa(ClanMoney, szTempBuff);
			dsTextLineOut(0, 152, 283, szTempBuff, lstrlen(szTempBuff));

			dsTextLineOut(0, 110, 301, OtherClanMaster[0], lstrlen(OtherClanMaster[0]));
			dsTextLineOut(0, 110, 320, OtherClanMaster[1], lstrlen(OtherClanMaster[1]));
			break;
		}

		//SelectObject(hdc, sinBoldFont);
		SetFontTextColor( RGB(255, 205, 4));
		dsTextLineOut(0, 223, 157, szClanName, lstrlen(szClanName));
	}
	char szTempNum[16];
	int  TempNum;
	...
	if(BoxIndex == 2)
	{
		while(j != sinClanRank.size())
		{
			if((j % 4) != 0)
			{
				...
				if(k + 1 == 10)
				{
					dsTextLineOut(0, 82, 177 + (k * 20), szTempNum, lstrlen(szTempNum));
				}
				else
				{
					dsTextLineOut(0, 86, 177 + (k * 20), szTempNum, lstrlen(szTempNum));
				}
				//SelectObject(hdc, sinFont);
				...
				if((j % 4) == 2)
				{
					...
					dsTextLineOut(0, 5 + Posi[j % 4], 177 + (k * 20), szTempBuff2, lstrlen(szTempBuff2));

				}
				else
				{
					dsTextLineOut(0, 5 + Posi[j % 4], 177 + (k * 20), sinClanRank[j].c_str(), lstrlen(sinClanRank[j].c_str()));
				}
			}
			j++;
			if((j % 4) == 0)
...
void cHASIEGE::DrawText()
{
	...
	if(haSiegeMenuFlag)
	{
		...
		case HASIEGE_TAXRATES:
			...
			dsTextLineOut(0, 97, 112, szTempBuff, lstrlen(szTempBuff));

			wsprintf(szTempBuff, SiegeMessage_Taxrates[0], HaTaxRate, "%");
			dsTextLineOut(0, 97, 182, szTempBuff, lstrlen(szTempBuff));
			...
			dsTextLineOut(0, 247 - ((Taxlen - 8) * 8), 260, szTempBuff, lstrlen(szTempBuff));


			//SelectObject(hdc, sinFont);
			SetFontTextColor( RGB(255, 255, 255));
			wsprintf(szTempBuff, SiegeMessage_Taxrates[1]);
			dsTextLineOut(0, 97, 244, szTempBuff, lstrlen(szTempBuff));

			SetFontTextColor( RGB(255, 255, 255));
			wsprintf(szTempBuff, SiegeMessage_Taxrates[3], HaTestMoney);
			dsTextLineOut(0, 97, 310, szTempBuff, lstrlen(szTempBuff));
		...
		case HASIEGE_DEFENSE:
			...
			if(TowerIconIndex > 3)
			{
				...
				dsTextLineOut(0, ClanSkillBoxPosi.x + LineSetX + LinePosX, ClanSkillBoxPosi.y + 20, szTempBuff, lstrlen(szTempBuff));
				...
				for(j = 0; j < cnt1 + 1; j++)
				{
					...
					LinePosX = (ClanSkillBoxSize.x * 16 - (Linelen * 8)) / 2;
					dsTextLineOut(0, ClanSkillBoxPosi.x + LineSetX + LinePosX, ClanSkillBoxPosi.y + 40 + j * 18, szTempBuff, lstrlen(szTempBuff));
				}
			}
			...
			if(TowerIconIndex > 0)
			{
				...
				for(i = 0; i < 5; i++)
				{
					...
					{
					case 0:
						dsTextLineOut(0, ClanSkillBoxPosi.x + LineSetX + LinePosX, ClanSkillBoxPosi.y + 14, szTempBuff, lstrlen(szTempBuff));
						break;
					case 1:
					case 2:
					case 3:
						...
						{
							dsTextLineOut(0, ClanSkillBoxPosi.x + LineSetX + LinePosX, ClanSkillBoxPosi.y + 20 + i * 18, szTempBuff, lstrlen(szTempBuff));
						}
		...
		for(int i = 0; i < 5; i++)
		{
			if(sHaClanData[i].Flag)
			{
				if(sHaClanData[i].ClanInfoNum >= 0)
				{
					...
					dsTextLineOut(0, WinSizeX / 2 + 40, 32 + i * 17, szTempBuff, lstrlen(szTempBuff));
				}
			}
		}
	}
	...
	if(haMovieFlag)
	{
		//SelectObject(hdc, sinBoldFont);
		SetFontTextColor( RGB(255, 255, 255));
		dsTextLineOut(0, 152, 80, haMovieName, lstrlen(haMovieName));
	}
