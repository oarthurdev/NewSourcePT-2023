...
{
	{SKILL_A_MIGAL,SKILL_A_MIDRANDA},
	{SKILL_R_MAKER},
	{SKILL_R_KNIGHT},
	{SKILL_ALAS},
	{SKILL_BLIND},
...
#define  CHAR_SKILL_MASK 0xFF000000
	...
	if(lpSkill->CODE == SKILL_SHIELD_STRIKE || lpSkill->CODE == SKILL_EXTREME_SHIELD || lpSkill->CODE == SKILL_DIVINE_INHALATION || lpSkill->CODE == SKILL_SPARK_SHIELD || lpSkill->CODE == SKILL_GODLY_SHIELD || lpSkill->CODE == SKILL_SOUL_SUCKER)
	{
		if(lpSkill->UseSkillFlag)
		{
...
int sinContinueSkillSet(sSKILL *lpSkill)
{
	...
	for(i = 0; i < MAX_CONTINUE_SKILL; i++)
	{
		if(!ContinueSkill[i].Flag)
		{
			...
			if((lpSkill->CODE != SKILL_FORCE_ORB && lpSkill->CODE != PLAYER_VIP && lpSkill->CODE != DELAY_GLOBAL && lpSkill->CODE != QUEST_DIARY && lpSkill->CODE != BATTLE_LEVEL && /*lpSkill->CODE != PLAYER_ARENA &&*/ lpSkill->CODE != CHANGE_JOB3_QUEST &&lpSkill->CODE != CHANGE_ELEMENTARY_QUEST &&
