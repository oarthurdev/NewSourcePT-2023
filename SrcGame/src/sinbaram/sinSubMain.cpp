...
float mana_regen_true = 0;
float life_regen_true = 0;
float stam_regen_true = 0;
float	real_mana = 0;
float	real_life = 0;
float	real_stm = 0;
int hackdetectado = false;
...
void MainSub()
{
	...
	if (InCreaMANA > real_mana || InCreaLIFE > real_life || InCreaSTM > real_stm && !hackdetectado) {
		hackdetectado = true;
...
void InitCursor()
{
	hCursor[0] = LoadCursorFromFile("game\\mouse\\default.cur");
	hCursor[1] = LoadCursorFromFile("game\\mouse\\sell.cur");
	hCursor[2] = LoadCursorFromFile("game\\mouse\\buy.cur");
	hCursor[3] = LoadCursorFromFile("game\\mouse\\repair.cur");
	hCursor[4] = LoadCursorFromFile("game\\mouse\\attack.cur");
	hCursor[5] = LoadCursorFromFile("game\\mouse\\dropsee.cur");
	hCursor[6] = LoadCursorFromFile("game\\mouse\\droppick.cur");
	hCursor[7] = LoadCursorFromFile("game\\mouse\\talk.cur");
}
...
void sinRegen()
{
	...
	if (lpCurPlayer->MotionInfo->State == CHRMOTION_STATE_STAND)
	{
		...
		case 1:
			InCreaSTM = (float)(3.8 + sinChar->Level / 7 + sinChar->Stamina_Regen);
			real_stm = (float)(3.8 + sinChar->Level / 7 + stam_regen_true);
			break;
		case 2:
			InCreaSTM = (float)(3.3 + sinChar->Level / 7 + sinChar->Stamina_Regen);
			real_stm = (float)(3.3 + sinChar->Level / 7 + stam_regen_true);
			break;
		case 3:
			InCreaSTM = (float)(2.9 + sinChar->Level / 7 + sinChar->Stamina_Regen);
			real_stm = (float)(2.9 + sinChar->Level / 7 + stam_regen_true);
			break;
		default:
			InCreaSTM = (float)(3.8 + sinChar->Level / 7 + sinChar->Stamina_Regen);
			real_stm = (float)(2.9 + sinChar->Level / 7 + stam_regen_true);
	...
	if (lpCurPlayer->MotionInfo->State == CHRMOTION_STATE_WALK)
	{
		...
		case 1:
			InCreaSTM = (float)((3.8 + sinChar->Level / 7 + sinChar->Stamina_Regen) * 0.6);
			real_stm = (float)((3.8 + sinChar->Level / 7 + stam_regen_true) * 0.6);
			break;
		case 2:
			InCreaSTM = (float)((3.3 + sinChar->Level / 7 + sinChar->Stamina_Regen) * 0.6);
			real_stm = (float)((3.8 + sinChar->Level / 7 + stam_regen_true) * 0.6);
	...
	if (lpCurPlayer->MotionInfo->State != CHRMOTION_STATE_DEAD)
	{
		...
		InCreaLIFE = (float)(((fLevel + (fStrength / 2) + fHealth) / 180 + sinChar->Life_Regen) / 1.5);
		real_life = (float)(((fLevel + (fStrength / 2) + fHealth) / 180 + life_regen_true) / 1.5) + dif;

		InCreaMANA = (fLevel + (fSpirit * 1.2f) + (fHealth / 2)) / 115 + sinChar->Mana_Regen;
		real_mana = (fLevel + (fSpirit * 1.2f) + (fHealth / 2)) / 115 + (mana_regen_true)+dif;
...
int sinGetAccuracy(int desLV, int desDefense)
{
	...
	if (Result < 30) Result = 30; //antigo era 30
	if(Result > 95) Result = 95;
	return Result;
}

extern bool defenseinfo;

////int GetPlayDefence(smCHAR_INFO *lpAttackCharInfo, smCHAR_INFO *lpDefenceCharInfo)
//int GetPlayDefence(int ar, int aLv, int def, int dLv)
//{
//	int defence;
//
...
//	if (defenseinfo)
//	{
//		//wsprintf(buff, "AttackRating: %d | Defence: %d", lpAttackCharInfo->Attack_Rating, lpDefenceCharInfo->Defence);
//		wsprintf(buff, "AttackRating: %d | Defence: %d", ar, def);
//		AddChatBuff(buff);
//	}
//
//	float def_rtg = def - ar;
//
//	defence = (float)(def_rtg / def) * 100.f;
//
//	if (defenseinfo)
//	{
//		wsprintf(buff, "Chance: %d", defence);
//		AddChatBuff(buff);
//	}
//
//	if (defence > 80) defence = 80;
//	if (defence < 10) defence = 10;
//
//	if (defenseinfo)
//	{
//		wsprintf(buff, "TrueChance: %d", defence);
//		AddChatBuff(buff);
//	}
//
//	return defence;
...
//	if (defenseinfo)
//	{
//		wsprintf(buff, "Received: %d | %d", MonsterLV, MonsterAttack_Rating);
//		AddChatBuff(buff);
//	}
//
//	while(1)
...
//	if (defenseinfo)
//	{
//		wsprintf(buff, "TrueResult: %d", Result);
//		AddChatBuff(buff);
//	}
//
//	return Result;*/
...
//RUBINHO CORRE豫o MS SKILL EXTREMO DO BLOCK
...
int StopCharMotion(int x, int y)
{
	...
	if (Battle::GetInstance()->IsOpen() || RankGame::GetInstance()->IsOpen())
		return TRUE;
	...
	if(smConfig.getSecurity() >= AccountTypes::SEC_MODERATOR)
	{
		if(sinSecretFlag)
		{
			if(CheatHelpOpenFlag)
			{
				...
				dsTextLineOut(0, 100, 200, strBuff, lstrlen(strBuff));

				wsprintf(strBuff, "(L) 돈 200000 ");
				dsTextLineOut(0, 100, 230, strBuff, lstrlen(strBuff));

				wsprintf(strBuff, "(B) LV 65 돈 300000");
				dsTextLineOut(0, 100, 260, strBuff, lstrlen(strBuff));

				wsprintf(strBuff, "(Y) 믹스쳐 아이템 실행 ");
				dsTextLineOut(0, 100, 290, strBuff, lstrlen(strBuff));

				wsprintf(strBuff, "(U) 전업  ");
				dsTextLineOut(0, 100, 320, strBuff, lstrlen(strBuff));

				wsprintf(strBuff, "(F) 숙련도 Up  ");
				dsTextLineOut(0, 100, 350, strBuff, lstrlen(strBuff));

				wsprintf(strBuff, "(N) 숙련도 Down  ");
				dsTextLineOut(0, 100, 380, strBuff, lstrlen(strBuff));
			}
		}
	}
...
int sinSetAgingItemIncreState(sITEMINFO *pItem)
{
	...
	switch(pItem->CODE & sinITEM_MASK2)
	{
		...
		break;
		case sinDA1: case sinDA2: case sinDA3: case sinDA4:
...
int RecvWingItem(sCRAFTITEM_SERVER *pWingItem)
{
	...
	for(i = 0; i < INVENTORY_MAXITEM * 2; i++)
	{
		...
		{
			if (pItem->ItemPosition == ITEM_CLASS_WING)
				continue;
		}
