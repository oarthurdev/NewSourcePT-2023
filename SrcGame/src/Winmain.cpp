...
//desabilitar borda - by bruno sobral
...
#ifdef CHRONOS_DEV
#include "def.h"
#include "Chronos/Engine/Components/UIImage.h"
#include "Chronos/Engine/Components/UIImageLoader.h"
#include "Chronos/Engine/Components/UIImageRender.h"
#include "Chronos/Engine/DirectX/DXGraphicEngine.h"
#include "Chronos/Engine/DirectX/DXFont.h"
#include "Chronos/Engine/DirectX/DXSelectGlow.h"
...
void ReadTexSettings()
{
	...
	imgPVP = CreateTextureMaterial("game\\GUI\\PVP\\bp_system.tga", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	iSettingsButton[0] = CreateTextureMaterial("game\\GUI\\Settings\\settings.png", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	iSettingsButton[1] = CreateTextureMaterial("game\\GUI\\Settings\\settings_.png", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	...
	LifeDamage = CreateTextureMaterial("game\\Images\\UI\\life.png", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
	Dead = CreateTextureMaterial("game\\Images\\UI\\Dead.png", 0, 0, 0, 0, SMMAT_BLEND_ALPHA);
}
...
BOOL CALLBACK listarJanelas(HWND hwnd, LPARAM lParam) {
...
INT WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPreInst, LPSTR lpCmdLine, INT nCmdShow)
{
...
#ifndef _W_SERVER
	const int MAX_WINDOW = 2;
	const char* appName = szAppName;
	int counter = 0;
	std::vector<std::string> titles;
	EnumWindows(listarJanelas, reinterpret_cast<LPARAM>(&titles));
	// At this point, titles if fully populated and could be displayed, e.g.:
	for (const auto& title : titles) {
		if (title.compare(appName) == 0)
		{
			counter++;
		}
		...
		{
			MessageBox(NULL, "Maximo de janelas atingido.", "Erro", 0x30);
			return FALSE;
		}
	}
#endif
	...
	/*
	if(strcmp(lpCmdLine, "Rubinho_FPT") != 0)
	{
		STARTUPINFO si;
	...
	*/
	...
	WinSizeX = SETTINGSHANDLER->getWidth();
	WinSizeY = SETTINGSHANDLER->getHeight();
	WinColBit = SETTINGSHANDLER->getBitDepth();
	...
	ShowWindow(hwnd, SETTINGSHANDLER->getStartMaximized() ? SW_MAXIMIZE : nCmdShow);
	UpdateWindow(hwnd);
	...
	{
		Logger::getInstance()->output(LogType::Error, "Failed InitD3D()");
		return FALSE;
	}
...
LONG APIENTRY WndProc(HWND hWnd, UINT messg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE:
		...
		if (wParam == SIZE_MAXIMIZED && SETTINGSHANDLER->getAutoAdjust())
		{
			...
			SETTINGSHANDLER->setWidth(WinSizeX);
			SETTINGSHANDLER->setHeight(WinSizeY);
			...
			if (GRAPHICENGINE)
			{
				GRAPHICENGINE->Reset(WinSizeX, WinSizeY);
			}
			BattleEventWindow->Resize();
		}
		else
		{
	...
	case WM_SOCKETCLOSE:
		SOCKETGAME->SocketClose((SocketData*)wParam);
		break;
	...
	case WM_MOUSEWHEEL:
		...
		if (cSettings.isOpen()) {
			cSettings.OnMouseScroll(zDelta);
		}
		/*if (cQuestWindow.OpenFlag)
		{
			cQuestWindow.MouseWheel(zDelta);
		}*/
		if (isChatBox(pRealCursorPos.x, pRealCursorPos.y))
		{
			if (zDelta >= 0)
			{
		...
		else if (g_IsDxProjectZoomIn <= 0 && !hFocusWnd || g_iFzmCursorFocusGame && !hFocusWnd)
		{
			//zDelta = (short)HIWORD(wParam);
	...
	case WM_IME_COMPOSITION:
		...
		cSettings.OnKeyDown((char)wParam);
	...
	case WM_DESTROY:
		...
		/*case WM_ACTIVATE:
			...
			//case WM_NCRBUTTONDOWN:
			//	break;
			//	//Removendo bug janela Bless (ajustar para botoes de minimizar, maxmizar e sair)
			//case WM_NCLBUTTONDOWN:
			//	CancelAttack(); // Cancelando ataque
			//	mouse_right_click = false;
			//	break;
			//
	...
	case WM_ACTIVATE:
		...
		// Disable right button on title bar to prevent bugs 
	//case WM_NCRBUTTONDOWN:
		//break;
	...
	else if (cSinHelp.sinGetHelpShowFlag() != TRUE)
	{
		...
		else
		{
			...
			if (VRKeyBuff[VK_DOWN])
			{
				...
				if (SETTINGSHANDLER->isWideScreen())
				{
					distance = 600;
				}
...
void showFPS()
{
	...
	std::string ping = "Ping: " + std::to_string(PingDiff);
	...
	dsTextLineOut(hdc, 10, 50, frames.c_str(), frames.length());

	SetFontTextColor(RGB(55, 214, 0));
	dsTextLineOut(hdc, 10, 65, ping.c_str(), ping.length());
}
...
int InitD3D(HWND hWnd)
{
	Logger::getInstance()->output(LogType::Debug, "InitD3D() - WinSizeX(%d)   WinSizeY(%d)   WinColBit(%d)", WinSizeX, WinSizeY, WinColBit);
	hwnd = hWnd;
	...
	{
		Logger::getInstance()->output(LogType::Debug, "CreateD3D() failed");
		return FALSE;
	}
	...
	smSetTextureLevel(SETTINGSHANDLER->getTextureQuality());
...
int GameInit()
{
	...
	viewdistZ = (SETTINGSHANDLER->getHeight() * 4) / 3;
...
int SetGameMode(int mode)
{
	...
	{
	case 1:
		SetDxProjection((PI / 4.4f), SETTINGSHANDLER->getWidth(), SETTINGSHANDLER->getHeight(), 20.f, 4000.f);


		smRender.CreateRenderBuff(SETTINGSHANDLER->getSight() ? 1 : 0);
...
int DrawGameState()
{
	...
	if (VRKeyBuff['A']) // drop no chão
	{
		for (cnt = 0; cnt < DISP_ITEM_MAX; cnt++)
		{
			if ((scItems[cnt].Flag && scItems[cnt].ItemCode == (sinOR2 | sin05)) ||
				(scItems[cnt].Flag && scItems[cnt].ItemCode == (sinOA1 | sin30)) ||
				(scItems[cnt].Flag && scItems[cnt].ItemCode == (sinDB1 | sin30)) ||
				(scItems[cnt].Flag && scItems[cnt].ItemCode == (sinDB1 | sin35)) ||
				(scItems[cnt].Flag && scItems[cnt].ItemCode == (sinDG1 | sin30)) ||
				(scItems[cnt].Flag && scItems[cnt].ItemCode == (sinOR1 | sin26)) ||
				(scItems[cnt].Flag && scItems[cnt].ItemCode == (sinOA1 | sin26)))
			{
				if (scItems[cnt].RendPoint.z >= 32 * fONE && scItems[cnt].RendPoint.z < 64 * 12 * fONE)
				{
					DrawCharacterMessage(scItems[cnt].RendPoint.x, scItems[cnt].RendPoint.y, scItems[cnt].szName, 0, 0, 0, RGB(173, 51, 243));
				}

			}

			else if (scItems[cnt].Flag && scItems[cnt].ItemCode == (sinGG1 | sin01))
			{
				if (scItems[cnt].RendPoint.z >= 32 * fONE && scItems[cnt].RendPoint.z < 64 * 12 * fONE)
				{
					DrawCharacterMessage(scItems[cnt].RendPoint.x, scItems[cnt].RendPoint.y, scItems[cnt].szName, 0, 0, 0, RGB(250, 235, 65));
				}
			}

			else if (scItems[cnt].Flag && scItems[cnt].ItemCode != 0 && lpSelItem != &scItems[cnt])
			{
				if (scItems[cnt].RendPoint.z >= 32 * fONE && scItems[cnt].RendPoint.z < 64 * 12 * fONE)
				{
					DrawCharacterMessage(scItems[cnt].RendPoint.x, scItems[cnt].RendPoint.y, scItems[cnt].szName, 0, 0, 0, RGB(235, 235, 235));
				}
			}
		}
	}
	...
	//Sumir com HP do boss - by LicanZero
	int BossCnt = 0;
	...
	int Hp2 = lpCurPlayer->smCharInfo.Life[1] - sinGetLife();
	int Hp3 = lpCurPlayer->smCharInfo.Life[1] / 10;
	if (StageField[lpCurPlayer->OnStageField]->State != FIELD_STATE_VILLAGE) {
		if (Hp2 < Hp3 * 4 && Hp2 >= Hp3 * 3) {
			dsDrawTexImage(LifeDamage, 0, 0, smScreenWidth, smScreenHeight, 100);
		}
		else if (Hp2 < Hp3 * 5 && Hp2 >= Hp3 * 4) {
			dsDrawTexImage(LifeDamage, 0, 0, smScreenWidth, smScreenHeight, 125);
		}
		else if (Hp2 < Hp3 * 6 && Hp2 >= Hp3 * 5) {
			dsDrawTexImage(LifeDamage, 0, 0, smScreenWidth, smScreenHeight, 150);
		}
		else if (Hp2 < Hp3 * 7 && Hp2 >= Hp3 * 6) {
			dsDrawTexImage(LifeDamage, 0, 0, smScreenWidth, smScreenHeight, 175);
		}
		else if (Hp2 < Hp3 * 8 && Hp2 >= Hp3 * 7) {
			dsDrawTexImage(LifeDamage, 0, 0, smScreenWidth, smScreenHeight, 200);
		}
		else if (Hp2 < Hp3 * 9 && Hp2 >= Hp3 * 8) {
			dsDrawTexImage(LifeDamage, 0, 0, smScreenWidth, smScreenHeight, 225);
		}
		else if (Hp2 >= Hp3 * 9 && lpCurPlayer->MotionInfo->State != CHRMOTION_STATE_DEAD) {
			dsDrawTexImage(LifeDamage, 0, 0, smScreenWidth, smScreenHeight, 250);
		}

		if (lpCurPlayer->MotionInfo->State == CHRMOTION_STATE_DEAD) {
			dsDrawTexImage(Dead, 0, 0, smScreenWidth, smScreenHeight, 165);
	...
	for (cnt = 0; cnt < ChatMsgSortCnt; cnt++)
	{
		...
		DWORD Color = RGB(150, 255, 200);
		if (lpCharMsgSort[cnt]->PlayerPvPMode)
			dwColor = RGB(255, 0, 0);
		if (lpCharMsgSort[cnt]->PlayerPvPMode)
		{

			dwColorPvP = RGB(255, 0, 0);
		}
		else
			dwColorPvP = RGB(255, 255, 255);
		...
		else
		{
			...
			else
			{
				...
				int iTopRankerJobCode = -1;

				if (lpCharMsgSort[cnt]->dwTopRankUserEffectTime)
				...
				if (szClanName)
				{
					...
					{
						// Char eh Top Ranking + clan
						if (iTopRankerJobCode != -1)
						{
							std::string sRankTitle = Midranda::HighLevelUserMerit::GetTopRankerTitle(iTopRankerJobCode);
							int iRankerTitleLen = sRankTitle.length();
							strcpy_s(message1, sizeof(message1), clanName);
							strcpy_s(message2, sizeof(message2), sRankTitle.c_str());
							strcpy_s(message3, sizeof(message3), pvpKing);
							strcpy_s(message4, sizeof(message4), lpCharMsgSort[cnt]->smCharInfo.szName);

							DrawFourLineMessage(
								lpCharMsgSort[cnt]->RendPoint.x,
								lpCharMsgSort[cnt]->RendRect2D.top + y,
								message1, // Top Level RGB(234, 255, 0)
								message2, // Rei do PVP RGB(150, 255, 200)
								message3, // Nome do Clan RGB(255, 51, 51),
								message4, // Nome do personagem dwColor
								clanColor, // Cor Top Level
								topLevelColor, // Cor Rei do PVP
						...
						else // Char eh rei do pvp, mas nao eh top raking
						{
							strcpy_s(message1, sizeof(message1), clanName);
					...
					else if (lpCharMsgSort[cnt]->dwTopRankUserEffectTime) // Char eh top ranking
					{
						...
						DrawThreeLineMessage(
							...
							message3, // Nome do personagem dwColor
							clanColor, // Cor Top Level
							topLevelColor, // Cor Nome do Clan
							dwColor, // Cor Nome do personagem
							lpDDS_clanMark,
							selectedBox,
							dwClanMgrBit);
					}
					else // Char nao eh top ranking nem rei do pvp
					{
						strcpy_s(message1, sizeof(message1), clanName);
						...
						DrawTwoLineMessage(
							lpCharMsgSort[cnt]->RendPoint.x,
							lpCharMsgSort[cnt]->RendRect2D.top + y,
							message1, // Nome do Clan RGB(255, 51, 51),
							message2, // Nome do personagem dwColor
							clanColor, // Cor Nome do Clan
							dwColor, // Cor Nome do personagem
							lpDDS_clanMark,
							selectedBox,
							dwClanMgrBit);
				...
				else
				{
					...
					if (lpCharMsgSort[cnt]->PlayCursePvP)
					{
						...
						if (iTopRankerJobCode != -1)
						{
							std::string sRankTitle = Midranda::HighLevelUserMerit::GetTopRankerTitle(iTopRankerJobCode);
							int iRankerTitleLen = sRankTitle.length();
							strcpy_s(message1, sizeof(message1), sRankTitle.c_str());
							strcpy_s(message2, sizeof(message2), pvpKing);
							strcpy_s(message3, sizeof(message3), lpCharMsgSort[cnt]->smCharInfo.szName);
		...
		if (DispBar || SETTINGSHANDLER->getHpBar() && lpCharMsgSort[cnt]->smCharInfo.State != smCHAR_STATE_NPC &&
			lpCharMsgSort[cnt]->smCharInfo.State == smCHAR_STATE_ENEMY)
		{
	...
	if (DisplayIME)
	{
		...
		if (ImeStr[0])
			...
			dsTextLineOut(0, 8, ImeY - 12, ImeStr, lstrlen(ImeStr));
	...
	if (DisplayDebug)
	{
		...
		wsprintf(strBuff, "%d(%d) %d x=%d z=%d y=%d VRAM=%d R1=%d(%d) R2=%d(%d) S1=%d S2=%d (%d/%d) (%d/%d) RcvTurb( %d )", DispRender, DispMainLoop, DispPolyCnt, lpCurPlayer->pX >> (FLOATNS), lpCurPlayer->pZ >> (FLOATNS), lpCurPlayer->pY >> (FLOATNS), (VramTotal / (1024 * 1024)),
			rcv1, Debug_RecvCount1, rcv2, Debug_RecvCount2, snd1, snd2, brcv1, arcv1, brcv2, arcv2, 0);

		SetFontTextColor(RGB(0, 0, 0));
		dsTextLineOut(0, 11, 11, strBuff, lstrlen(strBuff));
		SetFontTextColor(RGB(255, 255, 255));
		dsTextLineOut(0, 10, 10, strBuff, lstrlen(strBuff));

		wsprintf(strBuff, "Stage : %s , %s", szGameStageName[0], szGameStageName[1]);
		SetFontTextColor(RGB(0, 0, 0));
		dsTextLineOut(0, 11, 31, strBuff, lstrlen(strBuff));
		SetFontTextColor(RGB(255, 255, 255));
		dsTextLineOut(0, 10, 30, strBuff, lstrlen(strBuff));

		wsprintf(strBuff, "World Time (%d:%d)", dwGameHour, dwGameMin);
		SetFontTextColor(RGB(0, 0, 0));
		dsTextLineOut(0, (WinSizeX >> 1) + 1, 31, strBuff, lstrlen(strBuff));
		SetFontTextColor(RGB(255, 255, 255));
		dsTextLineOut(0, (WinSizeX >> 1), 30, strBuff, lstrlen(strBuff));
		...
		dsTextLineOut(0, (g_iFzmCursorPosX - 65) + cursorX, (g_iFzmCursorPosY - 10) + cursorY, strBuff, lstrlen(strBuff));
		...
		if (lpCurPlayer->PatLoading == FALSE)
		{
			...
			SetFontTextColor(RGB(0, 0, 0));
			dsTextLineOut(0, (WinSizeX >> 1) + 1 + 120, 31, strBuff, lstrlen(strBuff));
			SetFontTextColor(RGB(255, 255, 255));
			dsTextLineOut(0, (WinSizeX >> 1) + 120, 30, strBuff, lstrlen(strBuff));

			wsprintf(strBuff, "Damage : %d", LastAttackDamage);
			SetFontTextColor(RGB(0, 0, 0));
			dsTextLineOut(0, 9, 48, strBuff, lstrlen(strBuff));
			SetFontTextColor(RGB(255, 255, 255));
			dsTextLineOut(0, 8, 47, strBuff, lstrlen(strBuff));
			...
			SetFontTextColor(RGB(0, 0, 0));
			dsTextLineOut(0, 9, 62, strBuff, lstrlen(strBuff));
			SetFontTextColor(RGB(255, 255, 255));
			dsTextLineOut(0, 8, 61, strBuff, lstrlen(strBuff));

			if (AreaServerMode)
			{
				if (lpWSockServer_Area[0])
				{
					wsprintf(strBuff, "Area Server[0]: (%s)", lpWSockServer_Area[0]->szIP);
					SetFontTextColor(RGB(0, 0, 0));
					dsTextLineOut(0, 9, 82, strBuff, lstrlen(strBuff));
					SetFontTextColor(RGB(255, 255, 255));
					dsTextLineOut(0, 8, 81, strBuff, lstrlen(strBuff));
				}
				if (lpWSockServer_Area[1])
				{
					wsprintf(strBuff, "Area Server[1]: (%s)", lpWSockServer_Area[1]->szIP);
					SetFontTextColor(RGB(0, 0, 0));
					dsTextLineOut(0, 9, 96, strBuff, lstrlen(strBuff));
					SetFontTextColor(RGB(255, 255, 255));
					dsTextLineOut(0, 8, 95, strBuff, lstrlen(strBuff));
				}

				if (lpWSockServer_DispArea[0])
				{
					wsprintf(strBuff, "Area Server[0]: (%s)", lpWSockServer_DispArea[0]->szIP);
					SetFontTextColor(RGB(0, 0, 0));
					dsTextLineOut(0, 209, 82, strBuff, lstrlen(strBuff));
					SetFontTextColor(RGB(255, 255, 255));
					dsTextLineOut(0, 208, 81, strBuff, lstrlen(strBuff));
				}

				if (lpWSockServer_DispArea[1])
				{
					wsprintf(strBuff, "Area Server[1]: (%s)", lpWSockServer_DispArea[1]->szIP);
					SetFontTextColor(RGB(0, 0, 0));
					dsTextLineOut(0, 209, 96, strBuff, lstrlen(strBuff));
					SetFontTextColor(RGB(255, 255, 255));
					dsTextLineOut(0, 208, 95, strBuff, lstrlen(strBuff));
				}
				...
				SetFontTextColor(RGB(0, 0, 0));
				dsTextLineOut(0, 9, 108, strBuff, lstrlen(strBuff));
				SetFontTextColor(RGB(255, 255, 255));
				dsTextLineOut(0, 8, 108, strBuff, lstrlen(strBuff));
			}
		}
		...
		for (i = 0; i < OTHER_PLAYER_MAX; i++)
		{
			if (chrOtherPlayer[i].Flag && chrOtherPlayer[i].Pattern && chrOtherPlayer[i].smCharInfo.State == smCHAR_STATE_USER)
			{
				...
				SetFontTextColor(RGB(0, 0, 0));
				dsTextLineOut(0, WinSizeX - 220, LineY + 1, strBuff, lstrlen(strBuff));
				SetFontTextColor(RGB(255, 255, 255));
				dsTextLineOut(0, WinSizeX - 221, LineY, strBuff, lstrlen(strBuff));
				LineY += 16;
				if (LineY > 460)
					break;
			}
		...
		for (i = 0; i < MonCharCnt; i++)
		{
			if (MonCharBuff[i] >= 0)
			{
				...
				SetFontTextColor(RGB(0, 0, 0));
				dsTextLineOut(0, WinSizeX - 380, LineY + 1, strBuff, lstrlen(strBuff));
				...
				dsTextLineOut(0, WinSizeX - 381, LineY, strBuff, lstrlen(strBuff));
				LineY += 16;
				if (LineY > 460) break;
			}
		}
...
#endif
	if (dwGameHour == 4 && dwGameMin == 0 && hrSod == FALSE && StageField[lpCurPlayer->OnStageField]->FieldCode == 9) {
		AddChatBuff("> A entrada para S.O.D está aberta!", 3);
		hrSod = TRUE;
	}
	else if (dwGameHour == 6 && dwGameMin == 0 && hrSod == TRUE && StageField[lpCurPlayer->OnStageField]->FieldCode == 9) {
		AddChatBuff("> A entrada para S.O.D está Fechada!", 3);
		hrSod = FALSE;
	}
...
int SetCreateFont()
{
	...
	hFont = CreateFontA(16, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, NULL, NULL, ANTIALIASED_QUALITY, FF_DONTCARE, dwFontGame);
	return TRUE;
}
