...
public:
	...
	BOOL Other_Player_Rei_PVP_Effect = false;
	...
	int RecvPlayData(SocketData* pcSocketData, char* psPacket);

	int SendPlayData(smCHAR *player);



	int SendCharInfo(SocketData *lpsmsock);


	int SendPlayDataSock(SocketData *lpsmsock, char *lpTransBuff, int ex, int ey, int ez);


	int SendTransAttack(smCHAR *lpChar, SocketData *lpsmsock, int AttackCode, int Add_Damage = 0, int Resistance = 0);
