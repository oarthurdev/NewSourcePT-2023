...
static void DrawEventScore(void)
{
	...
	if(g_EventScore.EntryTeamNum > 0)
	{
		...
		dsTextLineOut(0, int(490 * g_fWinSizeRatio_X), int(320 * g_fWinSizeRatio_Y), szBuff, lstrlen(szBuff));
		dsTextLineOut(0, int(490 * g_fWinSizeRatio_X), int(350 * g_fWinSizeRatio_Y), g_lpBellatraResult_Text3, lstrlen(g_lpBellatraResult_Text3));
		dsTextLineOut(0, int(490 * g_fWinSizeRatio_X), int(380 * g_fWinSizeRatio_Y), g_lpBellatraResult_Text4, lstrlen(g_lpBellatraResult_Text4));
	}
...
void CreateBellatraFontEffect(void)
{
	...
	s_EventScore_Font = ::CreateFont(16, 0, 0, 0, FW_THIN, FALSE, FALSE, FALSE, ANSI_CHARSET | FS_LATIN1, OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH | FF_DONTCARE, dwFontGame);
#endif
