...
namespace UI
{
	...
	{
		if ((cItemWrapper.sItem.iChk1 == pcItemData->sItem.iChk1) && (cItemWrapper.sItem.iChk2 == pcItemData->sItem.iChk2) && (cItemWrapper.sPosition.Equals(&pcItemData->sPosition)))
			return TRUE;
	...
	DWORD ItemInfoBox::GetItemPlayTime(EItemID eID)
	{
		DWORD dwTime = 0;
		switch (eID)
		{
	...
	BOOL ItemInfoBox::IsExpiredItemTime(Item* psItemInfo)
	{
		...
		DWORD dwTime = ItemInfoBox::GetItemPlayTime(eItemID);
	...
	BOOL ItemInfoBox::FormatItemTimeInfo(ItemData* pcItemData)
	{
		...
		DWORD dwTotalTime = ItemInfoBox::GetItemPlayTime(eItemID);
	...
	BOOL ItemInfoBox::PrepareShowItem(ItemData* pcItemData, BOOL bShowBuyPrice, BOOL bShowSellPrice, BOOL bShowChecksums)
	{
		...
		if ((*(BOOL*)0x0363C110) || (*(BOOL*)0x03642090))
		{
			if (bShowSellPrice)
			{
				// Next Line
