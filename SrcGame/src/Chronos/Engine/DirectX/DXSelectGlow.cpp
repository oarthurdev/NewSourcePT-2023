...
namespace DX
{
	...
	void SelectGlow::Init(Renderer_ptr _pRenderer)
	{
		...
		pEffect = pRenderer->GetGraphicsDevice()->GetEffectFactory()->MakeEffect("game/shaders/SelectGlow.fx");

		pDepthStencilBuffer = pRenderer->GetGraphicsDevice()->MakeDepthStencilBuffer(0, 0);
		pRenderTarget1 = pRenderer->GetGraphicsDevice()->MakeRenderTarget(0, 0);
		pRenderTarget2 = pRenderer->GetGraphicsDevice()->MakeRenderTarget(1, 0);
