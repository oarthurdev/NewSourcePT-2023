...
#define MAX_ITEM2PASS_TEX 14
static smTEXTUREHANDLE* s_Item2PassTexHandleBuff[MAX_ITEM2PASS_TEX];
static char* s_Item2PassTexName[MAX_ITEM2PASS_TEX] =
{
 "mixM_01.bmp", "mixM_02.bmp", "mixM_03.bmp", "mixM_04.bmp",
 "mixM_05.bmp", "mixM_06.bmp", "mixM_07.bmp", "mixM_08.bmp",
 "mixM_09.bmp", "mixM_10.bmp", "mixM_11.bmp", "mixM_12.bmp",
 "mixM_13.bmp", "mixM_14.bmp",
...
smRENDER3D::smRENDER3D()
{
	...
	//{
	// SMSHIFT_PERSPECTIVE_WIDTH = 0; // Widescreen
	//}
	//else
	//{
...
int smRENDER3D::RenderD3D(int MatNum)
{
	...
	if (RendMatrial[MatNum].MatrialCounter)
	{
		...
		GRAPHICDEVICE->SetFVF(D3DFVF_TEXTURE[smRendMaterial[MatNum].TextureCounter + TexCnt]);
		if ((rendcnt / 3) > 0)
			GRAPHICDEVICE->DrawPrimitiveUP(D3DPT_TRIANGLELIST, rendcnt / 3, (LPVOID)RendBuff, D3DFVF_SIZE[smRendMaterial[MatNum].TextureCounter + TexCnt]);
	...
	if (RendMatrial[MatNum].TexClip_MatrialCounter)
	{
		...
		GRAPHICDEVICE->SetFVF(D3DFVF_TEXTURE[smRendMaterial[MatNum].TextureCounter]);
		if ((rendcnt / 3) > 0)
			GRAPHICDEVICE->DrawPrimitiveUP(D3DPT_TRIANGLELIST, rendcnt / 3, (LPVOID)RendBuff, D3DFVF_SIZE[smRendMaterial[MatNum].TextureCounter]);
...
int smRENDER3D::RenderD3D_OutLine(void)
{
	...
	GRAPHICDEVICE->SetRenderState(D3DRS_TEXTUREFACTOR, OutLineColor);
	GRAPHICDEVICE->SetTextureStageState(0, D3DTSS_TEXCOORDINDEX, 0);
	GRAPHICDEVICE->SetTextureStageState(0, D3DTSS_COLOROP, D3DTOP_MODULATE);
	GRAPHICDEVICE->SetTextureStageState(0, D3DTSS_COLORARG1, D3DTA_TFACTOR);
	GRAPHICDEVICE->SetTextureStageState(0, D3DTSS_COLORARG2, D3DTA_CURRENT);
	GRAPHICDEVICE->SetTextureStageState(0, D3DTSS_ALPHAOP, D3DTOP_MODULATE);
	GRAPHICDEVICE->SetTextureStageState(0, D3DTSS_ALPHAARG1, D3DTA_TEXTURE);
	GRAPHICDEVICE->SetTextureStageState(0, D3DTSS_ALPHAARG2, D3DTA_CURRENT);
	...
	GRAPHICDEVICE->SetTexture(1, NULL);
	GRAPHICDEVICE->SetTextureStageState(1, D3DTSS_TEXCOORDINDEX, 0);
	GRAPHICDEVICE->SetTextureStageState(1, D3DTSS_COLOROP, D3DTOP_DISABLE);
	GRAPHICDEVICE->SetTextureStageState(1, D3DTSS_ALPHAOP, D3DTOP_DISABLE);
	...
	for (cnt = 0; cnt < RMatListCnt; cnt++)
	{
		...
		{
			GRAPHICDEVICE->SetTexture(0, lpMaterial->smTexture[0]->lpD3DTexture);
...
int smRENDER3D::RenderD3D_OutLine(int MatNum)
{
	...
	GRAPHICDEVICE->SetFVF(D3DFVF_XYZRHW | D3DFVF_DIFFUSE | D3DFVF_TEX1);
	if ((rendcnt / 3) > 0)
		GRAPHICDEVICE->DrawPrimitiveUP(D3DPT_TRIANGLELIST, rendcnt / 3, (LPVOID)RendBuff, 0x1C);
