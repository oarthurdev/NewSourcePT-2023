...
smTEXTUREHANDLE* smTEXTURE::Add(char* name, char* nameA)
{
	...
	{
		Logger::getInstance()->output(LogType::Error, "[smTEXTURE::Add] - [Buffer Overflow][Capacity: %d] - [required: %d] Buffer: (%s)", sizeof(Texture[TextureCnt].Name), strlen(name), name);
		return NULL;
	}
...
int smMATERIAL_GROUP::CreateTextureMaterial(char* szBitmap, char* szOpacity, int TwoSide, int Shade, DWORD TexSwap, DWORD BlendType)
{
	...
	if (!PathFileExists(szBitmap))
	{
		Logger::getInstance()->output(LogType::Error, "smMATERIAL_GROUP::CreateTextureMaterial() texture %s not found", szBitmap);
		return NULL;
	}
...
smMATERIAL* smMATERIAL_GROUP::FindNextMaterialFromTextureName()
{
	...
	//RUBINHO CORRE플O TELEPORTE
	for (cnt = LastSearchMaterial; cnt < MaterialCount; cnt++) {
		if (smMaterial[cnt].InUse) {

			for (cnt2 = 0; cnt2 < smMaterial[cnt].TextureCounter; cnt2++) {
				if (smMaterial[cnt].smTexture[cnt2] && lstrcmpi(smMaterial[cnt].smTexture[cnt2]->Name, szName) == 0) {
					return	&smMaterial[cnt];
				}
			}

			for (cnt2 = 0; cnt2 < smMaterial[cnt].AnimTexCounter; cnt2++) {
				if (smMaterial[cnt].smAnimTexture[cnt2] && lstrcmpi(smMaterial[cnt].smAnimTexture[cnt2]->Name, szName) == 0) {
					return	&smMaterial[cnt];
				}
			}
...
{
	return smMaterialGroup ? smMaterialGroup->CreateTextureMaterial(szBitmap, szOpacity, TwoSide, Shade, TexSwap, BlendType) : NULL;
}
...
LPDIRECT3DTEXTURE9 LoadDibSurfaceOffscreen(char* Filename)
{
...

	{
...

		{
			...
			if (GetFileExtension(Filename).compare("bmp") == 0)
			{
				if (!UI::DecryptBMP(pBuffer, iSize))
				{
					delete[] pBuffer;
					return NULL;
				}
			...
			if (FAILED(D3DXCreateTextureFromFileInMemoryEx(GRAPHICDEVICE, pBuffer, iSize, ImageInfo.Width, ImageInfo.Height, D3DX_FROM_FILE, 0, D3DFMT_FROM_FILE, D3DPOOL_MANAGED, D3DX_FILTER_POINT, D3DX_DEFAULT, 0xFF000000, &ImageInfo, NULL, &Image)))
			{
				delete[] pBuffer;
				return NULL;
			}
	...
	{
		Logger::getInstance()->output(LogType::Error, "Sprite %s not found", Filename);
...
int new_smCreateTexture(smTEXTUREHANDLE* lpTexHandle, int MipMap)
{
	if (PathFileExists(lpTexHandle->Name))
	{
		...
		if (pFile)
		{
			...
			{
				if (!UI::DecryptTGA(pBuffer, iSize))
			...
			if (FAILED(D3DXCreateTextureFromFileInMemoryEx(GRAPHICDEVICE, pBuffer, iSize, ImageInfo.Width, ImageInfo.Height, D3DX_FROM_FILE, 0, D3DFMT_FROM_FILE, D3DPOOL_MANAGED, D3DX_FILTER_POINT, D3DX_DEFAULT, 0xFFF0FFFF, &ImageInfo, NULL, &lpTexHandle->lpD3DTexture)))
