#define _WIN32_WINNT    0x0501  /* Get new material from the headers */
...
static HDC	hdc = NULL;
static int Quit = 0;
static char* szAppName = "Priston Tale";
...
#ifdef _LANGUAGE_BRAZIL
	SetWindowText(hwnd, "Server [ Priston Tale ]");
#endif
...
extern	SocketData	smWSock[CONNECTMAX];
...
int	rsDisconnectSystemLine(SocketData* lpsmSock);


int DisconnectUser(SocketData* lpsmSock);
...
int SendOpenCollectMoney(SocketData* lpsmSock);
...
int rsSendServerList(SocketData* lpsmSock, int ClanTicket);


int rsSendDataServer(SocketData* lpsmSock, void* szData);

int rsSendGameServer(SocketData* lpsmSock, void* szData);

int rsSendAreaServer(SocketData* lpsmSock, void* szData);
...
int rsShowBlackList(SocketData* lpsmSock, char* szID);
...
SocketData* lpsmSock_SerToServer[SER_TO_SERVER_SOCKET_MAX] = { 0,0,0,0 };
...
SocketData* rsFindNextSertoServerSock();
...
int rsLogOut(rsPLAYINFO* lpPlayInfo)
{
	...
	if (lpPlayInfo && lpPlayInfo->getAccount().size() && lpPlayInfo->lpsmSock)
	{
		...
		if (lpPlayInfo->getName().size())
		{
			...
			RecordLogSql(LOG_SQL_LOGOUT, lpPlayInfo->lpsmSock->szIP, lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(), lpPlayInfo->szGPCode, 1, lpPlayInfo->smCharInfo.JOB_CODE,
		...
		RecordGameLogSql(GAMELOG_SQL_LOGOUT, lpPlayInfo->lpsmSock->szIP,
		...
		if (lpPlayInfo->szGPCode[0])
		{
			...
			bSql_Logout(lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(), lpPlayInfo->lpsmSock->szIP, lpPlayInfo->szGPCode,
...
int srTransPlayUserData(rsPLAYINFO* lpPlayInfo)
{
	...
	if (lpPlayInfo->szChatMessage[0])
	{
		...
		{
			TransChatMessage.dwIP = lpPlayInfo->lpsmSock->lIP;
				...
				(rsPlayInfo[cnt].dwHopeDisconnectTime && rsPlayInfo[cnt].dwHopeDisconnectTime < dwPlayServTime))
			{
				...
				if (rsPlayInfo[cnt].lpsmSock)
				{
					wsprintf(szTempBuff, "ID( %s ) Name( %s ) IP( %s ) TimeOut Message\r\n",
						rsPlayInfo[cnt].getAccount().c_str(), rsPlayInfo[cnt].getName().c_str(), rsPlayInfo[cnt].lpsmSock->szIP);
...
int AddNewPlayInfo(SocketData* lpsmSock)
{
	int cnt, cnt2;
	DWORD	dwMyIP;
	int	SameIPCount = 0;
	...
	for (cnt = 0; cnt < CONNECTMAX; cnt++)
	{
		if (rsPlayInfo[cnt].lpsmSock && rsPlayInfo[cnt].lpsmSock->lIP == dwMyIP)
		{
			SameIPCount++;
		}
		else
	...
	if (ShutDownFlag)
	{
		...
		if (strstr(lpsmSock->szIP, szAdminIP) == 0)
		{
			DisconnectUser(lpsmSock);
			return FALSE;
		}
	...
	rsPlayInfo[cnt].dwTime_PrimeItem_AwellPower = 0;
	rsPlayInfo[cnt].dwTime_PrimeItem_MigalPower = 0;
	rsPlayInfo[cnt].dwTime_PrimeItem_HPBooster = 0;
	rsPlayInfo[cnt].dwTime_PrimeItem_MPBooster = 0;
	rsPlayInfo[cnt].dwTime_PrimeItem_SPBooster = 0;
	rsPlayInfo[cnt].dwTime_PrimeItem_SPRecover = 0;
	rsPlayInfo[cnt].dwTime_PrimeItem_BDivina = 0;
	rsPlayInfo[cnt].dwTime_PrimeItem_Force = 0;
	rsPlayInfo[cnt].dwTime_PrimeItem_ForceType = 0;
...
int SendShopItemList(SocketData* lpsmSock, smCHAR* lpChar)
{
	int cnt;
...
int SendSkillMenu(SocketData* lpsmSock, smCHAR* lpChar, int Mode)
{
	smTRANS_COMMAND		smTransCommand;
...
int SendOpenWareHouse(SocketData* lpsmSock)
{
	smTRANS_COMMAND		smTransCommand;
...
int SendOpenMixItem(SocketData* lpsmSock, int MixFlag)
{
	smTRANS_COMMAND		smTransCommand;
...
int SendOpenSmelting(SocketData* lpsmSock)
{
	smTRANS_COMMAND smTransCommand;
...
int SendOpenManufacture(SocketData* lpsmSock)
{
	smTRANS_COMMAND smTransCommand;
...
int SendOpenEventGift(SocketData* lpsmSock)
{
	smTRANS_COMMAND		smTransCommand;
...
int SendOpenAgingItem(SocketData* lpsmSock)
{
	smTRANS_COMMAND		smTransCommand;
...
int SendOpenMixtureItemReset(SocketData* lpsmSock)
{
	smTRANS_COMMAND		smTransCommand;
...
int rsSend_BadPlayerToGameServer(SocketData* lpsmSock, int BadPlayer)
{
	smTRANS_COMMAND		smTransCommand;
...
int rsSend_PlayerKilling(SocketData* lpsmSock)
{
	smTRANS_COMMAND		smTransCommand;
...
int SendOpenClanMenu(SocketData* lpsmSock)
{
	smTRANS_COMMAND		smTransCommand;
...
int rsSendAGameServer(SocketData* lpsmSock, void* szData)
{
	TRANS_SEND_DATASERVER	TransSendDataServer;
	int size;
...
int rsSendAGameServer_PrimeItem2(SocketData* lpsmSock, void* szData)
{
	TRANS_SEND_DATASERVER	TransSendDataServer;
	int size;
...
int rsSendAGameServer_PrimeItem3(SocketData* lpsmSock, void* szData)
{
	TRANS_SEND_DATASERVER	TransSendDataServer;
	int size;
...
int rsSendAGameServer_PrimeItem4(SocketData* lpsmSock, void* szData)
{
	TRANS_SEND_DATASERVER	TransSendDataServer;
	int size;
...
int	SendFindCrack(SocketData* lpsmSock)
{
	return TRUE;
}
...
int Serv_ConnectPlayer(SocketData* lpsmSock)
{
...
int Serv_DisconnectPlayer(SocketData* lpsmSock)
{
	rsPLAYINFO* tpInfo;
	int cnt;
	smTRANS_COMMAND	smTransCommand;
...
int rsLogAccount(rsPLAYINFO* lpPlayInfo, char* szID, char* szPassword)
{
	...
	if (lpPlayInfo->lpsmSock)
		return PushLogAccount(lpPlayInfo, szID, szPassword, lpPlayInfo->lpsmSock->szIP);
	else
		return FALSE;
}
...
int RecordHackLogFile(rsPLAYINFO* lpPlayInfo, void* lpBuff)
{
	...
	if (lpPlayInfo && lpPlayInfo->lpsmSock)
	{
		...
		wsprintf(szBuff, "%d:%d:%d IP-( %s ) ID-( %s ) Name-( %s ) / ( %d ) - ( %d , %d )\r\n",
			st.wHour, st.wMinute, st.wSecond, lpPlayInfo->lpsmSock->szIP, lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(),
	...
	if (lpPlayInfo && lpPlayInfo->lpsmSock)
	{
		...
		case 1000:
		{
			...
			wsprintf(szBuff, msg1000,
				...
				lpPlayInfo->lpsmSock->szIP, lpTransCommand->LParam);
		...
		case 1100:
		{
			wsprintf(szBuff, msg1100,
				st.wHour, st.wMinute, st.wSecond, lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(),
				lpPlayInfo->lpsmSock->szIP, (char*)lpTransCommand->LParam);
		...
		case 1103:
		{
			...
			wsprintf(szBuff, msg1103,
				st.wHour, st.wMinute, st.wSecond, lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(),
				lpPlayInfo->lpsmSock->szIP, lpTransCommand->SParam, lpTransCommand->LParam);
		}
		break;
		...
		case 1130:
		{
			...
			wsprintf(szBuff, msg1130,
				st.wHour, st.wMinute, st.wSecond, lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(),
				lpPlayInfo->lpsmSock->szIP, lpTransCommand->LParam, lpTransCommand->SParam);
		...
		case 2120:
		{
			...
			wsprintf(szBuff, msg2120,
				st.wHour, st.wMinute, st.wSecond, lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(),
				lpPlayInfo->lpsmSock->szIP, lpTransCommand->LParam, lpTransCommand->SParam, lpTransCommand->EParam);
		}break;
		case 2300:
		{
		...
		case 6910:
		{
			...
			wsprintf(szBuff, msg6910,
				st.wHour, st.wMinute, st.wSecond, lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(),
				lpPlayInfo->lpsmSock->szIP, lpTransCommand->LParam, (lpTransCommand->SParam & 0xFFFF), (lpTransCommand->SParam >> 16), lpTransCommand->EParam);
		}break;
		case 6920:
		{
		...
		case 7140:
		{
			...
			wsprintf(szBuff, msg7140,
				st.wHour, st.wMinute, st.wSecond, lpPlayInfo->getAccount().c_str(), lpPlayInfo->lpsmSock->szIP);
		...
		case 8000:
		{
			...
			wsprintf(szBuff, msg8000,
				st.wHour, st.wMinute, st.wSecond, lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(),
				lpPlayInfo->lpsmSock->szIP, lpTransCommand->WxParam, lpTransCommand->LxParam, lpTransCommand->LParam, lpTransCommand->SParam, lpTransCommand->EParam);
		}break;
		case 8010:
		{
		...
		case 8210:
		{
			...
			wsprintf(szBuff, msg8210,
				st.wHour, st.wMinute, st.wSecond, lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(),
				lpPlayInfo->lpsmSock->szIP, lpTransCommand->LParam, lpTransCommand->SParam, lpTransCommand->EParam, lpTransCommand->SxParam, lpTransCommand->ExParam);
		}break;
		case 8300:
		{
		...
		case 8510:
		{
			...
			wsprintf(szBuff, msg8510,
				st.wHour, st.wMinute, st.wSecond, lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(),
				lpPlayInfo->lpsmSock->szIP, lpTransCommand->WxParam,
		...
		case 8860:
		{
			...
			wsprintf(szBuff, msg8860,
				st.wHour, st.wMinute, st.wSecond, lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(),
				lpPlayInfo->lpsmSock->szIP, lpTransCommand->WxParam, lpTransCommand->LParam, lpTransCommand->SParam, lpTransCommand->EParam, lpTransCommand->LxParam);
		}break;
		case 8870:
		{
		...
		if (lpTransCommand->WParam < 1000)
		{
			...
			if (lpTransCommand->WParam != 100)
			{
				...
				wsprintf(szBuff, msgHack,
					st.wHour, st.wMinute, st.wSecond, lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(),
					lpPlayInfo->lpsmSock->szIP, lpTransCommand->WParam);
			...
			else
			{
				...
				wsprintf(szBuff, msgHack100,
					st.wHour, st.wMinute, st.wSecond, lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(),
					lpPlayInfo->lpsmSock->szIP, lpTransCommand->WParam, lpTransCommand->SParam, lpTransCommand->LParam, lpTransCommand->EParam);
...
int RecordDebugPlayLogFile(rsPLAYINFO* lpPlayInfo)
{
	...
	if (!lpPlayInfo->getAccount().empty())
	{
		wsprintf(szBuff, msgDebugloginID,
			st.wHour, st.wMinute, st.wSecond, lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(),
			lpPlayInfo->lpsmSock->szIP, (int)lpPlayInfo->getSecurity());
	}
	else
	{
...
int RecordJobChanger(rsPLAYINFO* lpPlayInfo)
{
	...
	wsprintf(szBuff, "ID( %s ) Name( %s ) IP( %s ) - 직업( %d )/전업( %d )/레벨( %d )\r\n",
		lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(), lpPlayInfo->lpsmSock->szIP,
...
int Record_BillingLogFile(rsPLAYINFO* lpPlayInfo, DWORD wParam, DWORD lParam, DWORD sParam, DWORD eParam)
{
	...
	wsprintf(szBuff, "%d:%d:%d - ID( %s ) Name( %s ) IP( %s ) - DisuseDay(%d) dwPeriodTime(%d) tTime(%d) PcUse(%d)\r\n", st.wHour, st.wMinute, st.wSecond,
		lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(), lpPlayInfo->lpsmSock->szIP, wParam, lParam, sParam, eParam);
...
int RecordMixItem(rsPLAYINFO* lpPlayInfo, sITEMINFO* lpSrcItem, sITEMINFO* lpMixItem)
{
	...
	wsprintf(szBuff, msgMixItem,
		st.wHour, st.wMinute, st.wSecond, lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(),
		lpPlayInfo->lpsmSock->szIP, lpSrcItem->ItemName,
...
int RecordAgingItem(rsPLAYINFO* lpPlayInfo, sITEMINFO* lpSrcItem, sITEMINFO* lpMixItem, int Mode)
{
	...
	if (lpMixItem)
	{
		...
		wsprintf(szBuff, lpMsg,
			st.wHour, st.wMinute, st.wSecond, lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(),
			lpPlayInfo->lpsmSock->szIP, lpSrcItem->ItemName, lpMixItem->ItemAgingNum[0],
			lpSrcItem->ItemHeader.Head, lpSrcItem->ItemHeader.dwChkSum, lpSrcItem->ItemHeader.dwTime,
			lpMixItem->ItemHeader.Head, lpMixItem->ItemHeader.dwChkSum, lpMixItem->ItemHeader.dwTime);
	...
	else
	{
		wsprintf(szBuff, msgAgingFailItem,
			st.wHour, st.wMinute, st.wSecond, lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(),
			lpPlayInfo->lpsmSock->szIP, lpSrcItem->ItemName, lpSrcItem->ItemAgingNum[0],
			lpSrcItem->ItemHeader.Head, lpSrcItem->ItemHeader.dwChkSum, lpSrcItem->ItemHeader.dwTime);
	}
...
int RecordCollectMoney(rsPLAYINFO* lpPlayInfo, int Money)
{
	...
	wsprintf(szBuff, msgKindPeople,
		st.wHour, st.wMinute, st.wSecond, lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(),
		lpPlayInfo->lpsmSock->szIP, Money, TotalMoney);
...
int RecordSmeltingItem(rsPLAYINFO* lpPlayInfo, sITEMINFO* lpSmeltingItem)
{
	...
	wsprintf(szBuff, msgSmeltingItem,
		st.wHour, st.wMinute, st.wSecond, lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(),
		lpPlayInfo->lpsmSock->szIP, lpSmeltingItem->ItemName,
...
int RecordManufactureItem(rsPLAYINFO* lpPlayInfo, sITEMINFO* lpManufactureItem)
{
	...
	wsprintf(szBuff, msgManufactureItem,
		st.wHour, st.wMinute, st.wSecond, lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(),
		lpPlayInfo->lpsmSock->szIP, lpManufactureItem->ItemName,
...
int RecordMixtureResetItem(rsPLAYINFO* lpPlayInfo, sITEMINFO* lpMixtureResetItem)
{
	...
	wsprintf(szBuff, msgMixtureResetItem,
		st.wHour, st.wMinute, st.wSecond, lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(),
		lpPlayInfo->lpsmSock->szIP, lpMixtureResetItem->ItemName,
...
int	rsRecord_ItemLOG(rsPLAYINFO* lpPlayInfo, DWORD dwItemCode, DWORD dwHead, DWORD dwChkSum, int Flag)
{
	...
	if (!rsServerConfig.TestSeverMode && (dwItemCode & sinITEM_MASK2) != sinGP1 &&
		...
		(dwItemCode & sinITEM_MASK2) != sinBI3 &&
		(dwItemCode & sinITEM_MASK2) != sinBI4 &&
		...
		(dwItemCode & sinITEM_MASK2) != sinSP1)
	{
		...
		else
		{
			...
			if ((dwItemCode & sinITEM_MASK2) == sinOS1 || (dwItemCode & sinITEM_MASK2) == sinFO1)
			{
				...
				if ((dwItemCode & sinITEM_MASK2) == sinOS1) {
	...
	LogItem.IP = lpPlayInfo->lpsmSock->lIP;
...
int	rsRecord_ItemLog_Trade(rsPLAYINFO* lpPlayInfo, rsPLAYINFO* lpPlayInfo2, TRANS_TRADE_ITEMKEY* lpTransTradeItemKey)
{
	...
	LogItem.IP = lpPlayInfo->lpsmSock->lIP;
	LogItem.TIP = lpPlayInfo2->lpsmSock->lIP;
...
int	rsRecord_ItemLog_MyShop(rsPLAYINFO* lpPlayInfo, rsPLAYINFO* lpPlayInfo2, DWORD dwItemCode, DWORD dwHead, DWORD dwChkSum, int Price)
{
	...
	LogItem.IP = lpPlayInfo2->lpsmSock->lIP;
	LogItem.TIP = lpPlayInfo->lpsmSock->lIP;
...
int	rsSendKillMonsterInfo(SocketData* lpsmSock)
{
	char	szBuff[4096];
...
int	rsSaveAdminCommand(rsPLAYINFO* lpPlayInfo, char* szChatCommand, int Level)
{
	...
	wsprintf(szBuff, "%d:%d:%d - ID( %s ) Name( %s ) IP( %s ) AdminLv( %d )> %s\r\n",
		st.wHour, st.wMinute, st.wSecond,
		lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(), lpPlayInfo->lpsmSock->szIP, Level, szChatCommand);
...
AccountTypes getSecurityLevel(rsPLAYINFO* lpPlayInfo)
{
	SocketData* lpsmSock = lpPlayInfo->lpsmSock;
...
int	rsClanMoneyLOG(rsPLAYINFO* lpPlayInfo, int Money, int RestMoney)
{
	...
	wsprintf(szBuff, "%d:%d:%d - ID( %s ) Name( %s ) IP( %s ) Money( %d ) RestMoney( %d )\r\n",
		st.wHour, st.wMinute, st.wSecond,
		lpPlayInfo->getAccount().c_str(), lpPlayInfo->getName().c_str(), lpPlayInfo->lpsmSock->szIP, Money, RestMoney);
...
int RecordBlessCastleSetup(rsPLAYINFO* lpPlayInfo, smTRANS_BLESSCASTLE* lpBlessCastleSetup)
{
	...
	wsprintf(szBuff, "%d:%d:%d - Name(%s)/IP(%s) MasterClan(%d) Price(%d)\r\n",
		st.wHour, st.wMinute, st.wSecond,
		lpPlayInfo->smCharInfo.szName, lpPlayInfo->lpsmSock->szIP,
...
int rsXignCode_resultCode_Verify(rsPLAYINFO* lpPlayInfo, XIGNCODE_PACKET* packet)
{
	if (ZSWAVE_Verify(lpPlayInfo->Xigncode_tempdata.xigncode_handle, (CPACKETDATA)packet->data, sizeof(packet->data)) != TRUE)
	{
		...
		case Z_RTN_NONCLIENT:
			rsXignCode_recordUtils_Log(lpPlayInfo, 8930, Z_RTN_NONCLIENT, szReport);
			lpPlayInfo->dwHopeDisconnectTime = dwPlayServTime + 5000;
			break;

		case Z_RTN_BLACK_CODE:
			rsXignCode_recordUtils_Log(lpPlayInfo, 8930, Z_RTN_BLACK_CODE, szReport);
			lpPlayInfo->dwHopeDisconnectTime = dwPlayServTime + 5000;
			break;
		...
		case Z_RTN_RESEND:
			...
			rsXignCode_recordUtils_Log(lpPlayInfo, 8930, Z_RTN_RESEND, szReport);
			break;
...
int rsXignCode_recordUtils_Log(rsPLAYINFO* lpPlayInfo, int msg_num, int result_data, char* szReport)
{
	char* return_value[6] = {
			"0"
			"Z_RTN_NONCLIENT",
