...
int rsRECORD_DBASE::ResotrRecordData(smCHAR_INFO *lpCharInfo, sITEM *lpItems, sITEM *lpItems2, sITEM *lpMouseItem)
{
	...
	for(cnt = 0; cnt < TransRecordData.ItemCount; cnt++)
	{
		...
		if(DeleteEventItem_TimeOut(&sRecordItem.sItemInfo) == TRUE)
		{
			...
			if(sRecordItem.sItemInfo.CODE == (sinCA1 | sin01) ||
				...
				sRecordItem.sItemInfo.CODE == (sinCA2 | sin01) || //Mudar no server
	...
	if(TransRecordData.GameSaveInfo.dwTime_PrimeItem_PhenixPet)
	{
		...
		if(PrimeItem_Time > 0)
		{
			...
			chaPremiumitem.SetUpKeepItem(nsPremiumItem::PHENIX_PET, chaPremiumitem.m_PhenixPetTime, true, UpKeepItemName[6], 40);
	...
	if (TransRecordData.GameSaveInfo.dwTime_PrimeItem_Caravan)
	{
		PrimeItem_Time = TransRecordData.GameSaveInfo.dwTime_PrimeItem_Caravan;
		if (PrimeItem_Time > 0)
		{
			chaPremiumitem.SetCaravanTime(PrimeItem_Time);
			chaPremiumitem.SetUpKeepItem(nsPremiumItem::CARAVAN, chaPremiumitem.m_CaravanTime, true, "Caravana");
		}
	}
