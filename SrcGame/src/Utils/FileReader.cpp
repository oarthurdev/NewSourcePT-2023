...
namespace INI
{
	...
	int CReader::ReadInt(const char* pszSection, const char* pszKey)
	{
		// Get Current Directory
		char szDirectory[512];
		GetCurrentDirectoryA(512, szDirectory);
		...
		// Read
		return GetPrivateProfileIntA(pszSection, pszKey, 0, szFormat);
	}
	...
	BOOL CReader::ReadBool(const char* pszSection, const char* pszKey)
	{
		char szBuffer[512];
		ZeroMemory(szBuffer, 512);
