...
std::vector<std::string> splitOnChar(const std::string& str, char delimiter)
{
	...
	while (pos != std::string::npos) {
		res.push_back(str.substr(initialPos, pos - initialPos));
		initialPos = pos + 1;
	...
	// Add the last one
	res.push_back(str.substr(initialPos, min(pos, str.size()) - initialPos + 1));
