...
int CLAN_MENU::Draw()
{
	...
	dsTextLineOut(0,destdc, 100, 0, szBuf, lstrlen(szBuf));

	SelectObject(hdc, ClanInfo[ClanInfo_nNum].hClanMark);
	BitBlt(destdc, 0, 0, 32, 32, sorDC, 0, 0, SRCCOPY);
	dsTextLineOut(0,destdc, 0, 50, ClanInfo[ClanInfo_nNum].ClanInfoHeader.ClanReport, lstrlen(ClanInfo[ClanInfo_nNum].ClanInfoHeader.ClanReport));
	DeleteDC(destdc);
	DeleteDC(sorDC);
...
void CLAN_MENU::Draw_C_Text(char *str, int x, int y, int sw = 0)
{
	...
	while(1)
	{
		...
		jmp1:
			...
			if(sw == 2)
			{
				...
				dsTextLineOut(0, x + 1, y + 1, strbuf, lstrlen(strbuf));

				SetFontTextColor( RGB(230, 255, 240));
				dsTextLineOut(0, x, y, strbuf, lstrlen(strbuf));
			}
			else
			{
				SetFontTextColor( RGB(230, 255, 240));
	...
	if(sw == 1)
	{
		...
		if(cnt < 20)
		{
			...
			dsTextLineOut(0, x + (lstrlen(str) * 6), y - 16, "|", 1);
		}
		if(cnt >= 40) cnt = 0;
	}
