...
void CLANEntity_chip::Draw()
{
	...
	switch(C_menuN)
	{
		...
		for(i = 0; i < DISPLAY_MAX; i++)
		{
			...
			if(nClanWon[nIconCount])
			{
				if(strcmp(cldata.clanWon[i + nScl_Bar], cldata.SubChip) == 0)
				{
					...
					dsTextLineOut(0,destdc, menuPos[nIconCount].left, menuPos[nIconCount].top, szClanWon, lstrlen(szClanWon));
				}
				else
				{
		...
		for(i = 0; i < DISPLAY_MAX; i++)
		{
			...
			{
				strcpy(szClanWon, cldata.clanWon[i + nScl_Bar]);
				dsTextLineOut(0,destdc, 0, 30 * i, szClanWon, lstrlen(szClanWon));
				dsTextLineOut(0,destdc, menuPos[nIconCount].left, menuPos[nIconCount].top, szClanWon, lstrlen(szClanWon));
				nIconCount += 1;
			}
		}
