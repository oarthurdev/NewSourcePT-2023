...
void CLANEntity_chatting::Draw_C_menuN()
{
	...
	switch(ChatList_CmenuN)
	{
		...
		if(cldata.myPosition == CLANCHIP)
		{
			for(i = 0; i < CHATING_DISPLAY_MAX; i++)
			{
				...
				if(ClanChatList.clanWon[i + nScl_Bar])
				{
					...
					dsTextLineOut(0,destdc, ChatList_menuPos[nMenuPos].left + 35, ChatList_menuPos[nMenuPos].top, szClanWon, lstrlen(szClanWon));
					nMenuPos += 1;
				}
			}
		}
		...
		dsTextLineOut(0,destdc, ChatListBoxX + 135, 85, cldata.name, lstrlen(cldata.name));
