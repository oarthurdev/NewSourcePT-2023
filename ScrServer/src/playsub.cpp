...
int AddExp(int Exp)
{
	if (lpCurPlayer->OnStageField == rsSOD_FIELD)
	{
		return FALSE;
	}
...
static DWORD GetRefromItemCodeLock(sITEMINFO* lpItemInfo, int Key, int locked)
{

	iFormCode = 0;
	iFromCnt = Key;

	lpItemInfo->ItemLocked = locked;
...
static DWORD rsGetRefromItemCode(sITEMINFO *lpItemInfo, int Key)
{
	...
	rsReformStateCode((char *)&lpItemInfo->ItemLocked, sizeof(int));
...
int ReformItemLock(sITEMINFO* lpItemInfo, int locked)
{
	DWORD dwCode;
	...
	dwCode = GetRefromItemCodeLock(lpItemInfo, lpItemInfo->ItemHeader.Head, locked);
