...
public:
	...
	int SendRecordDataToServer(SocketData *lpsmSock);
	...
	int SendRecordDataToClient(rsPLAYINFO *lpPlayInfo, char *szName, SocketData *lpsmSock, int Mode = 0);
	...
	int SendUserDataToClient(const char *szID, SocketData *lpsmSock, char *szServerID);
...
int rsSaveWareHouseData(const char *szID, TRANS_WAREHOUSE *lpTransWareHouse, int Caravan = 0);

int rsLoadWareHouseData(rsPLAYINFO *lpPlayInfo, int Caravan = 0);
