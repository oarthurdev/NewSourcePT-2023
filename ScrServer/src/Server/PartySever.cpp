...
float PartyExpTable[8][2] = {
	{ 1.f, 0 },       // 1 Player = 100%
	{ 1.f, 1.f }, // 2 Player = 140% 
	{ 1.f, 1.f }, // 3 Player = 180%
	{ 1.f, 1.f }, // 4 Player = 220%
	{ 1.f, 1.f }, // 5 Player = 260%
	{ 1.f, 1.f }, // 6 Player = 300%
	{ 1.f, 1.f },
	{ 1.f, 1.f }
};


extern bool PunicaoExp(int Level, int Field);
extern int PorcentagemPunicaoExp(int Field);
			...
			if (lpPartyPlayers[cnt] && lpPartyPlayers[cnt]->lpsmSock)
			{
				...
				if (smTransCommand.WParam)
				{
					...
					if (ExpUp)
					{
						...
						if (!PunicaoExp(lpPartyPlayers[cnt]->smCharInfo.Level, lpPlayInfo->Position.Area))
							smTransCommand.WParam -= (smTransCommand.WParam * PorcentagemPunicaoExp(lpPlayInfo->Position.Area)) / 100;
