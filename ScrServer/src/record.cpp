...
static int GetWareHouseFile(const char *szName, char *szFileName, int Caravan = 0)
{
	...
	wsprintf(szFileName, "DataServer\\%s\\%d\\%s.war", szRecordWareHouseDir, GetUserCode(szName), szName, Caravan ? "war" : "war");
...
int rsRECORD_DBASE::ResotrRecordData(smCHAR_INFO *lpCharInfo, sITEM *lpItems, sITEM *lpItems2, sITEM *lpMouseItem)
{
	...
	for (cnt = 0; cnt < TransRecordData.ItemCount; cnt++)
	{
		...
		if (DeleteEventItem_TimeOut(&sRecordItem.sItemInfo) == TRUE)
		{
			...
			if (sRecordItem.sItemInfo.CODE == (sinCA1 | sin01) ||
				...
				sRecordItem.sItemInfo.CODE == (sinOR2 | sin01) ||
	...
	if (TransRecordData.GameSaveInfo.dwTime_PrimeItem_BigHead)
	{
		...
		if (PrimeItem_Time > 0)
		{
			chaPremiumitem.SetBigHeadTime(PrimeItem_Time);
			chaPremiumitem.SetUpKeepItem(nsPremiumItem::BIG_HEAD, chaPremiumitem.m_BigHeadTime, true, UpKeepItemName[13]);
		}
	}

	if (TransRecordData.GameSaveInfo.dwTime_PrimeItem_AwellPower)
	{
		PrimeItem_Time = TransRecordData.GameSaveInfo.dwTime_PrimeItem_AwellPower;
		if (PrimeItem_Time > 0)
		{
			chaPremiumitem.SetAwellPowerTime(PrimeItem_Time);
			chaPremiumitem.SetUpKeepItem(nsPremiumItem::AWELLPOWER, chaPremiumitem.m_AwellPowerTime, true, UpKeepItemName[14]);
		}
	}

	if (TransRecordData.GameSaveInfo.dwTime_PrimeItem_MigalPower)
	{
		PrimeItem_Time = TransRecordData.GameSaveInfo.dwTime_PrimeItem_MigalPower;
		if (PrimeItem_Time > 0)
		{
			chaPremiumitem.SetMigalPowerTime(PrimeItem_Time);
			chaPremiumitem.SetUpKeepItem(nsPremiumItem::MIGALPOWER, chaPremiumitem.m_MigalPoweTime, true, UpKeepItemName[15]);
		}
	}

	if (TransRecordData.GameSaveInfo.dwTime_PrimeItem_HPBooster)
	{
		PrimeItem_Time = TransRecordData.GameSaveInfo.dwTime_PrimeItem_HPBooster;
		if (PrimeItem_Time > 0)
		{
			chaPremiumitem.SetHPBoosterTime(PrimeItem_Time);
			chaPremiumitem.SetUpKeepItem(nsPremiumItem::HPBOOSTER, chaPremiumitem.m_HPBoosterTime, true, UpKeepItemName[16]);
		}
	}

	if (TransRecordData.GameSaveInfo.dwTime_PrimeItem_MPBooster)
	{
		PrimeItem_Time = TransRecordData.GameSaveInfo.dwTime_PrimeItem_MPBooster;
		if (PrimeItem_Time > 0)
		{
			chaPremiumitem.SetMPBoosterTime(PrimeItem_Time);
			chaPremiumitem.SetUpKeepItem(nsPremiumItem::MPBOOSTER, chaPremiumitem.m_MPBoosterTime, true, UpKeepItemName[17]);
		}
	}

	if (TransRecordData.GameSaveInfo.dwTime_PrimeItem_SPBooster)
	{
		PrimeItem_Time = TransRecordData.GameSaveInfo.dwTime_PrimeItem_SPBooster;
		if (PrimeItem_Time > 0)
		{
			chaPremiumitem.SetSPBoosterTime(PrimeItem_Time);
			chaPremiumitem.SetUpKeepItem(nsPremiumItem::SPBOOSTER, chaPremiumitem.m_SPBoosterTime, true, UpKeepItemName[18]);
		}
	}

	if (TransRecordData.GameSaveInfo.dwTime_PrimeItem_SPRecover)
	{
		PrimeItem_Time = TransRecordData.GameSaveInfo.dwTime_PrimeItem_SPRecover;
		if (PrimeItem_Time > 0)
		{
			chaPremiumitem.SetRecoverSPTime(PrimeItem_Time);
			chaPremiumitem.SetUpKeepItem(nsPremiumItem::RECOVERSP, chaPremiumitem.m_RecoverSPTime, true, UpKeepItemName[19]);
		}
	}

	if (TransRecordData.GameSaveInfo.dwTime_PrimeItem_BDivina)
	{
		PrimeItem_Time = TransRecordData.GameSaveInfo.dwTime_PrimeItem_BDivina;
		if (PrimeItem_Time > 0)
		{
			chaPremiumitem.SetBDivinaTime(PrimeItem_Time);
			chaPremiumitem.SetUpKeepItem(nsPremiumItem::BDIVINA, chaPremiumitem.m_BDivinaTime, true, UpKeepItemName[20]);
		}
	}

	if (TransRecordData.GameSaveInfo.dwTime_PrimeItem_Force)
	{
		PrimeItem_Time = TransRecordData.GameSaveInfo.dwTime_PrimeItem_Force;
		if (PrimeItem_Time > 0)
		{
			chaPremiumitem.SetForceTime(PrimeItem_Time);
			chaPremiumitem.SetForceType(TransRecordData.GameSaveInfo.dwTime_PrimeItem_ForceType);
			chaPremiumitem.SetUpKeepItem(nsPremiumItem::FORCE, chaPremiumitem.m_ForceTime, true, UpKeepItemName[20 + TransRecordData.GameSaveInfo.dwTime_PrimeItem_ForceType], TransRecordData.GameSaveInfo.dwTime_PrimeItem_ForceType);
			AssaParticle_ShelltomWeapon(lpCurPlayer, (DWORD)60 * 60 * 1 * 1000, TransRecordData.GameSaveInfo.dwTime_PrimeItem_ForceType);
		}
	}
...
int rsRECORD_DBASE::SendRecordDataToServer(SocketData *lpsmSock)
{
	int cnt;
	int PartTotal;
	char *lpData;
...
int rsRECORD_DBASE::SendRecordDataToClient(rsPLAYINFO *lpPlayInfo, char *szName, SocketData *lpsmSock, int Mode)
{
	int cnt;
	int PartTotal;
	char *lpData;
...
int rsRECORD_DBASE::SendUserDataToClient(const char *szID, SocketData *lpsmSock, char *szServerID)
{
	char	szFile[256];
	char	szFile2[256];
	char	szFileInfo[256];
...
int rsSaveWareHouseData(const char *szID, TRANS_WAREHOUSE *lpTransWareHouse, int Caravan)
{
	char szFileName[128];
	FILE	*fp;

	GetWareHouseFile(szID, szFileName, Caravan);
...
int rsLoadWareHouseData(rsPLAYINFO *lpPlayInfo, int Caravan)
{
	char szFileName[128];
	TRANS_WAREHOUSE TransWareHouse = {};
	sWAREHOUSE	WareHouseCheck;
	...
	GetWareHouseFile(lpPlayInfo->getAccount().c_str(), szFileName, 0);
	...
	lpPlayInfo->LastWarehouseOpen = Caravan;
	TransWareHouse.dwTemp[0] = Caravan;
	if (!lpPlayInfo->OpenWarehouseInfoFlag)
	{

		Server_DebugCount = 510;

		lpPlayInfo->OpenWarehouseInfoFlag = 1;
		ZeroMemory(lpPlayInfo->WareHouseItemInfo, sizeof(lpPlayInfo->WareHouseItemInfo));
...
int rsLoadServerForce(rsPLAYINFO *lpPlayInfo, sGAME_SAVE_INFO *lpGameSaveInfo)
{
	...
	if (lpGameSaveInfo->dwTime_PrimeItem_Caravan > 0)
	{
		lpPlayInfo->dwTime_PrimeItem_Caravan = lpGameSaveInfo->dwTime_PrimeItem_Caravan;
	}
	else
	{
		lpPlayInfo->dwTime_PrimeItem_Caravan = 0;
	}

	if (lpGameSaveInfo->dwTime_PrimeItem_BigHead > 0)
		lpPlayInfo->dwTime_PrimeItem_BigHead = lpGameSaveInfo->dwTime_PrimeItem_BigHead;
	else
		lpPlayInfo->dwTime_PrimeItem_BigHead = 0;

	if (lpGameSaveInfo->dwTime_PrimeItem_AwellPower > 0)
		lpPlayInfo->dwTime_PrimeItem_AwellPower = lpGameSaveInfo->dwTime_PrimeItem_AwellPower;
	else
		lpPlayInfo->dwTime_PrimeItem_AwellPower = 0;

	if (lpGameSaveInfo->dwTime_PrimeItem_MigalPower > 0)
		lpPlayInfo->dwTime_PrimeItem_MigalPower = lpGameSaveInfo->dwTime_PrimeItem_MigalPower;
	else
		lpPlayInfo->dwTime_PrimeItem_MigalPower = 0;

	if (lpGameSaveInfo->dwTime_PrimeItem_HPBooster > 0)
		lpPlayInfo->dwTime_PrimeItem_HPBooster = lpGameSaveInfo->dwTime_PrimeItem_HPBooster;
	else
		lpPlayInfo->dwTime_PrimeItem_HPBooster = 0;

	if (lpGameSaveInfo->dwTime_PrimeItem_MPBooster > 0)
		lpPlayInfo->dwTime_PrimeItem_MPBooster = lpGameSaveInfo->dwTime_PrimeItem_MPBooster;
	else
		lpPlayInfo->dwTime_PrimeItem_MPBooster = 0;

	if (lpGameSaveInfo->dwTime_PrimeItem_SPBooster > 0)
		lpPlayInfo->dwTime_PrimeItem_SPBooster = lpGameSaveInfo->dwTime_PrimeItem_SPBooster;
	else
		lpPlayInfo->dwTime_PrimeItem_SPBooster = 0;

	if (lpGameSaveInfo->dwTime_PrimeItem_SPRecover > 0)
		lpPlayInfo->dwTime_PrimeItem_SPRecover = lpGameSaveInfo->dwTime_PrimeItem_SPRecover;
	else
		lpPlayInfo->dwTime_PrimeItem_SPRecover = 0;

	if (lpGameSaveInfo->dwTime_PrimeItem_BDivina > 0)
		lpPlayInfo->dwTime_PrimeItem_BDivina = lpGameSaveInfo->dwTime_PrimeItem_BDivina;
	else
		lpPlayInfo->dwTime_PrimeItem_BDivina = 0;

	if (lpGameSaveInfo->dwTime_PrimeItem_Force > 0)
	{
		lpPlayInfo->dwTime_PrimeItem_Force = lpGameSaveInfo->dwTime_PrimeItem_Force;
		lpPlayInfo->dwTime_PrimeItem_ForceType = lpGameSaveInfo->dwTime_PrimeItem_ForceType;
	}
	else
	{
		lpPlayInfo->dwTime_PrimeItem_Force = 0;
		lpPlayInfo->dwTime_PrimeItem_ForceType = 0;
	}
...
int rsSaveServerForce(rsPLAYINFO *lpPlayInfo, sGAME_SAVE_INFO *lpGameSaveInfo)
{
	...
	if (lpPlayInfo->dwTime_PrimeItem_Caravan > 0)
	{
		lpGameSaveInfo->dwTime_PrimeItem_Caravan = lpPlayInfo->dwTime_PrimeItem_Caravan;
	}
	else
	{
		lpGameSaveInfo->dwTime_PrimeItem_Caravan = 0;
	}

	if (lpPlayInfo->dwTime_PrimeItem_BigHead > 0)
		lpGameSaveInfo->dwTime_PrimeItem_BigHead = lpPlayInfo->dwTime_PrimeItem_BigHead;
	else
		lpGameSaveInfo->dwTime_PrimeItem_BigHead = 0;

	if (lpPlayInfo->dwTime_PrimeItem_AwellPower > 0)
		lpGameSaveInfo->dwTime_PrimeItem_AwellPower = lpPlayInfo->dwTime_PrimeItem_AwellPower;
	else
		lpGameSaveInfo->dwTime_PrimeItem_AwellPower = 0;

	if (lpPlayInfo->dwTime_PrimeItem_MigalPower > 0)
		lpGameSaveInfo->dwTime_PrimeItem_MigalPower = lpPlayInfo->dwTime_PrimeItem_MigalPower;
	else
		lpGameSaveInfo->dwTime_PrimeItem_MigalPower = 0;

	if (lpPlayInfo->dwTime_PrimeItem_HPBooster > 0)
		lpGameSaveInfo->dwTime_PrimeItem_HPBooster = lpPlayInfo->dwTime_PrimeItem_HPBooster;
	else
		lpGameSaveInfo->dwTime_PrimeItem_HPBooster = 0;

	if (lpPlayInfo->dwTime_PrimeItem_MPBooster > 0)
		lpGameSaveInfo->dwTime_PrimeItem_MPBooster = lpPlayInfo->dwTime_PrimeItem_MPBooster;
	else
		lpGameSaveInfo->dwTime_PrimeItem_MPBooster = 0;

	if (lpPlayInfo->dwTime_PrimeItem_SPBooster > 0)
		lpGameSaveInfo->dwTime_PrimeItem_SPBooster = lpPlayInfo->dwTime_PrimeItem_SPBooster;
	else
		lpGameSaveInfo->dwTime_PrimeItem_SPBooster = 0;

	if (lpPlayInfo->dwTime_PrimeItem_SPRecover > 0)
		lpGameSaveInfo->dwTime_PrimeItem_SPRecover = lpPlayInfo->dwTime_PrimeItem_SPRecover;
	else
		lpGameSaveInfo->dwTime_PrimeItem_SPRecover = 0;

	if (lpPlayInfo->dwTime_PrimeItem_BDivina > 0)
		lpGameSaveInfo->dwTime_PrimeItem_BDivina = lpPlayInfo->dwTime_PrimeItem_BDivina;
	else
		lpGameSaveInfo->dwTime_PrimeItem_BDivina = 0;

	if (lpPlayInfo->dwTime_PrimeItem_Force > 0)
	{
		lpGameSaveInfo->dwTime_PrimeItem_Force = lpPlayInfo->dwTime_PrimeItem_Force;
		lpGameSaveInfo->dwTime_PrimeItem_ForceType = lpPlayInfo->dwTime_PrimeItem_ForceType;
	}
	else
	{
		lpGameSaveInfo->dwTime_PrimeItem_Force = 0;
		lpGameSaveInfo->dwTime_PrimeItem_ForceType = 0;
	}
...
int CheckWareHouseData(char *szFile)
{
	...
	if ((WareHouse.Weight[0] + WareHouse.Weight[2]) > (WareHouse.Weight[1] + WareHouse.Weight[3]) || WareHouse.Money >= RECORD_MONEY_LIMIT)
	{
