...
const char* FormatTimeLong(__int64 iTotalSeconds)
{
	...
	if (iSeconds > 0)
	{
		...
		STRINGFORMAT(szTemp, "%I64d second%s", iSeconds, iSeconds == 1 ? "" : "s");
...
int STRCMP(const char* lhs, const int lhs_size, const char* rhs, const int rhs_size, bool case_sensitive)
{
	if (!lhs || !rhs)
	{
		std::abort();
...
int STRICMP(const char* lhs, const int lhs_size, const char* rhs, const int rhs_size)
{
	return STRCMP(lhs, lhs_size, rhs, rhs_size, false);
}


void STRCPY(char* dest, const int dest_capacity, const char* src)
{
	...
	strcpy_s(dest, dest_capacity, src);
