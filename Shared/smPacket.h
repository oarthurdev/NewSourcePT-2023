...
enum OpCode
{
	...
	OPCODE_GET_OPENCOINSHOP,
	OPCODE_GET_SHOPCOIN,
	OPCODE_GET_SHOPITEM,
	OPCODE_GET_SHOPITEM_T,
	OPCODE_GET_SHOPCOIN_T,
	OPCODE_BUY_SHOPITEM,
		OPCODE_BUY_SHOPITEM_T,
	HIGH_LEVEL_USER,
	EFFECT_HIGH_LEVEL_USER,
	OPCODE_SHOW_DMG,
	OPCODE_GET_RESTAURE,
	OPCODE_RESTAURE_LIST,
	OPCODE_RESTAURE_ITEM,
	OPCODE_GET_OPENRESTAURE,
	VALE_LEVEL,
	BOSS_TIME,
	PLAYER_GM,
	EFFECT_GM,
...
struct smTRANS_SHOPDATA
{
	...
	smTRANS_SHOPDATA()
	{
		...
		Count = 0;
		Section = -1;
		ZeroMemory(sCompItem, sizeof(sCOMPRESSED) * 15);
	}
};
...
struct smTRANS_HGINFO
{
	...
	smTRANS_HGINFO()
	{
		...
		ZeroMemory(szBossName, sizeof(szBossName));
	}
};
...
public:
	...
	DWORD	dwTime_PrimeItem_Caravan;
	DWORD	dwTime_PrimeItem_BigHead;
	DWORD	dwTime_PrimeItem_AwellPower;
	DWORD	dwTime_PrimeItem_MigalPower;
	DWORD	dwTime_PrimeItem_HPBooster;
	DWORD	dwTime_PrimeItem_MPBooster;
	DWORD	dwTime_PrimeItem_SPBooster;
	DWORD	dwTime_PrimeItem_SPRecover;
	DWORD	dwTime_PrimeItem_BDivina;
	DWORD	dwTime_PrimeItem_Force;
	int	dwTime_PrimeItem_ForceType;
	...
	bool setAccountId(int value)
	{
		...
		{
			Logger::getInstance()->output(LogType::Server, "[OUT OF RANGE]: setAccountId(int value): value < 0 [%d]", value);
		}
	...
	bool setAccount(const char* value)
	{
		...
		if (value)
		{
			...
			{
				Logger::getInstance()->output(LogType::Server, "[OVERFLOW]: setAccount(const char* value): strlen(value) > (ACCOUNT_SIZE - 1) [%s]", value);
			}
			else
			{
	...
	bool setPassword(const char* value)
	{
		...
		if (value)
		{
			...
			{
				Logger::getInstance()->output(LogType::Server, "[OVERFLOW]: setPassword(const char* value): strlen(value) > (PASSWORD_SIZE - 1) [%s]", value);
			}
			else
			{
	...
	bool setName(const char* value)
	{
		...
		if (value)
		{
			...
			if (len > NAME_SIZE - 1)
			{
				Logger::getInstance()->output(LogType::Server, "[OVERFLOW]: setName(const char* value): strlen(value) > (NAME_SIZE - 1) [%s]", value);
			}
			else
			{
...
struct	TRANS_WAREHOUSE
{
	...
	int DataSize;
	BYTE	Data[sizeof(sITEM) * 300 + 256];
};
